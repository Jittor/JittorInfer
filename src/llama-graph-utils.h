...
struct ggml_tensor * llm_build_moe_ffn_unmerge_ge(
    struct ggml_context * ctx, struct llama_context & lctx, struct ggml_cgraph * graph, struct ggml_tensor * cur,
    struct ggml_tensor * gate_inp, struct ggml_tensor * up_exps, struct ggml_tensor * gate_exps,
    struct ggml_tensor * down_exps, struct ggml_tensor * exp_probs_b, int64_t n_expert, int64_t n_expert_used,
    bool norm_w, bool scale_w, float w_scale, llama_expert_gating_func_type gating_op, const llm_build_cb & cb, int il);
