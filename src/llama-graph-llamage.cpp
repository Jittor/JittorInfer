...
struct ggml_tensor * llm_llama_context_ge::build_attn_indices() {
...
struct ggml_tensor * llm_llama_context_ge::build_length_q() {
...
struct ggml_tensor * llm_llama_context_ge::build_length_kv() {
...
struct ggml_cgraph * llm_llama_context_ge::build_llama_ge() {
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        // self-attention
        {
            ...
            cb(Qcur, "Qcur", il);
            // LLaMA has no QKV bias; only add when present
...
struct ggml_cgraph * llm_build_llama_ge(llama_context & lctx, std::vector<uint8_t> & buf_compute_meta,
                                        const llama_ubatch & ubatch, llm_build_cb & cb, bool worst_case,
                                        int print_layer) {
    struct ggml_cgraph * result = NULL;

    llm_llama_context_ge llm(lctx, buf_compute_meta, ubatch, cb, worst_case, print_layer);

    llm.init();

    result = llm.build_llama_ge();
...
void llm_update_llama_ge(llama_context & lctx) {
    ...
    for (int i = 0; i < n_nodes; i++) {
        ...
        if (cur->op == GGML_OP_FLASH_ATTN_PROMPT) {
            ...
            // MHA: key_num_heads == n_head_kv == n_head (=32 for LLaMA-2-7B)
