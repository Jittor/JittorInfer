    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        // self_attention
        {
            ...
            if (!is_lite) {
                ...
                if (hparams.merge_qk) {
                    // merged path: wq contains [q | kv_a_mqa] concatenated
                    ...
                    // non-merged path: wq for q only, wkv_a_mqa for kv separately
                    struct ggml_tensor * q_raw = ggml_mul_mat_fp16(ctx0, model.layers[il].wq, cur);
                    cb(q_raw, "q", il);

                    q_raw = ggml_reshape_3d(ctx0, q_raw, n_embd_head_qk_nope + n_embd_head_qk_rope, n_head, n_tokens);
                    struct ggml_tensor * qsplit[2];
                    ...
                    struct ggml_tensor * kv_pe_compresseed =
                        ggml_mul_mat_fp16(ctx0, model.layers[il].wkv_a_mqa, cur);
                    cb(kv_pe_compresseed, "kv_pe_compresseed", il);

                    kv_compressed = ggml_get_slice(ctx0, kv_pe_compresseed, 0, kv_lora_rank, 0);
                    ggml_set_name(kv_compressed, "kv_compressed");

                    k_pe = ggml_get_slice(ctx0, kv_pe_compresseed, kv_lora_rank,
                                          kv_lora_rank + n_embd_head_qk_rope, 0);
                    k_pe = ggml_reshape_3d(ctx0, k_pe, n_embd_head_qk_rope, 1, n_tokens);
                    ggml_set_name(k_pe, "k_pe");
            ...
            if (hparams.enable_mla) {
                ...
                cur = llm_attn_mla(ctx0, lctx, kv_self, gf, model.layers[il].wo, NULL, model.layers[il].wk_b,
                                   ...
                                   length_kv, n_embd_head_qk_nope, n_tokens, n_head, kq_scale, cb, il);
            } else {
        ...
        if ((uint32_t) il < hparams.n_layer_dense_lead) {
            if (hparams.merge_ffn) {
                ...
                cur = llm_build_ffn(ctx0, lctx, cur, model.layers[il].ffn_up, NULL, NULL,
                                    ...
                                    LLM_FFN_SILU, LLM_FFN_PAR, cb, il, true);
            }
            cb(cur, "ffn_out", il);
        } else {
            // MoE branch
            ...
            ggml_tensor * moe_out;
            if (hparams.merge_moe) {
                moe_out = llm_build_moe_ffn_merge(
                ...
                moe_out = llm_build_moe_ffn_unmerge_ge(
                    ctx0, lctx, gf, cur, model.layers[il].ffn_gate_inp, model.layers[il].ffn_up_exps,
                    model.layers[il].ffn_gate_exps, model.layers[il].ffn_down_exps, model.layers[il].ffn_exp_probs_b,
                    n_expert, n_expert_used, hparams.expert_weights_norm, true, hparams.expert_weights_scale,
                    (enum llama_expert_gating_func_type) hparams.expert_gating_func, cb, il);
            }
