...
static const char * GGML_OP_NAME[GGML_OP_COUNT] = {
    ...
    "DIAG_MASK_INF", "DIAG_MASK_ZERO", "SOFT_MAX", "SOFT_MAX_BACK", "ROPE", "ROPE_BACK", "ROPE_SIN_COS", "ROPE_CACHE",
    "CLAMP", "CONV_TRANSPOSE_1D", "IM2COL", "IM2COL_BACK", "CONV_TRANSPOSE_2D", "POOL_1D", "POOL_2D", "POOL_2D_BACK",
    "UPSCALE", "PAD", "PAD_REFLECT_1D", "ARANGE", "TIMESTEP_EMBEDDING", "ARGSORT", "LEAKY_RELU",
...
struct ggml_tensor * ggml_mla_prefill_jittor(struct ggml_context * ctx, struct ggml_tensor * query,
...
struct ggml_tensor * ggml_mla_preprocess(
...
struct ggml_tensor * ggml_moe_gating_topk_softmax(struct ggml_context * ctx, struct ggml_tensor * x, int k,
...
struct ggml_tensor * ggml_rope_sin_cos(struct ggml_context * ctx, struct ggml_tensor * x, struct ggml_tensor * sin,
...
struct ggml_tensor * ggml_rope_cache(struct ggml_context * ctx, struct ggml_tensor * sin, struct ggml_tensor * cos,
                                     int n_rot, int n_ctx, int n_ctx_orig, float freq_base, float freq_scale,
                                     float ext_factor, float attn_factor, float beta_fast, float beta_slow) {
