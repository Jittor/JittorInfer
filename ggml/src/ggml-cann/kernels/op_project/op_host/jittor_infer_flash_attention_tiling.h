...
constexpr uint32_t BYTE_BLOCK = 32;  // The block size of datacopy, which moves
                                     // data at the block granularity.
...
constexpr int32_t BLOCK_SIZE_BASE =
    128;  // The current requirement is a multiple of 128, and to prevent cross
          // block handling, the mm base is also set to 128.
...
constexpr uint32_t CVDIFF_SINNER_FACTOR_SMALL_KVS =
    512;  // kv_s <= 512 scene sinner slice size
constexpr uint32_t CVDIFF_SINNER_FACTOR_DEFAULT =
    1024;  // CV diff general scene sinner slice size
constexpr uint32_t CVDIFF_SINNER_FACTOR_SMALL_QS =
    2048;  // q_s <= 16 scene sinner slice size
...
END_TILING_DATA_DEF;
REGISTER_TILING_DATA_CLASS(PromptAttentionBaseParamsOp,
                           PromptAttentionBaseParams)
...
END_TILING_DATA_DEF;
REGISTER_TILING_DATA_CLASS(PromptAttentionSingleCoreParamsOp,
                           PromptAttentionSingleCoreParams)
...
END_TILING_DATA_DEF;
REGISTER_TILING_DATA_CLASS(PromptAttentionSingleCoreTensorSizeOp,
                           PromptAttentionSingleCoreTensorSize)
...
END_TILING_DATA_DEF;
REGISTER_TILING_DATA_CLASS(PromptAttentionInitOutputParamsOp,
                           PromptAttentionInitOutputParams)
...
TILING_DATA_FIELD_DEF_STRUCT(PromptAttentionBaseParams,
                             promptAttentionBaseParams);
TILING_DATA_FIELD_DEF_STRUCT(PromptAttentionSeqParams,
                             promptAttentionSeqParams);
TILING_DATA_FIELD_DEF_STRUCT(PromptAttentionSingleCoreParams,
                             promptAttentionSingleCoreParams);
TILING_DATA_FIELD_DEF_STRUCT(PromptAttentionSingleCoreTensorSize,
                             promptAttentionTensorSizeRect);
TILING_DATA_FIELD_DEF_STRUCT(PromptAttentionInitOutputParams,
                             promptAttentionInitOutputParams);
...
REGISTER_TILING_DATA_CLASS(JittorInferFlashAttention,
                           JittorInferFlashAttentionTilingData)
...
END_TILING_DATA_DEF;
REGISTER_TILING_DATA_CLASS(JittorInferFlashAttention_4000000000000000000,
                           MLAGeneralTilingData)
REGISTER_TILING_DATA_CLASS(JittorInferFlashAttention_4000000000000000001,
                           MLAGeneralTilingData)
...
class BufferNum {
   public:
    ...
    size_t bufferS1DNum;
    size_t bufferExpNum;  // unit: input dtype, shape: [S1, 1], inner axis align
                          // 32B.
};

class JittorInferFlashAttentionTiling {
   public:
    JittorInferFlashAttentionTiling(fe::PlatFormInfos* platFormInfo)
        : ascendcPlatform(platFormInfo) {}
    ge::graphStatus RunBigKernelTilingWithParams(
    ...
    ge::graphStatus JittorInferFlashAttentionSetTilingData(
   ...
   protected:
    ...
    void JittorInferFlashAttentionSplitNS(
        ContextParamsForPFATiling& contextKeyParams,
        JittorInferFlashAttentionTilingData& tilingData, uint32_t curCoreNum,
        std::vector<int64_t>& actualSeqLengths);
    void JittorInferFlashAttentionSplitNSNew(
        ContextParamsForPFATiling& contextKeyParams,
        JittorInferFlashAttentionTilingData& tilingData, uint32_t curCoreNum,
        std::vector<int64_t>& actualSeqLengths,
        std::vector<int64_t>& actualSeqLengthsKV, bool useBalanceTiling);
    void GetPreNextTokensLeftUp(JittorInferFlashAttentionTilingData& tilingData,
                                uint32_t actualSeqLength,
                                uint32_t actualSeqLengthKV,
                                int64_t& preTokensLeftUp,
                                int64_t& nextTokensLeftUp);
    void SetSplitCoreMode(JittorInferFlashAttentionTilingData& tilingData,
                          uint32_t sOuterFactor);
    void JittorInferFlashAttentionSplitSeqOneN(
        JittorInferFlashAttentionTilingData& tilingData, uint32_t curCoreNum,
        bool isVectorCore);
    bool EnableMTE2BmmPipe(JittorInferFlashAttentionTilingData& tilingData,
    ...
    ge::graphStatus AdjustBasicBlock(
        JittorInferFlashAttentionTilingData& tilingData,
        uint32_t& sOuterFactor);
    ge::graphStatus JittorInferFlashAttentionApiTiling(
    ...
    ge::graphStatus GetRectangleFactor(uint32_t seqSplit,
                                       std::queue<uint32_t>& sQueue,
    ...
    uint32_t CalculateL1SizeUsed(
        JittorInferFlashAttentionTilingData& tilingData,
        const uint32_t typeByteSize);
    bool CheckInputDimAndHeadNum(ContextParamsForPFATiling& contextKeyParams,
                                 uint32_t nQAttr, uint32_t nKVAttr);
    bool SetTilingHeadNumRatio(ContextParamsForPFATiling& contextKeyParams,
                               const int32_t* numQueryHeads,
                               const int32_t* numKeyValueHeads,
                               JittorInferFlashAttentionTilingData& tilingData);
    void JittorInferFlashAttentionInitOutputSplit(
        uint64_t totalSize, JittorInferFlashAttentionTilingData& tilingData,
        uint32_t curCoreNum);
    void JittorInferFlashAttentionInitSoftmaxLseOutputSplit(
        uint64_t totalSize, JittorInferFlashAttentionTilingData& tilingData);
    void Align(uint32_t& num);
    ge::graphStatus GetBasicShape(uint32_t& b, uint32_t& s, uint32_t& h,
                                  uint32_t& seqInnerSize,
                                  const gert::StorageShape* queryShape,
                                  const gert::StorageShape* keyShape,
                                  const uint32_t n);
    ge::graphStatus GetBasicShape910B(uint32_t& b, uint32_t& s, uint32_t& h,
                                      uint32_t& seqInnerSize,
                                      const gert::StorageShape* queryShape,
                                      const gert::StorageShape* keyShape,
                                      const uint32_t n);
    ...
    void GetMatMulType(matmul_tiling::DataType& mmInputType,
                       matmul_tiling::DataType& mmOutputType);
    ge::graphStatus CheckKeyValueParamsConsistency(
        const ContextParamsForPFATiling& contextKeyParams);
    bool CheckActualSeqLength(ContextParamsForPFATiling& contextKeyParams,
                              uint32_t b, uint32_t sQ, uint32_t sKV,
                              const gert::Tensor* actualSeqLenQ,
                              const gert::Tensor* actualSeqLenKV,
                              InputLayout inLayout,
                              JittorInferFlashAttentionTilingData& tilingData);
    bool CheckPseShiftTypeAndShape(ContextParamsForPFATiling& contextKeyParams,
                                   const gert::StorageShape* pseShiftShape,
                                   uint32_t b, uint32_t n, uint32_t s1,
                                   uint32_t s2);
    bool CheckPATypeAndShape(ContextParamsForPFATiling& contextKeyParams,
                             const gert::Tensor* actualSeqLenKV, int32_t b,
                             int32_t n, int32_t h, int32_t headNumRatio);
    bool CheckAttenMaskShape(ContextParamsForPFATiling& contextKeyParams,
                             const int32_t* sparseMode,
                             const gert::StorageShape* attenMaskShape,
                             uint32_t sQ, uint32_t sK, uint32_t batchSize);
    ge::graphStatus JittorInferFlashAttentionCVDiffSetTensorSize(
        JittorInferFlashAttentionTilingData& tilingData,
        PromptAttentionSingleCoreTensorSize& tensorSize, uint32_t sOuterFactor,
        uint32_t sInnerFactor, uint32_t softmaxSOuterFactor);
    bool JittorInferFlashAttentionComputeCVDiffParams(
        JittorInferFlashAttentionTilingData& tilingData, int64_t ubSize,
        int64_t l1Size, int64_t l0CSize, uint32_t typeByteSize,
        uint32_t& sOuterFactor, uint32_t& sInnerFactor, uint32_t maskTypeSize,
        uint32_t& softmaxSOuterFactor);
    bool FindOptimalTilingBasicBLock(
    ...
    bool FindOptimalTilingSouter(
        JittorInferFlashAttentionTilingData& tilingData, uint32_t& sOuterFactor,
        uint32_t& sInnerFactor, uint32_t& softmaxSOuterFactor, int64_t ubSize,
        uint32_t typeByteSize, uint32_t maskTypeSize);
    void InferTilingMod(const ContextParamsForPFATiling& contextKeyParams,
                        const std::vector<int64_t>& actualSeqLengths,
                        const std::vector<int64_t>& actualSeqLengthsKV,
                        uint32_t actualSeqArrayLen, uint32_t hDivN,
                        uint32_t seqInnerSize, int32_t sparseModeVal);
    ge::graphStatus AdjustCVTiling(
        uint32_t hDivN, uint32_t n, int64_t middle_actualSeqLengths,
        int64_t ubSize, int64_t l1Size, int64_t l0CSize, uint32_t maskElemSize,
        uint32_t& sOuterFactor, uint32_t& sInnerFactor,
        JittorInferFlashAttentionTilingData& tilingData);
    ge::graphStatus AdjustCVTilingCVDiff(
        int64_t ubSize, int64_t l1Size, int64_t l0CSize, uint32_t maskElemSize,
        uint32_t& sOuterFactor, uint32_t& sInnerFactor,
        uint32_t& softmaxSOuterFactor,
        JittorInferFlashAttentionTilingData& tilingData);
    bool CheckSparseModeRightDown(
        ContextParamsForPFATiling& contextKeyParams,
        const std::vector<int64_t>& actualSeqLengths,
    ...
    void UpdateTilingKeyFlag(ContextParamsForPFATiling& contextKeyParams,
                             uint64_t& tilingKey);
    int64_t JittorInferFlashAttentionSetMsdUbSize(
        JittorInferFlashAttentionTilingData& tilingData,
        PromptAttentionSingleCoreTensorSize& tensorSize,
        int32_t sInnerFactorTmp) const;

    ge::graphStatus CheckIOType(ContextParamsForPFATiling& contextKeyParams,
                                JittorInferFlashAttentionTilingData& tilingData,
                                int32_t& outputDataTypeSize);
    ge::graphStatus CheckMaskType(
        ContextParamsForPFATiling& contextKeyParams,
        JittorInferFlashAttentionTilingData& tilingData,
        uint32_t& maskElemSize);
    void SetMaskSize(const gert::StorageShape* attenMaskShape,
                     JittorInferFlashAttentionTilingData& tilingData);
    ge::graphStatus CheckShape(ContextParamsForPFATiling& contextKeyParams,
                               const gert::StorageShape* queryShape,
                               const gert::StorageShape* keyShape,
                               ...
                               const gert::StorageShape* pseShiftShape,
                               const gert::StorageShape* attenMaskShape);
    uint32_t CalcTschBlockDim(uint32_t sliceNum, uint32_t aicCoreNum,
                              uint32_t aivCoreNum);
   ...
   protected:
    ...
    uint32_t tmpS2 =
        0;  // In the PA scenario, there is no S2 axis. Use the change amount to
...
PFA_EXTERN_C ge::graphStatus TilingJittorInferFlashAttention(
    gert::TilingContext* context);
}  // namespace optiling
