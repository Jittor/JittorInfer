...
static ge::DataType ValidPfaDataType(ge::DataType type) {
    return (g_strDataTypePfa.find(type) == g_strDataTypePfa.end())
               ? ge::DT_UNDEFINED
               : type;
}

void JittorInferFlashAttentionTiling::UpdateTilingKeyFlag(
    ContextParamsForPFATiling& contextKeyParams, uint64_t& tilingKey) {
    ...
    if ((queryDtype == ge::DT_FLOAT16) && (kvDtype == ge::DT_INT8)) {
        binaryFlag +=
            8;  // 4bit flag bit, the leftmost side indicates whether to perform
                // inverse quantization operation, with a corresponding value of
                // 2**3 = 8, and the remaining 3bit is reserved
    }
    tilingKey +=
        (binaryFlag *
         100000000000);  // If inverse quantization is performed, tilingKey
                         // should increase by 8*100000000000.
    return;
}

bool JittorInferFlashAttentionTiling::GetApiTmpSize(
    const uint32_t sOuterFactor, const uint32_t sInnerFactor,
    const uint32_t typeByteSize) {
    auto tmpShape = Shape({sOuterFactor, sInnerFactor});
    uint32_t softmaxTmpSize =
        GetSoftMaxMinTmpSize(tmpShape, typeByteSize, true);
    uint32_t softmaxFlashTmpSize =
        GetSoftMaxFlashMinTmpSize(tmpShape, typeByteSize, true, true);
...
size_t JittorInferFlashAttentionTiling::GetPFAWorkSpaceSize(
    ...
    uint64_t maxSpmSize =
        tilingData.promptAttentionTensorSizeRect.get_spmTmpSize();
    sysWorkspaceSize =
        defaultSysWorkspaceSize910B;  // sys workspace size default value
    if (tilingMod == TilingMod::CVDIFF) {
        int64_t mm1ResSize = tilingData.promptAttentionSingleCoreParams
                                 .get_singleProcessSOuterSize() *
                             tilingData.promptAttentionSingleCoreParams
                                 .get_singleProcessSInnerSize();
        int64_t mm2ResSize =
            tilingData.promptAttentionSingleCoreParams
                .get_singleProcessSOuterSize() *
        ...
        workspaceSize =
            sysWorkspaceSize + coreNum * softmaxDataTypeSize *
                                   (maxSpmSize + mm1ResSize * NUM_2 +
                                    mm2ResSize * NUM_2);  // 2:use 2mm ub

        if (enablePA) {
            workspaceSize += static_cast<uint64_t>(coreNum) * 2 * 2 *
                             64;  // 2 bmm, db, ensure alignment of each
                                  // structure 64B, dcci cacheline needs to
        }
    } else {
        if ((splitS2 == 1) && (splitD == 1)) {
            workspaceSize =
                sysWorkspaceSize +
                coreNum * softmaxDataTypeSize *
                    (maxSpmSize + NUM_2 *
                                      tilingData.promptAttentionTensorSizeRect
                                          .get_mmResUbSize() *  // 2 : 2 mm ub
                                      tilingData.promptAttentionSingleCoreParams
                                          .get_multiSmaxsInnerLoopTimes());
        } else {
...
ge::graphStatus
JittorInferFlashAttentionTiling::TilingGetTilingKeyAttentionAscendC(
    ...
    tilingData.promptAttentionBaseParams.set_attenMaskElemType(
        attenMaskElemType);

    tilingKey = 0U;
    // If not in CV diff template,when there is a tail block, tilingKey should
    // increase by 1.
    tilingKey += (tilingMod == TilingMod::CVDIFF) ||
                         (isSOuterNoTail && isSInnerNoTail && isDNoTail)
                     ? 0U
                     : 1U;
    tilingKey += inputDataType == ge::DT_BF16
                     ? 100U
                     : 0U;  // When the input qkv is BF16, add 100.
    tilingKey += inputDataType == ge::DT_INT8
                     ? 200U
                     : 0U;  // When the input qkv is INT8, add 200.
    tilingKey +=
        tilingMod == TilingMod::CVDIFF
            ? 1002U
            : 0U;  // 1002: Add 1000 when using CV diff; Without distinguishing
                   // between tail and no tail, add 2 uniformly.
    tilingKey += outputDataType == ge::DT_INT8
                     ? 20000U
                     : 0U;  // When output is INT8, add 20000.

    if (!useNewTiling) {
        return ge::GRAPH_SUCCESS;  // The old template does not consider NSD
                                   // differences, only 0, 1, 100, 101
    }

    tilingKey += 10U;  // New Template 10、11、15、16、110、111、115、116.
    tilingKey +=
        (inputLayout == InputLayout::BNSD) || (inputLayout == InputLayout::NSD)
            ? 5U
            : 0U;

    // The KV cache inverse quantization for CV diff currently only handles the
    // case where Q in the CV diff template is FP16.
    if ((inputDataType == ge::DT_FLOAT16 || inputDataType == ge::DT_BF16) &&
        (tilingMod == TilingMod::CVDIFF)) {
        tilingKey = 1012;  // 1012：CV diff, +1000; new_tiling, +10; not
                           // distinguishing between tail and total, +2.
        tilingKey +=
            ((inputDataType == ge::DT_FLOAT16) &&
             (innerPrecise == HIGH_PRECISION)) ||
                    (contextKeyParams.inputDataType == ge::DT_FLOAT16 &&
                     contextKeyParams.kDataType == ge::DT_INT8)
                ? 600
                : 0;  // fp16 high precision mode, regarded as a type 600.
        tilingKey += (inputDataType == ge::DT_BF16) ? 100 : 0;  // 100: bf16
        tilingKey += (outputDataType == ge::DT_BF16)
                         ? 10000
                         : 0;  // When the output dtype is bf16, tilingKey
                               // should increase by 10000.
        tilingKey +=
            (outputDataType == ge::DT_INT8)
                ? 20000
        ...
        tilingKey +=
            ((inputLayout == InputLayout::BSH) ||
             ...
             (inputLayout == InputLayout::BSND))
                ? 100000
                : 0;  // When the inputLayout is BSH, SH or BSND, plus 100000.
        tilingKey +=
            ((splitCoreMode != SplitCoreMode::SPLIT_NBS_CUBE &&
              splitCoreMode != SplitCoreMode::SPLIT_ONEN_CUBE) &&
                     enableMatmulNorm
                 ? 1000000
                 : 0);  // Only enable matmul tiling optimization, do not enable
                        // l1reuse, add 1000000, mutually exclusive with the
                        // following situation of 2000000.
        tilingKey += ((splitCoreMode == SplitCoreMode::SPLIT_NBS_CUBE ||
                       splitCoreMode == SplitCoreMode::SPLIT_ONEN_CUBE)
                          ? 2000000
                          : 0);  // l1reuse defaults to enabling matmul tiling
                                 // optimization, with an additional 2000000,
                                 // which is mutually exclusive from the 1000000
                                 // situation mentioned above.
        UpdateTilingKeyFlag(
            contextKeyParams,
            tilingKey);  // Determine whether to perform inverse quantization
                         // and generate a binary number by combining it with
                         // the remaining reserved bits, and take its decimal
                         // representation.
    }
...
void JittorInferFlashAttentionTiling::JittorInferFlashAttentionSplitNS(
    ...
    PromptAttentionSingleCoreParams* singleCoreParams =
        &tilingData.promptAttentionSingleCoreParams;
    PromptAttentionBaseParams* baseParams =
    ...
    for (uint32_t i = LOOP_BEGIN_NUM; i < arrayLen; i++) {
        ...
        sOuterBlockNums[i] =
            (seqLen + singleCoreParams->get_singleProcessSOuterSize() - 1) /
            (singleCoreParams->get_singleProcessSOuterSize());
        sInnerLoopTimes[i] =
            (seqLen + singleCoreParams->get_singleProcessSInnerSize() - 1) /
            (singleCoreParams->get_singleProcessSInnerSize());

        multiSmaxsInnerLoopTimes =
            std::max(multiSmaxsInnerLoopTimes, sInnerLoopTimes[i]);
...
void JittorInferFlashAttentionTiling::JittorInferFlashAttentionInitOutputSplit(
    uint64_t totalSize, JittorInferFlashAttentionTilingData& tilingData,
    uint32_t curCoreNum) {
    PromptAttentionInitOutputParams* initParams =
        &tilingData.promptAttentionInitOutputParams;

    uint32_t singleCoreSize =
        (totalSize + curCoreNum - 1) /
        (curCoreNum);  // Upward rounding, coreNum has been verified to be
                       // non-zero when obtained.

    if (outputType == ge::DT_INT8) {
        singleCoreSize =
            (singleCoreSize + 1) / 2 *
            2;  // 2：In the int8 scenario, when initializing, fill in 0
                // according to the half type, requiring that the number of
                // points allocated to each kernel must be even.
    }
...
void JittorInferFlashAttentionTiling::
    JittorInferFlashAttentionInitSoftmaxLseOutputSplit(
        uint64_t totalSize, JittorInferFlashAttentionTilingData& tilingData) {
    PromptAttentionInitOutputParams* initParams =
        &tilingData.promptAttentionInitOutputParams;
...
void JittorInferFlashAttentionTiling::JittorInferFlashAttentionSplitNSNew(
    ...
    std::vector<uint32_t> accumSOuterTilingNums(static_cast<size_t>(arrayLen),
                                                0U);
    ...
    // The tiling structure element needs to have a length greater than or equal
    // to the length specified by TILING_DATA_FIELD_DEF_ARR. If the tiling
    // structure definition specifies a length of 50, the vector definition
    // needs to compare its size with curCoreNum and take the larger value.
    const size_t tilingElementArrayLen =
        (static_cast<size_t>(curCoreNum) > 50UL)
            ? static_cast<size_t>(curCoreNum)
            : 50UL;
    ...
    for (uint32_t i = LOOP_BEGIN_NUM; i < arrayLen; i++) {
        ...
        sInnerLoopTimes[i] = (subSeqInnerLen + sInnerSize - 1) / sInnerSize +
                             sInnerPrefixLoopTimes;
        accumSOuterTilingNums[i] =
            (sOuterBlockNums[i] * baseParams->get_headNumSize()) +
            preAccumSOuterNum;
        ...
        if (nextTokensPerBatch == 0) {
            totalBlockWight += (static_cast<int64_t>(sOuterBlockNums[i]) + 1) *
                               static_cast<int64_t>(sOuterBlockNums[i]) /
                               NUM_2;  // div 2
        } else {
            totalBlockWight += static_cast<int64_t>(sOuterBlockNums[i]) *
                               static_cast<int64_t>(sInnerLoopTimes[i]);
        }
    }
    if ((!useBalanceTiling)) {
    ...
    float coreWightTarget =
        (float(totalBlockWight * baseParams->get_headNumSize()) /
         float(curCoreNum));
    ...
    for (uint32_t i = LOOP_BEGIN_NUM; i < baseParams->get_headNumSize(); i++) {
        for (uint32_t j = 0; j < arrayLen; j++) {
            if (baseParams->get_sparseMode() == SPARSE_MODE_RIGHT_DOWN) {
                nextTokensPerBatch =
                    actualSeqLengthsKV[j] - actualSeqLengths[j];
            } else {
                nextTokensPerBatch = baseParams->get_nextTokens();
            }
            for (uint32_t k = 0; k < sOuterBlockNums[j]; k++) {
                int64_t dif =
                    int64_t(coreWightTarget * float(curCore + 1)) - curWight;
...
void JittorInferFlashAttentionTiling::GetPreNextTokensLeftUp(
    JittorInferFlashAttentionTilingData& tilingData, uint32_t actualSeqLength,
    uint32_t actualSeqLengthKV, int64_t& preTokensLeftUp,
    ...
    if (baseParams->get_sparseMode() == SPARSE_MODE_RIGHT_DOWN) {
        ...
        nextTokensLeftUp =
            (int64_t)actualSeqLengthKV - (int64_t)actualSeqLength;
    } else if (baseParams->get_sparseMode() == SPARSE_MODE_BAND) {
        preTokensLeftUp = sparsePreTokens - (int64_t)actualSeqLengthKV +
                          (int64_t)actualSeqLength;
        nextTokensLeftUp = sparseNextTokens + (int64_t)actualSeqLengthKV -
                           (int64_t)actualSeqLength;
    } else {
...
void JittorInferFlashAttentionTiling::SetSplitCoreMode(
    JittorInferFlashAttentionTilingData& tilingData, uint32_t sOuterFactor) {
    PromptAttentionBaseParams* baseParams =
        &tilingData.promptAttentionBaseParams;
    ...
    uint32_t sOuterLoopByCube =
        (actualSeqLength + sOuterSizeByCube - 1) / sOuterSizeByCube;
    ...
    bool enableRingAttention =
        (contextKeyParamsPtr->isSoftMaxLseEnable == true);

    GetPreNextTokensLeftUp(tilingData, actualSeqLength, actualSeqLengthKV,
    ...
    bool baseCond = (d == MATMUL_NORM_MIN_HEADSIZE) &&
                    (inputTypeFp16 || inputTypeBf16) && (usePseShift == 0) &&
                    !enableRingAttention &&
                    (baseParams->get_isActualSeqLengthsNull() == 1) &&
                    (baseParams->get_isActualSeqLengthsKVNull() == 1) &&
                    (actualSeqLength == actualSeqLengthKV) &&
                    (tilingMod == TilingMod::CVDIFF);
    ...
    if (contextKeyParamsPtr->attentionMask != nullptr) {
        enableOneNByCubeToken =
            (preTokensLeftUp >= actualSeqLength &&
             nextTokensLeftUp >= actualSeqLengthKV) ||
            (nextTokensLeftUp ==
             0);  // When mask exists, only support nextTokens is 0 or all data
                  // are calculated.
        enableNBSByCubeToken =
            ((preTokensLeftUp >= actualSeqLength) &&
             (nextTokensLeftUp >= actualSeqLengthKV ||
              nextTokensLeftUp ==
                  0));  // When mask exists, only support the triangle scene or
                        // all data are calculated.
    }
    bool enableOneNByCubeSeqMode =
        actualSeqLength >= seq16K &&
        (b * n >= 12);  // 12 : b * n should be more than 12.
    bool enableNBSByCubeSeqMode =
        actualSeqLength >= seq3K && (b * n * sOuterLoopByCube >= coreNum);
    bool noBalance =
        (baseParams->get_headNumRatio() != 1 || b != 1 ||
         tilingData.promptAttentionInitOutputParams.get_needInit()) ||
        (actualSeqLength >= seq8K &&
         contextKeyParamsPtr->attentionMask == nullptr);
    if (baseCond && enableOneNByCubeToken && enableOneNByCubeSeqMode) {
        splitCoreMode = SplitCoreMode::SPLIT_ONEN_CUBE;
    } else if (baseCond && enableNBSByCubeToken && enableNBSByCubeSeqMode &&
               noBalance) {
        splitCoreMode = SplitCoreMode::SPLIT_NBS_CUBE;
    }
}

void JittorInferFlashAttentionTiling::JittorInferFlashAttentionSplitSeqOneN(
    JittorInferFlashAttentionTilingData& tilingData, uint32_t curCoreNum,
    bool isVectorCore) {
    PromptAttentionBaseParams* baseParams =
        &tilingData.promptAttentionBaseParams;
    ...
    uint32_t sOuterSize = singleCoreParams->get_singleProcessSOuterSize();
    if (!isVectorCore) {  // When viewed from the perspective of a cube, sOuter
                          // * 2 is used for kernel partitioning, and within
                          // each cube kernel, 2 vector kernels still receive
                          // sOuter.
        sOuterSize =
            sOuterSize * 2;  // 2 : sOuter * 2 is used for kernel partitioning.
        curCoreNum = curCoreNum / 2;  // 2 : within each cube kernel, 2 vector
                                      // kernels still receive sOuter.
    }
    ...
    int64_t outerBlockFirstColNums =
        (preTokensLeftUp < static_cast<int32_t>(actualSeqLength))
            ...
            : outerBlockNums;
    int64_t outerBlockLeftDownFirstColNums =
        outerBlockNums - outerBlockFirstColNums;
    int64_t leftDownBlockNums = (outerBlockLeftDownFirstColNums + 1) *
                                outerBlockLeftDownFirstColNums / 2;
    ...
    int64_t innerBlockFirstRowNums =
        (nextTokensLeftUp < static_cast<int32_t>(actualSeqLengthKV))
            ...
            : innerBlockNums;
    int64_t innerBlockRightUpFirstRowNums =
        innerBlockNums - innerBlockFirstRowNums;
    int64_t rightUpBlockNums =
        (innerBlockRightUpFirstRowNums + 1) * innerBlockRightUpFirstRowNums / 2;

    int64_t toCalcBlockNums =
        innerBlockNums * outerBlockNums - rightUpBlockNums - leftDownBlockNums;
    double perWeight =
        static_cast<double>(toCalcBlockNums) / static_cast<double>(curCoreNum);
    ...
    for (uint32_t sOuterIndex = 0; sOuterIndex < outerBlockNums;
         sOuterIndex++) {
        sInnerIndexStart =
            (sOuterIndex < outerBlockFirstColNums) ? 0 : (sInnerIndexStart + 1);
...
bool JittorInferFlashAttentionTiling::EnableMTE2BmmPipe(
    JittorInferFlashAttentionTilingData& tilingData,
    ...
    if (tilingData.promptAttentionBaseParams.get_seqSize() >
        16) {  // When the size is greater than 16, use xiaoe speculative
               // inference.
        return true;
    }
    uint32_t baseK = 32U;
    ...
    OPS_ERR_IF(
        ret != 0,
        OPS_REPORT_VECTOR_INNER_ERR("JittorInferFlashAttention",
                                    "bmm SetFixSplit failed, ret = %d!", ret),
        return false);
    bool res = bmm.GetTiling(bmmTilingData) != -1;
    return res;
}

void JittorInferFlashAttentionTiling::EnableBmmDoubleBuffer(
    TCubeTiling& bmmTilingData) {
    if ((bmmTilingData.get_depthA1() == 1) &&
        (bmmTilingData.get_depthB1() == 1)) {
        bmmTilingData.set_depthA1(2);  // 2 : depthA1
        bmmTilingData.set_depthB1(2);  // 2 : depthB1
    }
}

ge::graphStatus JittorInferFlashAttentionTiling::CheckKeyValueParamsConsistency(
    ...
    OPS_ERR_IF(contextKeyParams.kDataType != contextKeyParams.vDataType,
               OPS_REPORT_VECTOR_INNER_ERR(
                   ...
                   "tensor key dtype(%d) must be consistent with tensor value "
                   "dtype(%d)!",
    ...
    OPS_ERR_IF(keyDimNum != valueDimNum,
               OPS_REPORT_VECTOR_INNER_ERR(
                   contextKeyParams.opName,
                   "tensor key shape dimNum(%u) must be consistent with tensor "
                   "value shape dimNum(%u)!",
                   keyDimNum, valueDimNum),
               return ge::GRAPH_FAILED);
    OPS_ERR_IF(
        (keyDimNum < INPUT_QKV_SHAPE_MIN_DIMS) ||
            (keyDimNum > INPUT_QKV_SHAPE_MAX_DIMS),
        OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                    "tensor key shape dimNum(%u) is invalid! "
                                    "Only support range [%u, %u]",
                                    keyDimNum, INPUT_QKV_SHAPE_MIN_DIMS,
...
bool JittorInferFlashAttentionTiling::JittorInferFlashAttentionCheckBmm1(
    JittorInferFlashAttentionTilingData& tilingData,
    TCubeTiling& bmm1TilingData, int64_t l1SizeRemain, int64_t l0CSize,
    uint32_t sOuterFactor, uint32_t sInnerFactor, bool allGM,
    bool autoBaseMNK) {
    int32_t ret = 0;
    ...
    ret = bmm1.SetShape(sOuterFactor, sInnerFactor,
                        tilingData.promptAttentionBaseParams.get_headSize());
    OPS_ERR_IF(
        ret != 0,
        OPS_REPORT_VECTOR_INNER_ERR(contextKeyParamsPtr->opName,
                                    "bmm1 SetShape failed, ret = %d!", ret),
        return false);
    ...
    if ((inputLayout == InputLayout::BSH) || (inputLayout == InputLayout::SH) ||
        ...
        if (enablePA && PAlayoutType == 1) {  // The left matrix of PA is BNSD,
                                              // and the right matrix is BSH.
            bmm1.SetOrgShape(
                tilingData.promptAttentionBaseParams.get_seqSize(),
    ...
    OPS_ERR_IF(
        ret != 0,
        OPS_REPORT_VECTOR_INNER_ERR(contextKeyParamsPtr->opName,
                                    "bmm1 SetBufferSpace failed, l1SizeRemain "
                                    "= %ld, l0CSize = %ld, ret = %d!",
    ...
    OPS_ERR_IF(ret != 0,
               OPS_REPORT_VECTOR_INNER_ERR(
                   ...
                   "bmm1 SetFixSplit failed, l1SizeRemain = %ld, l0CSize = "
                   "%ld, sOuterFactor = %u, sInnerFactor = %u, ret = %d!",
    ...
    OPS_ERR_IF(
        ret != 0,  // Get tiling fail for bmm1.
        OPS_REPORT_VECTOR_INNER_ERR(
            contextKeyParamsPtr->opName,
            "bmm1 GetTiling failed, l1SizeRemain = %ld, l0CSize = %ld, "
            "sOuterFactor = %u, sInnerFactor = %u, autoBaseMNK = %d, ret = %d!",
            l1SizeRemain, l0CSize, sOuterFactor, sInnerFactor, autoBaseMNK,
            ret),
        return false);
    ...
    bool res = EnableMTE2BmmPipe(tilingData, bmm1, bmm1TilingData, sOuterFactor,
                                 sInnerFactor);  // Open MTE2 Matmul pipeline.

    OPS_ERR_IF(res == false,  // EnableMTE2BmmPipe fail.
               OPS_REPORT_VECTOR_INNER_ERR(contextKeyParamsPtr->opName,
                                           "EnableMTE2BmmPipe failed!"),
               return false);

    return true;
}

void JittorInferFlashAttentionTiling::GetMatMulType(
...
bool JittorInferFlashAttentionTiling::JittorInferFlashAttentionCheckBmm2(
    JittorInferFlashAttentionTilingData& tilingData,
    TCubeTiling& bmm2TilingData, int64_t l1SizeRemain, int64_t l0CSize,
    uint32_t sOuterFactor, uint32_t sInnerFactor, uint32_t dSplitFactor,
    ...
    matmul_tiling::DataType bmm2OutputType =
        matmul_tiling::DataType::DT_FLOAT16;
    GetMatMulType(bmm2InputType, bmm2OutputType);
    if ((splitS2 == 1) && (splitD == 1)) {
        bmm2.SetAType(matmul_tiling::TPosition::GM,
                      matmul_tiling::CubeFormat::ND, bmm2InputType, false);
        bmm2.SetBType(matmul_tiling::TPosition::GM,
                      matmul_tiling::CubeFormat::ND, bmm2InputType, false);
        bmm2.SetCType(matmul_tiling::TPosition::GM,
                      matmul_tiling::CubeFormat::ND, bmm2OutputType);
        ret = bmm2.SetShape(
            sOuterFactor, tilingData.promptAttentionBaseParams.get_headSize(),
            tilingData.promptAttentionBaseParams.get_seqInnerSize());
    } else {
        matmul_tiling::TPosition aPosition =
            allGM ? matmul_tiling::TPosition::GM
                  : matmul_tiling::TPosition::TSCM;
        matmul_tiling::TPosition cPosition =
            allGM ? matmul_tiling::TPosition::GM
                  : matmul_tiling::TPosition::VECCALC;
        bmm2.SetAType(aPosition, matmul_tiling::CubeFormat::NZ, bmm2InputType,
                      false);
        bmm2.SetBType(matmul_tiling::TPosition::GM,
                      matmul_tiling::CubeFormat::ND, bmm2InputType, false);
        bmm2.SetCType(cPosition, matmul_tiling::CubeFormat::ND_ALIGN,
                      bmm2OutputType);
        ret = bmm2.SetShape(sOuterFactor,
                            tilingData.promptAttentionBaseParams.get_headSize(),
                            sInnerFactor);
    }
    OPS_ERR_IF(
        ret != 0,
        OPS_REPORT_VECTOR_INNER_ERR(contextKeyParamsPtr->opName,
                                    "bmm2 set SetShape failed, sOuterFactor = "
    ...
    if ((inputLayout == InputLayout::BSH) ||
        ...
        (inputLayout == InputLayout::SH)) {
        bmm2.SetOrgShape(
            tilingData.promptAttentionBaseParams.get_seqSize(), strideV,
            tilingData.promptAttentionBaseParams.get_seqInnerSize());
    } else if ((inputLayout == InputLayout::BNSD) ||
               (inputLayout == InputLayout::NSD)) {
        if (enablePA &&
            PAlayoutType == 1) {  // The left matrix of PA is BNSD, and the
                                  // right matrix is of PA is BSH.
            bmm2.SetOrgShape(
                tilingData.promptAttentionBaseParams.get_seqSize(), strideV,
                tilingData.promptAttentionBaseParams.get_seqInnerSize());
        } else {
    ...
    OPS_ERR_IF(
        ...
        OPS_REPORT_VECTOR_INNER_ERR(
            contextKeyParamsPtr->opName,
            "bmm2 set SetBufferSpace failed, l1SizeRemain = %ld, l0CSize = "
            "%ld, sOuterFactor = %u, sInnerFactor = %u, ret = %d!",
            l1SizeRemain, l0CSize, sOuterFactor, sInnerFactor, ret),
        return false);
    if (inputType == ge::DT_INT8) {
    ...
    OPS_ERR_IF(
        ...
        OPS_REPORT_VECTOR_INNER_ERR(
            contextKeyParamsPtr->opName,
            "bmm2 set GetTiling failed, l1SizeRemain = %ld, l0CSize = %ld, "
            "sOuterFactor = %u, sInnerFactor = %u, autoBaseMNK = %d, ret = %d!",
            l1SizeRemain, l0CSize, sOuterFactor, sInnerFactor, autoBaseMNK,
            ret),
        return false);
...
void JittorInferFlashAttentionTiling::JittorInferFlashAttentionSetTensorSize(
    ...
    if (tilingData.promptAttentionBaseParams.get_useMask() == 0U &&
        usePseShift == 0U) {
        // In scenarios where attentionMask is not configured and there is no
        // pse, UB memory for attentionMask can be saved But 2 BYTE_BLOCK
        // (32BYTE) UB memory needs to be reserved for Bmm2UpdateDiv
        tensorSize.set_attenMaskUbSize(sOuterFactor * BYTE_BLOCK * NUM_2 /
                                       softmaxDataTypeSize);
    } else {
    ...
    tensorSize.set_softmaxExpSize(
        sOuterFactor *
        tilingData.promptAttentionBaseParams.get_softmaxTypeByteNum());
    tensorSize.set_softmaxValueSize(sOuterFactor * sInnerFactor);
    tensorSize.set_bmm2ResUbSize(
        sOuterFactor *
        tilingData.promptAttentionBaseParams.get_alignedHeadSize());
    tensorSize.set_tmpMMResBmm2PreUbSize(
        std::max(tensorSize.get_mmResUbSize(), tensorSize.get_bmm2ResUbSize()));
    tensorSize.set_tmpSoftmaxBmm2UbSize(SOFTMAX_BUFFER_NUM *
                                        tensorSize.get_softmaxMaxSize());
    if ((splitS2 == 1) && (splitD == 1)) {
        tensorSize.set_spmTmpSize(tensorSize.get_bmm2ResUbSize() +
                                  tensorSize.get_softmaxExpSize() *
                                      SPLIT_DOUBLE_UB);
    } else {
        tensorSize.set_spmTmpSize(tensorSize.get_bmm2ResUbSize());
    }
    tensorSize.set_softmaxMaxSize(sOuterFactor * (BYTE_BLOCK / sizeof(float)));
    if (tilingData.promptAttentionBaseParams.get_maskTypeByteNum() ==
        (BYTE_BLOCK / BOOLSIZE)) {
...
uint32_t JittorInferFlashAttentionTiling::CalculateL1SizeUsed(
    JittorInferFlashAttentionTilingData& tilingData,
    const uint32_t typeByteSize) {
    return (typeByteSize *
            tilingData.promptAttentionTensorSizeRect.get_scmTmpSize());
}

bool JittorInferFlashAttentionTiling::JittorInferFlashAttentionCheckArgsLegal(
    JittorInferFlashAttentionTilingData& tilingData, int64_t ubSize,
    ...
    int64_t softmaxFlashTmpSize = GetSoftMaxFlashV2MinTmpSize(
        tmpShape, typeByteSize, softmaxDataTypeNZ_, true, true);
    if ((softmaxTmpSize == 0) || (softmaxFlashTmpSize == 0)) {
        return false;
    }
    ...
    JittorInferFlashAttentionSetTensorSize(
        tilingData, tilingData.promptAttentionTensorSizeRect, sOuterFactor,
        sInnerFactor);
    int32_t l1SizeRemain =
        l1Size - CalculateL1SizeUsed(tilingData, typeByteSize);
    if (l1SizeRemain < 0) {
        updateDiv = true;
        res = false;
        return res;
    ...
    // 这里决定了能否按照这个参数分块
    res = (JittorInferFlashAttentionCheckBmm1(
              tilingData, tilingData.bmm1TilingDataRect, l1SizeRemain, l0CSize,
              sOuterFactor, sInnerFactor)) &&
          (JittorInferFlashAttentionCheckBmm2(
              tilingData, tilingData.bmm2TilingDataRect, l1SizeRemain, l0CSize,
              sOuterFactor, sInnerFactor, dSplitFactor));

    OPS_ERR_IF(res == false,
               OPS_REPORT_VECTOR_INNER_ERR(
                   contextKeyParamsPtr->opName,
                   "JittorInferFlashAttentionCheckBmm1 or "
                   "JittorInferFlashAttentionCheckBmm2 failed."),
               return false);

    queueBufferSize =
        tilingData.promptAttentionTensorSizeRect.get_attenMaskUbSize();

    pseShiftBufferSize =
        tilingData.promptAttentionTensorSizeRect.get_pseShiftUbSize();

    pseMaskMaxSize = std::max(maskTypeSize, pseShiftElemSize);

    uint32_t pseShiftCastSize = 0U;
    if ((usePseShift == 1) &&
        (((inputType == ge::DT_FLOAT16) && (innerPrecise == HIGH_PRECISION)) ||
         pseShiftElemType == ge::DT_BF16)) {
        pseShiftCastSize =
            FLOAT32SIZE;  // In the case of high-precision effectiveness or
                          // BF16, PSE needs to do a cast and apply for UB
    }
    ...
    if ((splitS2 == 1) && (splitD == 1)) {
        ubSizeRemain =
            ...
            (tilingData.promptAttentionTensorSizeRect.get_mmResUbSize() *
             ...
             SOFTMAX_BUFFER_NUM * tilingData.promptAttentionTensorSizeRect
                                      .get_softmaxExpSize()) *
                typeByteSize -
        ...
        ubSizeRemain =
            ubSize - apiTmpSize -
            (tilingData.promptAttentionTensorSizeRect.get_mmResUbSize() +
             SPLIT_DOUBLE_UB * tilingData.promptAttentionTensorSizeRect
                                   .get_softmaxExpSize()) *
                typeByteSize -
            (queueBufferSize * pseMaskMaxSize) -
            tilingData.promptAttentionTensorSizeRect.get_selectSpaceUbSize() -
...
ge::graphStatus
JittorInferFlashAttentionTiling::JittorInferFlashAttentionApiTiling(
    JittorInferFlashAttentionTilingData& tilingData, uint32_t typeSize,
    ...
    SoftMaxTilingFunc(softmaxShapeRect, sizeof(float),
                      ubSizeRemain + apiTmpSize,
                      tilingData.softmaxTilingDataRect);
    SoftMaxFlashV2TilingFunc(softmaxShapeRect, softmaxDataTypeSize,
                             sizeof(float), ubSizeRemain + apiTmpSize,
                             tilingData.softmaxFlashTilingDataRect, true, true);

    auto transposeSrcShapeRect =
        Shape({1, 1, sOuterFactor,
               tilingData.promptAttentionBaseParams.get_headSize()});
    auto transposeDstShape =
        Shape({tilingData.promptAttentionBaseParams.get_batchSize(),
               tilingData.promptAttentionBaseParams.get_headNumSize(),
               tilingData.promptAttentionBaseParams.get_seqSize(),
               tilingData.promptAttentionBaseParams.get_headSize() *
                   tilingData.promptAttentionBaseParams.get_headNumSize()});

    GetDataCopyTransposeTiling(transposeDstShape, transposeSrcShapeRect,
                               typeSize, tilingData.transposeTilingDataRect);
    return ge::GRAPH_SUCCESS;
}

ge::graphStatus
JittorInferFlashAttentionTiling::JittorInferFlashAttentionSetTilingData(
...
ge::graphStatus JittorInferFlashAttentionTiling::GetRectangleFactor(
    uint32_t seqFactorThreshold, std::queue<uint32_t>& sQueue,
    int32_t threshold) {
    for (int i = seqFactorThreshold; i >= threshold;
         i = (i - threshold)) {  // threshold 16
        sQueue.push(i);
    }
    return ge::GRAPH_SUCCESS;
}

ge::graphStatus JittorInferFlashAttentionTiling::SetInputLayout(
    const char* layout) {
    if (layout == nullptr) {
        inputLayout = InputLayout::BSH;
        return ge::GRAPH_SUCCESS;
...
bool JittorInferFlashAttentionTiling::CheckInputDimAndHeadNum(
    ContextParamsForPFATiling& contextKeyParams, const uint32_t nQAttr,
    const uint32_t nKVAttr) {
    uint32_t nQ = nQAttr;
    uint32_t nKV = nKVAttr;
    if (nKVAttr == 0U) {  // Detected that nKVAttr is the default value, which
                          // means that the customer did not pass in.
        nKV = nQAttr;
    }
    ...
    const uint32_t nIdx =
        inputLayout == InputLayout::BNSD ? 1U : 2U;  // BNSD: 1; BSND:2

    if (((inputLayout == InputLayout::BNSD) ||
         (inputLayout == InputLayout::BSND)) &&
        (!enablePA)) {
        if ((queryDim == 4) && (keyDim == 4) &&
            (valueDim == 4)) {  // dim num: 4
            ...
            OPS_LOG_E(contextKeyParams.opName,
                      "input dim of q(%u), k(%u), v(%u) must be 4 for BNSD or "
                      "BSND format!",
                      queryDim, keyDim, valueDim);
            return false;
        }
    } else if ((inputLayout == InputLayout::NSD) && (!enablePA)) {
        if ((queryDim == 3) && (keyDim == 3) &&
            (valueDim == 3)) {  // dim num: 3
            ...
            OPS_LOG_E(
                contextKeyParams.opName,
                "input dim of q(%u), k(%u), v(%u) must be 3 for NSD format!",
                queryDim, keyDim, valueDim);
            return false;
        }
    }
...
bool JittorInferFlashAttentionTiling::SetTilingHeadNumRatio(
    ContextParamsForPFATiling& contextKeyParams, const int32_t* numQueryHeads,
    ...
    if ((nQ < 0) || (nKV < 0)) {
        OPS_LOG_E(contextKeyParams.opName,
                  "numHeads(%d) or numKeyValueHeads(%d) is negative!", nQ, nKV);
        return false;
    }
    ...
    if (nKV == 0) {  // Detected that nKV is the default value, which means that
                     // the customer did not pass in.
    ...
    if (nQ % nKV != 0) {
        OPS_LOG_E(contextKeyParams.opName,
                  "numHeads(%d) must be divisible by numKeyValueHeads(%d)!", nQ,
                  nKV);
        return false;
    } else {
        if (nQ / nKV > 64) {  // G cannot be greater than 64.
            OPS_LOG_E(
                contextKeyParams.opName,
                "numHeads / numKeyValueHeads = %d, cannot be larger than 64",
                nQ / nKV);
            return false;
        }
...
bool JittorInferFlashAttentionTiling::CheckNonEmptyShapeExceptions(
    ContextParamsForPFATiling& contextKeyParams,
    const gert::StorageShape* shape, const std::string& sName) {
    OPS_ERR_IF(shape == nullptr,
               OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                           "%s shape is null.", sName.c_str()),
               return true);
    OPS_ERR_IF(shape->GetStorageShape().GetShapeSize() ==
                   gert::Shape::kInvalidDimValue,
               OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                           "Shape size of %s is overflow.",
                                           sName.c_str()),
               return true);
    return false;
}

bool JittorInferFlashAttentionTiling::CheckActualSeqLength(
    ContextParamsForPFATiling& contextKeyParams, uint32_t b, uint32_t sQ,
    uint32_t sKV, const gert::Tensor* actualSeqLenQ,
    const gert::Tensor* actualSeqLenKV, InputLayout inLayout,
    JittorInferFlashAttentionTilingData& tilingData) {
    uint64_t actualLenDimsQ =
        (actualSeqLenQ != nullptr) ? actualSeqLenQ->GetShapeSize() : 0;
    uint64_t actualLenDimsKV =
        (actualSeqLenKV != nullptr) ? actualSeqLenKV->GetShapeSize() : 0;
    bool inputActualSeqQ =
        !((actualLenDimsQ == 0) || (actualSeqLenQ == nullptr) ||
          (actualSeqLenQ->GetData<int64_t>() == nullptr));
    bool inputActualSeqKV =
        !((actualLenDimsKV == 0) || (actualSeqLenKV == nullptr) ||
    ...
    constexpr uint64_t actualLenDimsKVMin =
        1;  // The length of actualSeqKV is 1
    ...
    if (inLayout == InputLayout::SH) {
        if (inputActualSeqQ) {
            for (uint32_t i = LOOP_BEGIN_NUM; i < b; ++i) {
                actualSeqQSum =
                    actualSeqQSum +
                    static_cast<uint32_t>(actualSeqLenQ->GetData<int64_t>()[i]);
            }
            OPS_ERR_IF(
                actualSeqQSum != sQ,
                OPS_REPORT_VECTOR_INNER_ERR(
    ...
    tilingData.promptAttentionBaseParams.set_actualSeqLengthsSize(
        actualLenDimsQ);
    tilingData.promptAttentionBaseParams.set_actualSeqLengthsKVSize(
        actualLenDimsKV);

    if (inputActualSeqQ) {  // check the length of actual_seq_lengthsQ, whether
                            // is 1 or batch size
        OPS_ERR_IF(actualLenDimsQ < b && actualLenDimsQ > actualLenDimsQMin,
                   OPS_REPORT_VECTOR_INNER_ERR(
                       contextKeyParams.opName,
                       "Dim(%lu) of actual_seq_lengths must equal to 1 or "
                       "greater than or equal to batch size(%u)!",
                       actualLenDimsQ, b),
                   return false);
        uint32_t actualSeqQLength =
            std::min(static_cast<uint32_t>(actualLenDimsQ),
                     b);  // actual_seq_lengths is 1 or batch size
        for (uint32_t i = LOOP_BEGIN_NUM; i < actualSeqQLength; ++i) {
            actualSeqTmp =
                static_cast<int64_t>(actualSeqLenQ->GetData<int64_t>()[i]);
            OPS_ERR_IF(
                actualSeqTmp < 0 || actualSeqTmp > sQ,
                OPS_REPORT_VECTOR_INNER_ERR(
                    contextKeyParams.opName,
    ...
    if (inputActualSeqKV) {  // check the length of actual_seq_lengthsKV,whether
                             // is 1 or batch size
        OPS_ERR_IF(actualLenDimsKV < b && actualLenDimsKV > actualLenDimsKVMin,
                   OPS_REPORT_VECTOR_INNER_ERR(
                       contextKeyParams.opName,
                       "Dim(%lu) of actual_seq_lengths_kv must equal to 1 or "
                       "greater than or equal to batch size(%u)!",
                       actualLenDimsKV, b),
                   return false);
        uint32_t actualSeqKVLength =
            std::min(static_cast<uint32_t>(actualLenDimsKV),
                     b);  // actual_seq_lengths_KV is 1 or batch size
        for (uint32_t i = LOOP_BEGIN_NUM; i < actualSeqKVLength; ++i) {
            actualSeqTmp =
                static_cast<int64_t>(actualSeqLenKV->GetData<int64_t>()[i]);
            if (!enablePA) {
                OPS_ERR_IF(
                    actualSeqTmp < 0 || actualSeqTmp > sKV,
                    OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                                "Actual_seq_lengths_kv[%u](%ld)"
                ...
                OPS_ERR_IF(actualSeqTmp < 0,
                           OPS_REPORT_VECTOR_INNER_ERR(
                               contextKeyParams.opName,
                               "Actual_seq_lengths_kv[%u](%ld) must >= 0", i,
                               actualSeqTmp),
                           return false);
            }
        }
...
bool JittorInferFlashAttentionTiling::CheckPseShiftTypeAndShape(
    ContextParamsForPFATiling& contextKeyParams,
    const gert::StorageShape* pseShiftShape, uint32_t b, uint32_t n,
    uint32_t s1, uint32_t s2) {
    pseShiftElemType = contextKeyParams.pseShiftDataType;

    OPS_ERR_IF(
        (inputType == ge::DT_FLOAT16 && pseShiftElemType != ge::DT_FLOAT16),
        OPS_REPORT_VECTOR_INNER_ERR(
            contextKeyParams.opName,
            "q type is fp16, but pse shift type is not fp16, pse shift type = "
            "%s",
            g_strDataTypePfa.at(ValidPfaDataType(pseShiftElemType)).c_str()),
        return false);

    OPS_ERR_IF(
        (inputType == ge::DT_BF16 && pseShiftElemType != ge::DT_BF16),
        OPS_REPORT_VECTOR_INNER_ERR(
            contextKeyParams.opName,
            "q type is bf16, but pse shift type is not bf16, pse shift type = "
            "%s",
            g_strDataTypePfa.at(ValidPfaDataType(pseShiftElemType)).c_str()),
        return false);

    OPS_ERR_IF(
        (inputType == ge::DT_INT8 && pseShiftElemType != ge::DT_FLOAT16),
        OPS_REPORT_VECTOR_INNER_ERR(
            contextKeyParams.opName,
            "q type is int8, but pse shift type is not fp16, pse shift type = "
            "%s",
            g_strDataTypePfa.at(ValidPfaDataType(pseShiftElemType)).c_str()),
        return false);
    ...
    if (((pseShiftShape != nullptr) &&
         (pseShiftShape->GetStorageShape().GetShapeSize() == 0)) ||
    ...
    OPS_ERR_IF(
        ...
        OPS_REPORT_VECTOR_INNER_ERR(
            contextKeyParams.opName,
            "pse shift shape must be 4 dimension, rather than %u dimension",
            pseShiftDim),
        return false);

    pseShiftBatch = pseShiftShape->GetStorageShape().GetDim(0);
    uint32_t pseShiftN = pseShiftShape->GetStorageShape().GetDim(
        1);  // 1: The sirst dimension is N.
    pseShiftS1 = pseShiftShape->GetStorageShape().GetDim(
        2);  // 2: The second dimension is S1.
    pseShiftS2 = pseShiftShape->GetStorageShape().GetDim(
        3);  // 3: The third dimension is S2.
    OPS_ERR_IF(
        ((pseShiftBatch != 1 && pseShiftBatch != b) || (pseShiftN != n) ||
         (pseShiftS1 < s1) || (pseShiftS2 < s2)),
        OPS_REPORT_VECTOR_INNER_ERR(
            contextKeyParams.opName,
            "pse shift shape must be [1 or %u, %u, >=%u, >=%u], but now it is "
            "[%u, %u, %u, %u]",
...
bool JittorInferFlashAttentionTiling::CheckPATypeAndShape(
    ContextParamsForPFATiling& contextKeyParams,
    const gert::Tensor* actualSeqLenKV, int32_t b, int32_t n, int32_t h,
    int32_t headNumRatio) {
    const int32_t* blockSize = contextKeyParams.blockSize;
    OPS_ERR_IF((*blockSize % BLOCK_SIZE_BASE != 0 ||
                *blockSize < BLOCK_SIZE_BASE || *blockSize > BLOCK_SIZE_MAX),
               OPS_REPORT_VECTOR_INNER_ERR(
                   contextKeyParams.opName,
                   "block size(%d) should be a multiple of %d, and can't "
                   "greater than %d when PA enable",
                   *blockSize, BLOCK_SIZE_BASE, BLOCK_SIZE_MAX),
               return false);

    const gert::StorageShape* blockTableShape =
        contextKeyParams.blockTableShape;
    OPS_ERR_IF(
        (((blockTableShape != nullptr) &&
          (blockTableShape->GetStorageShape().GetShapeSize() == 0)) ||
         (blockTableShape == nullptr)),
        OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                    "blockTable can't be empty when PA enable"),
        return false);
    int32_t blockTableDim1 =
        static_cast<int32_t>(blockTableShape->GetStorageShape().GetDim(0));
    blockTableDim2 =
        static_cast<int32_t>(blockTableShape->GetStorageShape().GetDim(1));
    ...
    OPS_ERR_IF(
        keyDim != valueDim,
        OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                    "the dim num of key(%d) and value(%d) are "
                                    "inconsistent when PA enable",
                                    keyDim, valueDim),
        return false);
    ...
    if (keyDim == 4) {  // dim num: 4
        keyDim4 =
            keyShape->GetStorageShape().GetDim(3);  // 3: The third dimension.
        valueDim4 =
            valueShape->GetStorageShape().GetDim(3);  // 3: The third dimension.
        tempN = keyDim2;
        tempBlockSize = keyDim3;
        tempD = keyDim4;
    }

    OPS_ERR_IF(((keyDim1 != valueDim1) || (keyDim2 != valueDim2) ||
                (keyDim3 != valueDim3) || (keyDim4 != valueDim4)),
               OPS_REPORT_VECTOR_INNER_ERR(
                   contextKeyParams.opName,
    ...
    if (keyDim == 3) {     // dim num: 3
        ...
        OPS_ERR_IF(
            ((tempBlockSize != *blockSize) || (tempH * headNumRatio != h)),
            OPS_REPORT_VECTOR_INNER_ERR(
                contextKeyParams.opName,
                "the dim of key [%d, %d, %d] is wrong, which should be [>=%ld, "
                "%d, %d] when PA enable",
                keyDim1, keyDim2, keyDim3, blockNumValid, *blockSize,
                h / headNumRatio),  // When assigning headNumRatio, it is
                                    // guaranteed that it will not be 0
            return false);
        // In the BSH input of the PA scenario, it is required that the h of the
        // KV matrix does not exceed 65535.  The dim and dim3 of the K/V have
        // already been verified to be equal, so only the K matrix is verified
        // here.
        OPS_ERR_IF(keyDim3 > 65535,
                   OPS_REPORT_VECTOR_INNER_ERR(
                       contextKeyParams.opName,
                       "layout of key/value is BSH, the h of key/value %d "
                       "should not > 65535 when PA enable",
                       keyDim3),
                   return false);
    } else {
        PAlayoutType = 0;  // If it is four-dimensional, PAlayoutType = 0
        OPS_ERR_IF(((tempN * headNumRatio != n) ||
                    (tempBlockSize != *blockSize) || (tempD != (h / n))),
                   OPS_REPORT_VECTOR_INNER_ERR(
                       contextKeyParams.opName,
                       "the dim of key [%d, %d, %d, %d] is wrong, which should "
                       "be [>=%ld, %d, %d, %d] when PA enable",
                       keyDim1, keyDim2, keyDim3, keyDim4, blockNumValid,
                       n / headNumRatio, *blockSize, (h / n)),
                   return false);
    }
    ...
    if (layoutStr == "BNSD" || layoutStr == "NSD") {
        OPS_ERR_IF(
            ...
            OPS_REPORT_VECTOR_INNER_ERR(
                contextKeyParams.opName,
                "the layout of query is %s, key and value layout should be "
                "[>=%ld, %d, %d] or [>=%ld, %d, %d, %d] when PA enable",
                layoutStr.c_str(), blockNumValid, *blockSize, h, blockNumValid,
                n, *blockSize, (h / n)),
            return false);
        ...
        OPS_ERR_IF(keyDim != 3,
                   OPS_REPORT_VECTOR_INNER_ERR(
                       contextKeyParams.opName,
                       "the layout of query is %s, key and value layout should "
                       "be [>=%ld, %d, %d] when PA enable",
                       layoutStr.c_str(), blockNumValid, *blockSize, h),
                   return false);
    } else {
        OPS_LOG_E(contextKeyParams.opName,
                  "unsupported input data layout when PA enable");
        return false;
    }
    ...
    int32_t blockTableDim =
        static_cast<int32_t>(blockTableShape->GetStorageShape().GetDimNum());
    ...
    OPS_ERR_IF(
        ((blockTableDim1 != b) || (blockTableDim2 < maxBlockNumPerBatch)),
        OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                    "block table shape should be [%d, >=%d], "
                                    "now is [%d, %d] when PA enable",
                                    b, maxBlockNumPerBatch, blockTableDim1,
                                    blockTableDim2),
        return false);
    ...
    OPS_ERR_IF(
        ...
        OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                    "the first dim of key(%d) should not less "
                                    "than valid block num(%ld) when PA enable",
                                    keyDim1, blockNumValid),
        return false);
...
bool JittorInferFlashAttentionTiling::CheckAttenMaskShape(
    ContextParamsForPFATiling& contextKeyParams, const int32_t* sparseMode,
    const gert::StorageShape* attenMaskShape, const uint32_t sQ,
    const uint32_t sK, const uint32_t batchSize) {
    // Attention mask empty Tensor scene, no need to verify attention mask shape
    // based on sparse mode value
    ...
    if (attenMaskDim == ATTENTION_MASK_DIM2) {
        ...
        if ((sparseMode == nullptr) ||
            (sparseMode != nullptr && *sparseMode == SPARSE_MODE_NO_MASK) ||
            (sparseMode != nullptr && *sparseMode == SPARSE_MODE_ALL_MASK)) {
            checkShapeRet =
                (attenMaskS1 >= sQ) && (attenMaskS2 >= sK) &&
        ...
        attenMaskS2 = attenMaskShape->GetStorageShape().GetDim(
            2);  // 2: When the dim is 3, the second dimension is S2.
        if ((sparseMode == nullptr) ||
            (sparseMode != nullptr && *sparseMode == SPARSE_MODE_NO_MASK) ||
            (sparseMode != nullptr && *sparseMode == SPARSE_MODE_ALL_MASK)) {
            checkShapeRet =
        ...
        attenMaskS1 = attenMaskShape->GetStorageShape().GetDim(
            2);  // 2: When the dim is 4, the second dimension is S1.
        attenMaskS2 = attenMaskShape->GetStorageShape().GetDim(
            3);  // 3: When the dim is 4, the third dimension is S2.
        if ((sparseMode == nullptr) ||
            (sparseMode != nullptr && *sparseMode == SPARSE_MODE_NO_MASK) ||
            (sparseMode != nullptr && *sparseMode == SPARSE_MODE_ALL_MASK)) {
            checkShapeRet =
        ...
        OPS_LOG_E(contextKeyParams.opName,
                  "attenMask dim(%u) must be 2 or 3 or 4!", attenMaskDim);
        return false;
    }
    if ((sparseMode == nullptr) ||
        ((sparseMode != nullptr) && (*sparseMode == SPARSE_MODE_NO_MASK)) ||
        ...
        OPS_ERR_IF(
            ...
            OPS_REPORT_VECTOR_INNER_ERR(
                contextKeyParams.opName,
                "attenMask batch(%u) must be 1 or %u, attenMask Q_S(%u) must "
                "be larger than sQ(%u), attenMask KV_S(%u) must be larger than "
                "sK(%u), please check",
    ...
    if ((sparseMode != nullptr) && ((*sparseMode == SPARSE_MODE_LEFT_UP) ||
        ...
        OPS_ERR_IF(checkShapeRet != 1,
                   OPS_REPORT_VECTOR_INNER_ERR(
                       contextKeyParams.opName,
                       "attenMask shape must be (2048, 2048) or (1, 2048, "
                       "2048) or (1, 1, 2048, 2048) when sparse mode = %d",
                       *sparseMode),
                   return false);
    }
    return true;
...
ge::graphStatus JittorInferFlashAttentionTiling::AdjustBasicBlock(
    JittorInferFlashAttentionTilingData& tilingData, uint32_t& sOuterFactor) {
    PromptAttentionBaseParams* baseParams =
        &tilingData.promptAttentionBaseParams;
    uint32_t headNumSize = baseParams->get_headNumSize();
    ...
    if ((coreNum % headNumSize == 0) && (sCoreNum > 1) &&
        (sOuterBlockNum % sCoreNum == 0) && sOuterBlockNum / sCoreNum == 1) {
        // Open all core in the n direction; Multiple cores are opened in the s
        // direction and each core only processes one Souter. At this point, the
        // Souter is divided into two blocks for load balancing optimization. To
        // ensure that the basic block is an integer multiple of typeByteNum.
        sOuterFactor = (sOuterFactor / 2 + typeByteNum - 1) / typeByteNum *
                       typeByteNum;  // split outer: 2
    }
    return ge::GRAPH_SUCCESS;
}
...
ge::graphStatus JittorInferFlashAttentionTiling::GetAndCheckEmptyQueryShape(
    ContextParamsForPFATiling& contextKeyParams,
    ...
    OPS_ERR_IF(b > BLIMIT,
               OPS_REPORT_VECTOR_INNER_ERR(
                   contextKeyParams.opName,
                   "batch size should <= 65536, but batch size = %u", b),
               return ge::GRAPH_FAILED);
    if (s > SLIMIT) {
        OPS_LOG_W(contextKeyParams.opName, "seq should <= 20m, but seq = %u",
                  s);
    }
    if (inputLayout == InputLayout::BSH || inputLayout == InputLayout::SH) {
        OPS_ERR_IF(
            h > DLIMIT * NLIMIT,
            OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                        "h should <= 512 * 256, but h = %u", h),
            return ge::GRAPH_FAILED);
    } else {
        OPS_ERR_IF(
            n > NLIMIT,
            OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                        "n should <= 256, but n = %u", n),
            return ge::GRAPH_FAILED);
    }
...
uint32_t JittorInferFlashAttentionTiling::CalcTschBlockDim(
    uint32_t sliceNum, uint32_t aicCoreNum, uint32_t aivCoreNum) {
    uint32_t ration;
...
ge::graphStatus JittorInferFlashAttentionTiling::RunBigKernelTilingWithParams(
    ContextParamsForPFATiling& contextKeyParams, uint64_t& tilingKey,
    ...
    uint64_t ubSize;
    contextKeyParamsPtr =
        &contextKeyParams;  // In subsequent rectification, contextKeyParams
                            // will be written as a member variable of the
                            // class.
    ...
    OPS_LOG_I(contextKeyParams.opName,
              "ascendPlatformInfo:aivNum = %u, aicNum = %u, l1Size = %lu, "
              "l0CSize = %lu, l0ASize = %lu, l0BSize = %lu, ubSize = %lu!",
              aivNum, aicNum, ascendPlatformInfo.l1Size,
              ascendPlatformInfo.l0CSize, ascendPlatformInfo.l0ASize,
              ascendPlatformInfo.l0BSize, ascendPlatformInfo.ubSize);

    int32_t outputDataTypeSize = FLOAT32SIZE;
    if (CheckIOType(contextKeyParams, tilingData, outputDataTypeSize) !=
        ge::GRAPH_SUCCESS) {
        return ge::GRAPH_FAILED;
    }
    ...
    OPS_ERR_IF(((inputType == ge::DT_FLOAT) || (outputType == ge::DT_FLOAT)),
               OPS_REPORT_VECTOR_INNER_ERR(
                   contextKeyParams.opName,
                   "inputType(%d) and outputType(%d) can not be DT_FLOAT",
    ...
    innerPrecise =
        innerPrecisePtr
            ? *innerPrecisePtr
            : HIGH_PERFORMANCE;  // 910B defaults to high-performance, while
                                 // 310P's high performance refers to high
                                 // accuracy (without using approximate
                                 // calculations).

    if (innerPrecise >=
        4) {  // 0: Invalid plural number; 4: Invalid if greater than or equal
              // to 4; 0,1,2,3 are effective values for innerPrecise.
        OPS_LOG_W(contextKeyParams.opName,
                  "innerPrecise [%lu] should be 0,1,2,3, please check.",
                  innerPrecise);
    }
    ...
    // Determine the bit0 bit of innerPrecise, high-performance or
    // high-precision mode.
    innerPrecise =
        ((innerPrecise >> 0) & 1) ? HIGH_PERFORMANCE : HIGH_PRECISION;
    OPS_ERR_IF(
        ((innerPrecise != HIGH_PERFORMANCE) &&
         (innerPrecise != HIGH_PRECISION)),
        OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                    "precision mode[%lu] should be 0 or 1",
                                    innerPrecise),
        return ge::GRAPH_FAILED);  // Currently only supports high-precision 0
                                   // and high-performance 1
    if (inputType != ge::DT_FLOAT16) {
        OPS_LOG_W(contextKeyParams.opName,
                  "innerPrecise will not take effect when input type is %d!",
                  inputType);
    }
    ...
    if ((contextKeyParams.pseShift != nullptr) &&
        (inputType == ge::DT_FLOAT16) && (innerPrecise == HIGH_PERFORMANCE)) {
        innerPrecise = HIGH_PRECISION;
        OPS_LOG_W(contextKeyParams.opName,
                  "when the input is fp16, the mode is forcibly switched to "
                  "high-precision!");
    }
    ...
    if (CheckMaskType(contextKeyParams, tilingData, maskElemSize) !=
        ge::GRAPH_SUCCESS) {
        return ge::GRAPH_FAILED;
    }
    ...
    tilingData.promptAttentionBaseParams.set_softmaxTypeByteNum(
        softmaxTypeByteNum);
    tilingData.promptAttentionBaseParams.set_outputTypeByteNum(
        outputTypeByteNum);
    ...
    const gert::StorageShape* SoftmaxLseOutShape =
        contextKeyParams.lseoutputShape;

    uint32_t deqScaleTypeFlag =
        (contextKeyParams.deqScaleType == DT_UINT64) ? 0U : 1U;
    uint32_t deqScale2TypeFlag =
        (contextKeyParams.deqScale2Type == DT_UINT64) ? 0U : 1U;
    ...
    OPS_ERR_IF(((contextKeyParams.inputDataType == ge::DT_INT8) &&
                ...
                ((contextKeyParams.scale2Shape != nullptr) ||
                 (contextKeyParams.offset2Shape != nullptr))),
               OPS_REPORT_VECTOR_INNER_ERR(
                   contextKeyParams.opName,
                   "When query dtype is int8 and output dtype is fp16, "
                   "quantScale2 and quantOffset2 should be null."),
               return ge::GRAPH_FAILED);
    ...
    if (CheckShape(contextKeyParams, queryShape, keyShape, valueShape, outShape,
                   pseShiftShape, attenMaskShape) != ge::GRAPH_SUCCESS) {
        return ge::GRAPH_FAILED;
    }

    // In the scene of entering the image, there may be a situation where out is
    // an empty tensor. Here, out is empty and size 0 is processed, which is
    // equivalent to doing nothing and returning directly.
    if ((keyShape->GetStorageShape().GetShapeSize() == 0) ||
        (valueShape->GetStorageShape().GetShapeSize() == 0) ||
        (outShape->GetStorageShape().GetShapeSize() == 0)) {
        tilingKey = EMPTY_KV_TILING_KEY;
        OPS_ERR_IF(
            GetAndCheckEmptyQueryShape(contextKeyParams, queryShape) ==
                ge::GRAPH_FAILED,
            OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                        "GetAndCheckEmptyQueryShape failed."),
            return ge::GRAPH_FAILED);
        OPS_ERR_IF(
            (contextKeyParams.inputDataType != ge::DT_FLOAT16) ||
            ...
            OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                        "when input or output is empty tensor, "
                                        "input datatype should be float16."),
            return ge::GRAPH_FAILED);
        JittorInferFlashAttentionInitOutputSplit(
            outShape->GetStorageShape().GetShapeSize(), tilingData, coreNum);
        ...
        uint32_t actualCore =
            (singleCoreSize > 0) ? (outShape->GetStorageShape().GetShapeSize() +
                                    singleCoreSize - 1) /
                                       singleCoreSize
                                 : coreNum;
        blockDimToBeSet =
            ascendcPlatform.CalcTschBlockDim(actualCore, aicNum, coreNum);

        size_t* workspace = contextKeyParams.workspaceSize;
        const size_t sysWorkspaceSize =
            16 * 1024 * 1024;  // workspace needs at least this much
    ...
    if (inputType == ge::DT_INT8) {
        OPS_ERR_IF(
            (deqScale1Shape == nullptr) || (quantScale1Shape == nullptr) ||
                (deqScale2Shape == nullptr),
            OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                        "dequant scale or first quant scale is "
                                        "nullptr when input type is int8."),
            return ge::GRAPH_FAILED);
        OPS_ERR_IF(
            (deqScale1Shape != nullptr &&
             deqScale1Shape->GetStorageShape().GetShapeSize() == 0) ||
                (quantScale1Shape != nullptr &&
                 quantScale1Shape->GetStorageShape().GetShapeSize() == 0) ||
                (deqScale2Shape != nullptr &&
                 deqScale2Shape->GetStorageShape().GetShapeSize() == 0),
            OPS_REPORT_VECTOR_INNER_ERR(
                contextKeyParams.opName,
                "dequant scale or first quant scale is empty tensor when input "
                "type is int8."),
            return ge::GRAPH_FAILED);
    }
    ...
    OPS_ERR_IF(
        CheckKeyValueParamsConsistency(contextKeyParams) != ge::GRAPH_SUCCESS,
        OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                    "key value consistency check failed!"),
        return ge::GRAPH_FAILED);

    const int32_t* numKeyValueHeads = contextKeyParams.numKeyValueHeads;
    if (!SetTilingHeadNumRatio(contextKeyParams, n, numKeyValueHeads,
                               tilingData)) {
        return ge::GRAPH_FAILED;
    }
    ...
    size_t actualLenDimsKV =
        (tempDataKV != nullptr) ? tempDataKV->GetShapeSize() : 0;
    uint32_t isActualSeqLengthsNull =
        (actualLenDims == 0 || tempData == nullptr ||
         tempData->GetData<int64_t>() == nullptr);
    uint32_t isActualSeqLengthsKVNull =
        (actualLenDimsKV == 0 || tempDataKV == nullptr ||
         tempDataKV->GetData<int64_t>() == nullptr);
    OPS_ERR_IF(
        enablePA && (isActualSeqLengthsKVNull == 1),
        OPS_REPORT_VECTOR_INNER_ERR(
            contextKeyParams.opName,
    ...
    if (inputLayout == (InputLayout::SH) && (actualLenDimsKV != 0)) {
        OPS_LOG_W(contextKeyParams.opName,
                  "actual_seq_lengths_kv is useless for SH format!");
    }
    ...
    if (inputLayout == InputLayout::BNSD || inputLayout == InputLayout::NSD) {
        if (queryShape->GetStorageShape().GetDimNum() == 3) {  // dim num: 3
            ...
            h = queryShape->GetStorageShape().GetDim(1) *
                queryShape->GetStorageShape().GetDim(3);  // dim num: 3
        ...
        if (queryShape->GetStorageShape().GetDimNum() == NUM_2) {  // dim num: 2
            b = actualLenDims == 0
                    ? 1
                    : actualLenDims;  // When the input layout is SH and
                                      // actual_seq is not input, the batch of
                                      // query is set to 1.
            bKV = actualLenDimsKV == 0
                      ? 1
                      : actualLenDimsKV;  // When the input layout is SH and
                                          // actual_seqkv is not input, the
                                          // batch of key/value is set to 1.
            s = queryShape->GetStorageShape().GetDim(0);
            h = queryShape->GetStorageShape().GetDim(1);
    ...
    if (((inputLayout == InputLayout::BSH) ||
        ...
        OPS_LOG_W(contextKeyParams.opName,
                  "h(%u) is larger than 65535, which may cause precision "
                  "problem! Please use BNSD instead.",
                  h);
    }
    ...
    OPS_ERR_IF((b != bKV) && (!enablePA),
               OPS_REPORT_VECTOR_INNER_ERR(
                   contextKeyParams.opName,
                   "query batch must be equal to key/value batch, query batch "
                   "= %u , key/value batch = %u .",
                   b, bKV),
               return ge::GRAPH_FAILED);
    OPS_ERR_IF((b > BLIMIT),
               ...
               return ge::GRAPH_FAILED);
    OPS_ERR_IF(
        (b > 128 && (inputLayout == InputLayout::SH)),
        OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                    "batch size(%u) should not be larger than "
                                    "128 when input layout is SH!",
                                    b),
        return ge::GRAPH_FAILED);

    bool iskvdiff = (seqInnerSize != s);
    OPS_ERR_IF(
        (iskvdiff) && (inputLayout == InputLayout::SH) && (!enablePA),
        OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                    "SH format not support q kv diff, length "
                                    "of q = %u , length of kv = %u.",
                                    s, seqInnerSize),
        return ge::GRAPH_FAILED);

    // Dims and length of actSeqLenQ & actSeqLenKV check.
    if (!CheckActualSeqLength(contextKeyParams, b, s, seqInnerSize, tempData,
                              tempDataKV, inputLayout, tilingData)) {
        return ge::GRAPH_FAILED;
    }
    // When verifying the shape of the mask in PA scenarios, using
    // maxBlockNumPerBatch * tempBlockSize as tmpS2.
    if (enablePA) {
        if (!CheckPATypeAndShape(contextKeyParams, tempDataKV, (int32_t)b,
                                 (int32_t)(*n), (int32_t)h,
                                 (int32_t)tilingData.promptAttentionBaseParams
                                     .get_headNumRatio())) {
            return ge::GRAPH_FAILED;
        }
    } else {
    ...
    if (!CheckAttenMaskShape(contextKeyParams, sparseMode, attenMaskShape, s,
                             tmpS2, b)) {
        return ge::GRAPH_FAILED;
    }
    ...
    if (contextKeyParams.pseShift != nullptr) {
        usePseShift = 1;
        if (!CheckPseShiftTypeAndShape(contextKeyParams, pseShiftShape, b, *n,
                                       s, tmpS2)) {
            return ge::GRAPH_FAILED;
        }
    } else {
    ...
    int32_t sparseRet = 0;
    if (sparseMode != nullptr) {
        sparseRet = (*sparseMode != SPARSE_MODE_NO_MASK &&
        ...
        if (((attenMaskShape != nullptr) &&
             (attenMaskShape->GetStorageShape().GetShapeSize() == 0)) ||
            (attenMaskShape == nullptr)) {
            tilingData.promptAttentionBaseParams.set_useMask(
                0);  // for sparse check rule 5
        }
    }
        ...
        if (sparseMode != nullptr) {
            if (*sparseMode == SPARSE_MODE_LEFT_UP) {
                ...
                sparseModeVal = *sparseMode;
                       ...
                       SPARSE_MODE_RIGHT_DOWN) {  // Right down tokens are
                                                  // calculated on the kernel
                                                  // side.
                sparsePreTokens = SPARSE_MODE_INT_MAX;
                sparseModeVal = *sparseMode;
                ...
                OPS_ERR_IF(
                    *preTokens < 0 && *nextTokens < 0,
                    OPS_REPORT_VECTOR_INNER_ERR(
                        contextKeyParams.opName,
                        "preTokens and nextTokens must not be negative number "
                        "in band mode, preTokens = %ld , nextTokens = %ld .",
    ...
    if ((sparseMode != nullptr) && (*sparseMode == SPARSE_MODE_LEFT_UP ||
        ...
        OPS_ERR_IF((sparseRet == 1),
                   OPS_REPORT_VECTOR_INNER_ERR(
                       contextKeyParams.opName,
                       "attenMask should not be null when sparse_mode is %d.",
                       *sparseMode),
                   return ge::GRAPH_FAILED);
        ...
        OPS_ERR_IF(
            ...
            OPS_REPORT_VECTOR_INNER_ERR(
                contextKeyParams.opName,
                "invalid maskType dtype[%s], maskType should be bool, int8 or "
                "uint8 when sparse mode is %d.",
                g_strDataTypePfa.at(ValidPfaDataType(maskDataType)).c_str(),
                *sparseMode),
            return ge::GRAPH_FAILED);
    }
    if ((sparseMode != nullptr) && (*sparseMode == SPARSE_MODE_NO_MASK)) {
        // sparse mode, We need to apply the same processing to two scenarios
        // where the attention mask is empty tensor
        if (((attenMaskShape != nullptr) &&
             (attenMaskShape->GetStorageShape().GetShapeSize() == 0)) ||
            (attenMaskShape == nullptr)) {
            sparsePreTokens = SPARSE_MODE_INT_MAX;
    ...
    if (isDefaultMode) {
        // For scenes with sparse mode=0 and left padding, the attention mask
        // part is fully calculated
        sparsePreTokens = SPARSE_MODE_INT_MAX;
        sparseNextTokens = SPARSE_MODE_INT_MAX;
    }
    ...
    OPS_ERR_IF(isDefaultMode && (sparseNextTokens < 0) &&
                   (sparseNextTokens * (-1)) >= (int32_t)s,
               OPS_REPORT_VECTOR_INNER_ERR(
                   contextKeyParams.opName,
                   "nextTokens absolute value should be smaller than length of "
                   "q, nextTokens = %ld, length of q = %u.",
                   sparseNextTokens, s),
               return ge::GRAPH_FAILED);

    OPS_ERR_IF(
        isDefaultMode && (sparsePreTokens < 0) &&
            (sparsePreTokens * (-1) >= ((int32_t)tmpS2)),
        OPS_REPORT_VECTOR_INNER_ERR(
            contextKeyParams.opName,
            "preToken absolute value should be smaller than length of k and v "
            "(length of k and v + length of prefix when enable prefix), "
            "preTokens = %ld, seqLengthKV = %u",
            sparsePreTokens, tmpS2),
        return ge::GRAPH_FAILED);
    ...
    size_t lenDims = b;  // The current length of the actSeqLen array is equal
                         // to batch size b.
    ...
    bool checkQuantValue =
        (outputType == ge::DT_INT8) && (quantOffset2Shape != nullptr) &&
        (quantOffset2Shape->GetStorageShape().GetShapeSize() != 0);

    OPS_ERR_IF((outputType == ge::DT_INT8 && isBandMode &&
                ((sparsePreTokens < 0) || sparseNextTokens < 0)),
               OPS_REPORT_VECTOR_INNER_ERR(
                   contextKeyParams.opName,
                   "When output type is int8, sparse mode = 4, preTokens (%ld) "
                   "or nextTokens (%ld) cannot be negative.",
                   sparsePreTokens, sparseNextTokens),
               return ge::GRAPH_FAILED);
    for (size_t i = LOOP_BEGIN_NUM; i < lenDims; i++) {
        if ((actualLenDims == 0) || (tempData == nullptr) ||
            (tempData->GetData<int64_t>() == nullptr)) {
            ...
            actualSeqLengths[i] =
                (actualLenDims > 1)
                    ? static_cast<uint32_t>(tempData->GetData<int64_t>()[i])
                    : static_cast<uint32_t>(tempData->GetData<int64_t>()[0]);
            if (actualSeqLengths[i] != s) {
                needInit = 1;
                OPS_ERR_IF(
                    isDefaultMode && sparseNextTokens < 0 &&
                        sparseNextTokens * (-1) >= (int32_t)actualSeqLengths[i],
                    OPS_REPORT_VECTOR_INNER_ERR(
                        contextKeyParams.opName,
                        "nexttoken absolute value should be smaller than "
                        "actual length of q, "
        ...
        if ((actualLenDimsKV == 0) || (tempDataKV == nullptr) ||
            (tempDataKV->GetData<int64_t>() ==
             nullptr)) {  // The user did not input act_seq_kv
        ...
        OPS_ERR_IF(isDefaultMode && sparsePreTokens < 0 &&
                   ...
                   OPS_REPORT_VECTOR_INNER_ERR(
                       contextKeyParams.opName,
                       "preToken absolute value should be smaller than actual "
                       "length of k and v "
                       "(actual length of k and v + length of prefix when "
                       "enable prefix), preToken = %ld, actual length of k and "
                       "v = %ld, actual prefix len = %u.",
        ...
        if (sparseModeVal == SPARSE_MODE_RIGHT_DOWN) {
            ...
            preTokensPerbatch =
                sparsePreTokens - actualSeqLengthsKV[i] + actualSeqLengths[i];
            nextTokensPerbatch =
                sparseNextTokens + actualSeqLengthsKV[i] - actualSeqLengths[i];
        } else {
        ...
        if ((nextTokensPerbatch < 0) ||
            (actualSeqLengths[i] >
             (actualSeqLengthsKV[i] + preTokensPerbatch))) {
            needInit = 1;
        }
        // If (preTokensPerbatch + actualSeqLengthsKV[i] - actualSeqLengths[i])
        // < 0 or nextTokensPerbatch < 0, the last few lines or the first few
        // lines of the QKt matrix are not computed.
        OPS_ERR_IF(
            (checkQuantValue &&
             ((preTokensPerbatch + actualSeqLengthsKV[i] - actualSeqLengths[i] <
               0) ||
              (nextTokensPerbatch < 0))),
            OPS_REPORT_VECTOR_INNER_ERR(
                contextKeyParams.opName,
                "When sparse mode = %d, output dtype is int8, quantOffset2 is "
                "not null or empty tensor, "
                "preTokens = %ld and nextTokens = %ld, some rows of the matrix "
                "do not participate in the calculation, "
                "the accuracy of the final result will be incorrect. Please "
                "see the documentation for more details.",
                sparseModeVal, *preTokens, *nextTokens),
            return ge::GRAPH_FAILED);
        OPS_LOG_I(
            contextKeyParams.opName,
            "preTokensPerbatch[%lu] is %ld, nextTokensPerbatch[%lu] is %ld", i,
            preTokensPerbatch, i, nextTokensPerbatch);
        if (!isBandMode && actualSeqLengths[i] > actualSeqLengthsKV[i] +
                                                     (int64_t)sparsePreTokens) {
            actualSeqLengths[i] =
                actualSeqLengthsKV[i] + (int64_t)sparsePreTokens;
        }

        OPS_ERR_IF((isBandMode && (*nextTokens < 0) &&
                    (*nextTokens * (-1) >= actualSeqLengthsKV[i])),
                   OPS_REPORT_VECTOR_INNER_ERR(
                       contextKeyParams.opName,
                       "nextTokens absolute value should be smaller than "
                       "actual length of k and v in band mode (actual length "
                       "of k and v + length of "
                       "prefix when enable prefix), nextTokens = %ld, actual "
                       "length of k and v = %ld",
                       *nextTokens, actualSeqLengthsKV[i]),
                   return ge::GRAPH_FAILED);

        OPS_ERR_IF(
            (isBandMode && (*preTokens < 0) &&
             (*preTokens * (-1) >= actualSeqLengths[i])),
            OPS_REPORT_VECTOR_INNER_ERR(
                contextKeyParams.opName,
                "preTokens absolute value should be smaller than actual length "
                "of q in band mode, preTokens = %ld, actual length of q = %ld",
        ...
        OPS_LOG_I(contextKeyParams.opName,
                  "actualSeqLengths[%lu] is %ld, actualSeqLengthsKV[%lu] is "
                  "%ld, needInit is %u",
    ...
    const uint32_t precisionBlockEleCut =
        BYTE_BLOCK / FLOAT16SIZE;  // High-precision currently only supports
                                   // FP16, aligned at 32/2=16.
    OPS_ERR_IF(
        (hDivN > DLIMIT),
        OPS_REPORT_VECTOR_INNER_ERR(
            contextKeyParams.opName,
            ...
            hDivN),
        return ge::GRAPH_FAILED);  // Both high-precision and high-performance d
                                   // cannot exceed 512.
    ...
    OPS_ERR_IF(
        ...
        return ge::GRAPH_FAILED);  // d will be padded here and the original
                                   // value cannot be obtained, so it will not
                                   // be printed
    if ((inputType == ge::DT_FLOAT16) && (outputType == ge::DT_INT8)) {
        OPS_ERR_IF((inputLayout == InputLayout::SH),
                   OPS_REPORT_VECTOR_INNER_ERR(
                       contextKeyParams.opName,
                       "When input dtype is fp16 and output dtype is int8, SH "
    ...
    tilingData.promptAttentionBaseParams.set_sparseMode(
        static_cast<uint32_t>(sparseModeVal));
    tilingData.promptAttentionBaseParams.set_isLayoutSH(isLayoutSH);
    tilingData.promptAttentionBaseParams.set_isActualSeqLengthsNull(
        isActualSeqLengthsNull);
    tilingData.promptAttentionBaseParams.set_isActualSeqLengthsKVNull(
        isActualSeqLengthsKVNull);
    tilingData.promptAttentionSingleCoreParams.set_attenMaskBatch(
        attenMaskBatch);
    tilingData.promptAttentionInitOutputParams.set_needInit(needInit);

    uint32_t originHeadSize =
        tilingData.promptAttentionBaseParams.get_headSize();
    uint32_t blockElementCnt = BYTE_BLOCK / dataTypeSize;
    if (originHeadSize % blockElementCnt !=
        0) {  // Determine if D is aligned with 32B, using fp16 type with 16
              // elements.
        tilingData.promptAttentionBaseParams.set_alignedHeadSize(
            ((originHeadSize + blockElementCnt - 1) / blockElementCnt) *
            blockElementCnt);
        isDNoTail = false;
    } else {
        tilingData.promptAttentionBaseParams.set_alignedHeadSize(
    ...
    if (((inputType == ge::DT_FLOAT16) && (innerPrecise == HIGH_PRECISION)) ||
        (enablePA)) {
        useNewTiling =
            true;  // High-precision mode does not follow the old template.
    }
    ...
    OPS_LOG_I(contextKeyParams.opName,
              "Tiling Info: b is %u, bKV is %u, n is %d, numKeyValueHeads is "
              "%d, s1 is %u, s2 is %u, h is %u, d is %u, headNumRatio = %u",
    ...
    OPS_LOG_I(contextKeyParams.opName,
              ...
              static_cast<int>(inputLayout), innerPrecise, *scaleValue,
              *preTokens, *nextTokens);
    // Infering whether the tiling mode is D-axis split, S2 full load, CV diff,
    // and whether to use the matmul norm template.
    InferTilingMod(contextKeyParams, actualSeqLengths, actualSeqLengthsKV,
                   lenDims, hDivN, tmpS2, sparseModeVal);
    ...
    // Currently, there will be no D splitting scenario, and split D = 0 is
    // default when splitting.
    if (tilingMod == TilingMod::CVSAME) {
        OPS_ERR_IF(lenDims > 128,
                   OPS_REPORT_VECTOR_INNER_ERR(
                       contextKeyParams.opName,
                       "when D axis size(%u) is unaligend with 32 bytes, batch "
                       "size(%zu) can not larger than 128.",
                       hDivN, lenDims),
                   return ge::GRAPH_FAILED);
        auto ret = AdjustCVTiling(hDivN, *n, middleActualSeqLengths, ubSize,
                                  l1Size, l0CSize, maskElemSize, sOuterFactor,
        ...
        auto ret = AdjustCVTilingCVDiff(ubSize, l1Size, l0CSize, maskElemSize,
                                        sOuterFactor, sInnerFactor,
                                        softmaxSOuterFactor, tilingData);
        OPS_ERR_IF(ret != ge::GRAPH_SUCCESS,
                   OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                               "adjust tiling cv diff fail"),
                   return ret);
        softmaxSInnerFactor = sInnerFactor;
    }

    tilingData.promptAttentionSingleCoreParams.set_singleProcessSOuterSize(
        sOuterFactor);
    tilingData.promptAttentionSingleCoreParams.set_singleProcessSInnerSize(
        sInnerFactor);
    tilingData.promptAttentionBaseParams.set_splitS2(splitS2);
    tilingData.promptAttentionBaseParams.set_splitD(splitD);
    tilingData.promptAttentionBaseParams.set_softmaxOuterSize(
        softmaxSOuterFactor);
    tilingData.promptAttentionBaseParams.set_usePseShift(usePseShift);
    tilingData.promptAttentionBaseParams.set_pseShiftTypeByteNum(
        pseShiftTypeByteNum);
    ...
    tilingData.promptAttentionBaseParams.set_isSoftMaxLseEnable(
        contextKeyParams.isSoftMaxLseEnable);

    // Compute tiling data.
    if (splitCoreMode ==
        SplitCoreMode::SPLIT_ONEN_CUBE) {  // Enable N split kernel mode from
                                           // the perspective of cube in long
                                           // sequence scenes.
        ...
        if (useNewTiling) {
            JittorInferFlashAttentionSplitNSNew(
                contextKeyParams, tilingData, coreNum, actualSeqLengths,
                actualSeqLengthsKV, useBalanceTiling);
        } else {
            JittorInferFlashAttentionSplitNS(contextKeyParams, tilingData,
                                             coreNum, actualSeqLengths);
        }
    }
    ...
    ge::graphStatus tilingRet = TilingGetTilingKeyAttentionAscendC(
        tilingKey, contextKeyParams, useNewTiling, tilingData);
    ...
    tilingRet = JittorInferFlashAttentionApiTiling(
        tilingData, outputDataTypeSize, sOuterFactor, softmaxSInnerFactor,
        softmaxSOuterFactor);
    OPS_ERR_IF(tilingRet != ge::GRAPH_SUCCESS,
               OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                           "Get apiTiling fail"),
...
ge::graphStatus JittorInferFlashAttentionTiling::CheckIOType(
    ContextParamsForPFATiling& contextKeyParams,
    JittorInferFlashAttentionTilingData& tilingData,
...
ge::graphStatus JittorInferFlashAttentionTiling::CheckMaskType(
    ContextParamsForPFATiling& contextKeyParams,
    JittorInferFlashAttentionTilingData& tilingData, uint32_t& maskElemSize) {
    if (contextKeyParams.attentionMask != nullptr) {
        ...
        if (maskDataType == ge::DT_FLOAT16) {
            ...
            maskElemSize = BOOLSIZE;
                   ...
                   ge::DT_INT8) {  // Adapt to static graph mode, bool type
                                   // attentionmask is converted to int8.
        ...
        OPS_ERR_IF(
            ...
            return ge::GRAPH_FAILED);
        // When in fp16 high-precision mode, the mask type only supports bool or
        // int8.
        OPS_ERR_IF(
            ((inputType == ge::DT_FLOAT16) &&
             (innerPrecise == HIGH_PRECISION)) &&
                (maskDataType != ge::DT_BOOL) &&
                (maskDataType != ge::DT_INT8) && (maskDataType != ge::DT_UINT8),
            OPS_REPORT_VECTOR_INNER_ERR(
                contextKeyParams.opName,
                "invalid maskType dtype[%s], maskType should be bool, int8 or "
                "uint8 when fp16 high-precision mode",
                g_strDataTypePfa.at(ValidPfaDataType(maskDataType)).c_str()),
            return ge::GRAPH_FAILED);
        // When bf16, the mask type only supports bool or int8.
        OPS_ERR_IF(
            (inputType == ge::DT_BF16) && (maskDataType != ge::DT_BOOL) &&
                (maskDataType != ge::DT_INT8) && (maskDataType != ge::DT_UINT8),
            OPS_REPORT_VECTOR_INNER_ERR(
                contextKeyParams.opName,
                "invalid maskType dtype[%s], maskType should be bool, int8 or "
                "uint8 when input type is bfloat16",
                g_strDataTypePfa.at(ValidPfaDataType(maskDataType)).c_str()),
            return ge::GRAPH_FAILED);
        // FP16 mask type does not support invalid line correction.
        OPS_ERR_IF(
            (maskDataType == ge::DT_FLOAT16 &&
             tilingData.promptAttentionBaseParams.get_isRowInvalid()),
            OPS_REPORT_VECTOR_INNER_ERR(
                contextKeyParams.opName,
                "invalid maskType dtype[%s], maskType should not be float16 "
                "when innerPrecise = 2 or 3",
                g_strDataTypePfa.at(ValidPfaDataType(maskDataType)).c_str()),
            return ge::GRAPH_FAILED);
    }
    return ge::GRAPH_SUCCESS;
...
void JittorInferFlashAttentionTiling::SetMaskSize(
    const gert::StorageShape* attenMaskShape,
    JittorInferFlashAttentionTilingData& tilingData) {
    ...
    if (attenMaskShape != nullptr) {
        maskKVsSize = attenMaskShape->GetStorageShape().GetDim(
            attenMaskShape->GetStorageShape().GetDimNum() -
            1);  // 1: last frist dim
...
ge::graphStatus JittorInferFlashAttentionTiling::CheckShape(
    ContextParamsForPFATiling& contextKeyParams,
    const gert::StorageShape* queryShape, const gert::StorageShape* keyShape,
    const gert::StorageShape* valueShape, const gert::StorageShape* outShape,
    ...
    OPS_ERR_IF(
        (pseShiftShape != nullptr) &&
            (pseShiftShape->GetStorageShape().GetShapeSize() ==
             gert::Shape::kInvalidDimValue),
        OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
    ...
    OPS_ERR_IF(
        (attenMaskShape != nullptr) &&
            (attenMaskShape->GetStorageShape().GetShapeSize() ==
             gert::Shape::kInvalidDimValue),
        OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
    ...
    OPS_ERR_IF((outShape->GetStorageShape().GetShapeSize() != 0) &&
               ...
               OPS_REPORT_VECTOR_INNER_ERR(contextKeyParams.opName,
                                           "query is empty tensor."),
               return ge::GRAPH_FAILED);
    OPS_ERR_IF((queryShape->GetStorageShape().GetDimNum() < NUM_2) ||
                   (queryShape->GetStorageShape().GetDimNum() > 4),
               OPS_REPORT_VECTOR_INNER_ERR(
                   contextKeyParams.opName,
...
void JittorInferFlashAttentionTiling::InferTilingMod(
    const ContextParamsForPFATiling& contextKeyParams,
    const std::vector<int64_t>& actualSeqLengths,
    const std::vector<int64_t>& actualSeqLengthsKV, uint32_t actualSeqArrayLen,
    uint32_t hDivN, uint32_t seqInnerSize, int32_t sparseModeVal) {
    if (hDivN > DSPLIT_THRESHOLDS_512) {  // D segmentation threshold // S1S2D
                                          // splits into fp16 and int8 types
        splitD = 1;
    }
...
ge::graphStatus JittorInferFlashAttentionTiling::AdjustCVTiling(
    uint32_t hDivN, uint32_t n, int64_t middleActualSeqLengths, int64_t ubSize,
    int64_t l1Size, int64_t l0CSize, uint32_t maskElemSize,
    uint32_t& sOuterFactor, uint32_t& sInnerFactor,
    JittorInferFlashAttentionTilingData& tilingData) {
    // D is not split, S2 is fixed and cut into 128 sizes, S1 adjusts the size
    // for splitting
    ...
    if (GetRectangleFactor(seqFactorThreshold, rectangleQueue) !=
        ge::GRAPH_SUCCESS) {
        return ge::GRAPH_FAILED;
    }
    ...
    while (true) {
        ...
        if (JittorInferFlashAttentionCheckArgsLegal(
                tilingData, ubSize, l1Size, l0CSize, softmaxDataTypeSize,
                minFactor, rectangleFactor, updateDivRect, maskElemSize,
                dSplitFactor)) {
            break;
        }
        if (updateDivRect) {
...
ge::graphStatus
JittorInferFlashAttentionTiling::JittorInferFlashAttentionCVDiffSetTensorSize(
    JittorInferFlashAttentionTilingData& tilingData,
    PromptAttentionSingleCoreTensorSize& tensorSize, uint32_t sOuterFactor,
    ...
    if (tilingData.promptAttentionBaseParams.get_maskTypeByteNum() ==
        (BYTE_BLOCK / BOOLSIZE)) {
        tensorSize.set_selectSpaceUbSize(GetSelectWithBytesMaskMinTmpSize(
            Shape({softmaxSOuterFactor, sInnerFactor}), Shape({1}), 1,
...
bool JittorInferFlashAttentionTiling::
    JittorInferFlashAttentionComputeCVDiffParams(
        JittorInferFlashAttentionTilingData& tilingData, int64_t ubSize,
        int64_t l1Size, int64_t l0CSize, uint32_t typeByteSize,
    ...
    if (inputType == ge::DT_INT8) {
        res = FindOptimalTilingSouter(tilingData, sOuterFactor, sInnerFactor,
                                      softmaxSOuterFactor, ubSize, typeByteSize,
                                      maskTypeSize);
    } else {
        res = FindOptimalTilingBasicBLock(tilingData, sOuterFactor,
                                          sInnerFactor, softmaxSOuterFactor,
                                          ubSize, typeByteSize, maskTypeSize);
    }
    OPS_ERR_IF(
        res == false,
        OPS_REPORT_VECTOR_INNER_ERR(contextKeyParamsPtr->opName,
                                    "FindOptimalTilingBasicBLock failed!"),
        return false);
    ...
    OPS_ERR_IF(res == false,
               OPS_REPORT_VECTOR_INNER_ERR(
                   contextKeyParamsPtr->opName,
                   "JittorInferFlashAttentionCheckBmm1 failed!"),
               return false);

    res = JittorInferFlashAttentionCheckBmm2(
        tilingData, tilingData.bmm2TilingDataRect, l1SizeRemain, l0CSize,
...
bool JittorInferFlashAttentionTiling::FindOptimalTilingSouter(
    JittorInferFlashAttentionTilingData& tilingData, uint32_t& sOuterFactor,
    uint32_t& sInnerFactor, uint32_t& softmaxSOuterFactor, int64_t ubSize,
    uint32_t typeByteSize, uint32_t maskTypeSize) {
    // This function has a fixed Sinner of 1024 or kvs, reducing Souter to make
    // ub sufficient. Currently, only Int8 is using it
    auto tmpShape = Shape({softmaxSOuterFactor, sInnerFactor});
    int64_t softmaxTmpSize = 0;
    int64_t softmaxFlashTmpSize = 0;
    int64_t queueBufferSize = 0;

    // Temporary solution, first calculate using the Tmp variable of type
    // int32_t, and then optimize by changing the input parameter to type
    // int32_t
    ...
    auto srcShape =
        Shape({1, tilingData.promptAttentionBaseParams.get_alignedHeadSize()});
    auto scaleShape =
        Shape({1, tilingData.promptAttentionBaseParams.get_alignedHeadSize()});
    GetAscendAntiQuantMaxMinTmpSize(srcShape, scaleShape, false, ge::DT_INT8,
                                    inputType, kvAntiquantApiSizeMax,
                                    kvAntiquantApiSize);
    // Minimum antiquant ub: api + scale offset + input/output only processes
    // one line at a time
    int64_t minAntiquantUbSizeNeed =
        kvAntiquantApiSize +
        tilingData.promptAttentionBaseParams.get_alignedHeadSize() * 2 *
            FLOAT16SIZE +  // scale offset fp16
        tilingData.promptAttentionBaseParams.get_alignedHeadSize() *
            (INT8SIZE + FLOAT16SIZE);  // Input int8 and output fp16

    // lse extra ub size
    int64_t lseUbSize = contextKeyParamsPtr->isSoftMaxLseEnable
                            ? 256
                            : 0;  // only the first 2 elements are valid

    ubSizeRemain = 0;
    while (ubSizeRemain <= 0 && sOuterFactorTmp > 0) {
        softmaxTmpSize = 0;
        softmaxFlashTmpSize = 0;
        while ((softmaxTmpSize == 0 || softmaxFlashTmpSize == 0) &&
               (softmaxSOuterFactorTmp > 0)) {
            tmpShape = Shape({softmaxSOuterFactorTmp, sInnerFactorTmp});
            softmaxTmpSize = GetSoftMaxMinTmpSize(tmpShape, typeByteSize, true);
            softmaxFlashTmpSize = GetSoftMaxFlashV2MinTmpSize(
                tmpShape, typeByteSize, sizeof(float), true, true);
            if (softmaxTmpSize == 0 || softmaxFlashTmpSize == 0) {
        ...
        if (JittorInferFlashAttentionCVDiffSetTensorSize(
                tilingData, tilingData.promptAttentionTensorSizeRect,
        ...
        int64_t maskBmm2ShareSize = std::max(
            int64_t(queueBufferSize * pseMaskMaxSize),
            int64_t(
                tilingData.promptAttentionTensorSizeRect.get_bmm2ResUbSize() *
                typeByteSize));
        ubSizeRemain =
            ...
            (tilingData.promptAttentionTensorSizeRect.get_mmResUbSize() *
                 NUM_2 +  // 2:2 mm ub
             tilingData.promptAttentionTensorSizeRect
                 .get_bmm2ResUbSize() +  // bmm2ResPrev resident in UB
             SOFTMAX_BUFFER_NUM * tilingData.promptAttentionTensorSizeRect
                                      .get_softmaxExpSize()) *
                typeByteSize -
            maskBmm2ShareSize -
    ...
    OPS_ERR_IF(
        (sOuterFactorTmp <= 0) || (sInnerFactorTmp <= 0) ||
            (softmaxSOuterFactorTmp <= 0),
        OPS_REPORT_VECTOR_INNER_ERR(contextKeyParamsPtr->opName,
                                    "cannot find valid sOuterFactor, "
                                    "sInnerFactor and softmaxSOuterFactor!"),
        return false);
...
bool JittorInferFlashAttentionTiling::FindOptimalTilingBasicBLock(
    JittorInferFlashAttentionTilingData& tilingData, uint32_t& sOuterFactor,
    uint32_t& sInnerFactor, uint32_t& softmaxSOuterFactor, int64_t ubSize,
    uint32_t typeByteSize, uint32_t maskTypeSize) {
    auto tmpShape = Shape({softmaxSOuterFactor, sInnerFactor});
    ...
    auto postQuantSrcShape =
        Shape({sOuterFactor,
               tilingData.promptAttentionBaseParams.get_alignedHeadSize()});
    uint32_t bmm2ResTypeSize =
        (((inputType == ge::DT_FLOAT16) && (innerPrecise == HIGH_PRECISION)) ||
         (inputType == ge::DT_BF16))
            ? FLOAT32SIZE
            : FLOAT16SIZE;
    ...
    while (ubSizeRemain <= 0 && sOuterFactorTmp > 0) {
        while ((ubSizeRemain <= 0 && sInnerFactorTmp > 0) ||
               (ubSizeRemain < minAntiquantUbSizeNeed && sInnerFactorTmp > 0)) {
            softmaxTmpSize = 0;
            softmaxFlashTmpSize = 0;
            while ((softmaxTmpSize == 0 || softmaxFlashTmpSize == 0) &&
                   (softmaxSOuterFactorTmp > 0)) {
            ...
            if (outputType == ge::DT_INT8) {
                postQuantSrcShape =
                    Shape({sOuterFactorTmp, tilingData.promptAttentionBaseParams
                                                .get_alignedHeadSize()});
                GetAscendQuantMaxMinTmpSize(postQuantSrcShape, bmm2ResTypeSize,
                                            postQuantApiSizeMax,
                                            postQuantApiSizeMin);
            }
            ...
            int64_t maskBmm2ShareSize =
                std::max(int64_t(queueBufferSize * pseMaskMaxSize),
                         int64_t(tilingData.promptAttentionTensorSizeRect
                                     .get_bmm2ResUbSize() *
                                 typeByteSize));
            ubSizeRemain =
                ubSize - apiTmpSize -
                ...
                maskBmm2ShareSize -
                tilingData.promptAttentionTensorSizeRect
                    .get_selectSpaceUbSize() -
                pseShiftBufferSize * pseShiftCastSize - postQuantUbSize -
                postQuantApiSizeMin - lseUbSize;
            if (ubSizeRemain <= 0 || (ubSizeRemain < minAntiquantUbSizeNeed)) {
                sInnerFactorTmp -= sInnerFactorStep;
                softmaxSOuterFactorTmp = (int32_t)softmaxSOuterFactor;
            }
...
ge::graphStatus JittorInferFlashAttentionTiling::AdjustCVTilingCVDiff(
    int64_t ubSize, int64_t l1Size, int64_t l0CSize, uint32_t maskElemSize,
    uint32_t& sOuterFactor, uint32_t& sInnerFactor,
    uint32_t& softmaxSOuterFactor,
    JittorInferFlashAttentionTilingData& tilingData) {
    // New softmax tiling strategy, unified big tiling for mm1 mm2 (e.g.
    // mm1=256x512, mm2=256xhead_size), softmax calculates multiple long tiling
    // based on the UB space by horizontally cutting the big tiling into
    // multiple long tiling (e.g. softmax=32x512). Softmax calculates multiple
    // long tiling based on the UB space by horizontally slicing big tiling
    // (e.g. softmax=32x512).
    ...
    if ((tilingData.promptAttentionBaseParams.get_seqInnerSize() <=
         CVDIFF_SMALL_KV_THRESHOLDS) &&
        (inputType != ge::DT_INT8)) {
    ...
    if (((inputType == ge::DT_FLOAT16) && (innerPrecise == HIGH_PRECISION)) ||
        (inputType ==
         ge::DT_BF16)) {  // When high-precision mode or BF16 takes effect,
                          // adjust the starting tiling block.
        if (tilingData.promptAttentionBaseParams.get_alignedHeadSize() >=
            200) {  // D: [200, ...)
            minFactor =
                64;  // 64:  Adjust the size of the basic block Souter to 64.
            rectangleFactor =
                512;  // 512: Adjust the size of the basic block Sinner to 512.
            softmaxSOuterFactor = 8;  // 8:   Adjust softmaxSOuter to 8.
        } else if (tilingData.promptAttentionBaseParams.get_alignedHeadSize() >=
                   128) {  // D: [128, 200)
            minFactor =
                128;  // 128: Adjust the size of the basic block Souter to 128.
            rectangleFactor =
                512;  // 512: Adjust the size of the basic block Sinner to 512.
            softmaxSOuterFactor = 8;  // 8:   Adjust softmaxSOuter to 8
        } else if (tilingData.promptAttentionBaseParams.get_alignedHeadSize() >=
    ...
    if (enablePA) {
        minFactor = 64;  // In the PA scenario, Souter starts cutting from 64
                         // and tries to ensure that Sinner does not cut, so
                         // that Single is a multiple of blockSize
    }
    if (tilingData.promptAttentionBaseParams.get_seqSize() <=
        CVDIFF_SMALL_QS_THRESHOLDS) {  // Minimum basic block size.
        minFactor =
            CVDIFF_SMALL_QS_THRESHOLDS;  // Reduce S1 to avoid unnecessary
                                         // calculation of mm1
        if ((tilingData.promptAttentionBaseParams.get_seqInnerSize() >
             CVDIFF_SINNER_FACTOR_SMALL_QS) &&
            (tilingData.promptAttentionBaseParams.get_useMask() ==
             0)) {  // Only in scenes without masks can it be set to 2048.
            rectangleFactor =
                CVDIFF_SINNER_FACTOR_SMALL_QS;  // Increase S2 to improve
                                                // softmax throughput.
        }
        softmaxSOuterFactor = softmaxUbSize / rectangleFactor;
    ...
    bool res = JittorInferFlashAttentionComputeCVDiffParams(
        tilingData, ubSize, l1Size, l0CSize, softmaxDataTypeSize, minFactor,
        rectangleFactor, maskElemSize, softmaxSOuterFactor);
    OPS_ERR_IF(res == false,
               OPS_REPORT_VECTOR_INNER_ERR(
                   contextKeyParamsPtr->opName,
                   "JittorInferFlashAttentionComputeCVDiffParams failed!"),
               return ge::GRAPH_FAILED);
