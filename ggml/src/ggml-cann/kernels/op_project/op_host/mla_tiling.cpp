...
void GetDecodingNormalTaskTiling(ContextParamsForMLA& contextParams, int32_t &curTask,
    int32_t &prevTaskNum, std::vector<uint32_t>& normalBatchIdx, std::vector<uint32_t>& normalBeginQ, 
    std::vector<uint32_t>& normalCurKvSeq, std::vector<uint32_t>& normalCurQLen)
{
    ...
    for (int32_t taskIdx = 0; taskIdx < contextParams.batch; taskIdx++) {
        ...
        int32_t qSeqLen = contextParams.qSeqLen == nullptr ? 1 : *(contextParams.qSeqLen + batchIdx);
...
ge::graphStatus GetNdMLADecodingMtpTilingTP1(ContextParamsForMLA& contextParams, MLATilingData& tilingData)
{
    int32_t curTask = 0;
    ...
    GetDecodingNormalTaskTiling(contextParams, curTask, prevTaskNum, batchIdxList, qSeqLenList, kvBlockIdxList, prevKVSeqlenList);
    tilingData.ndMLADecodingMtpTilingTP1.set_tailBatch(static_cast<uint32_t>(contextParams.batch - curTask));
    ...
    for (int32_t taskIdx = curTask; taskIdx < contextParams.batch; taskIdx++) {
        ...
        for (int32_t qSeq = 0; qSeq < qLoop; qSeq++) {
            int32_t nowKVSeqlen = contextParams.batchList[taskIdx].kvSeqlen - qLoop + qSeq + 1;
            int32_t kvBlocks = (nowKVSeqlen + contextParams.blockSize - 1) / contextParams.blockSize;
            if (kvBlocks < splitNum) {
                splitNum = kvBlocks;
            }
            int32_t prevKVSeqlen = 0;
            int32_t splitBlocks = (kvBlocks + splitNum - 1) / splitNum;
            if (qSeqLen == 2 && splitBlocks == 1 && nowKVSeqlen % contextParams.blockSize == 1) {
                splitNum--;
            ...
            for (int kvBlockIdx = 0; kvBlockIdx < splitNum; kvBlockIdx++) {
                ...
                kvBlockIdxList[prevTaskNum] = kvBlockIdx == splitNum - 1 ? static_cast<uint32_t>(nowKVSeqlen - prevKVSeqlen) : static_cast<uint32_t>(nowBlocks) * static_cast<uint32_t>(contextParams.blockSize);
                prevKVSeqlenList[prevTaskNum] = static_cast<uint32_t>(prevKVSeqlen);
                batchSizeList[prevTaskNum] = contextParams.quantFlag ? static_cast<uint32_t>(batchIdx) * static_cast<uint32_t>(qSeqLen) + static_cast<uint32_t>(qSeq) : static_cast<uint32_t>(batchIdx) * static_cast<uint32_t>(qSeqLen);
                splitNumList[prevTaskNum] = static_cast<uint32_t>(splitNum);
                prevSplitNumSumList[prevTaskNum] = static_cast<uint32_t>(contextParams.prevSplitNumSum);
    ...
    for (int32_t taskIdx = curTask; taskIdx < contextParams.batch; taskIdx++) {
        ...
        for (int32_t qSeq = 0; qSeq < qSeqLen; qSeq++) {
            uint32_t batchTilingOffset = contextParams.quantFlag ? static_cast<uint32_t>(batchTilingMap[batchIdx * qSeqLen + qSeq]) : static_cast<uint32_t>(batchTilingMap[batchIdx * qSeqLen]);
            uint32_t qOffset = contextParams.quantFlag ? 0 : static_cast<uint32_t>(qSeq);
    ...
    tilingData.ndMLADecodingMtpTilingTP1.set_prevKVSeqlen(prevKVSeqlenList.data());
    ...
    tilingData.ndMLADecodingMtpTilingTP1.set_prevSplitNumSum(prevSplitNumSumList.data());
    return ge::GRAPH_SUCCESS;
}

void GetNdMLAMtpTilingTP1(ContextParamsForMLA& contextParams,  uint32_t &blockDimToBeSet, MLATilingData& tilingData)
{
    ...
    for (int32_t seqIdx = 0; seqIdx < contextParams.batch; seqIdx++) {
        int32_t qSeqLen = contextParams.qSeqLen == nullptr ? 1 : *(contextParams.qSeqLen + seqIdx);
        ...
        int32_t curKvSeq = kvSeqlen - qSeqLen;
        for (int32_t qSeq = 0; qSeq < qSeqLen; qSeq += maxQPerJob) {
            if (totalTaskNum <= static_cast<int32_t>(blockDimToBeSet) && (prevTaskNum % static_cast<int32_t>(blockDimToBeSet) == 0)) {
                maxQPerJob = 1;
            }
            int32_t curQLen = ((qSeqLen - qSeq) > maxQPerJob) ? maxQPerJob : (qSeqLen - qSeq);
            curKvSeq += curQLen;
            batchIdxList[prevTaskNum] = static_cast<uint32_t>(batchIdx);
            beginQList[prevTaskNum] = static_cast<uint32_t>(beginQ) + static_cast<uint32_t>(qSeq);
...
int32_t GetMaxQseqlen(ContextParamsForMLA& contextParams)
{
    ...
    auto maxQSeqlenIter = std::max_element(contextParams.qSeqLen, contextParams.qSeqLen + contextParams.batch);
    auto maxQseqlen = maxQSeqlenIter != contextParams.qSeqLen + contextParams.batch ? *maxQSeqlenIter : 1;
    return maxQseqlen;
}

int32_t GetMaxKVseqlen(ContextParamsForMLA& contextParams)
{
    auto maxKVSeqlenIter = std::max_element(contextParams.kvSeqLen, contextParams.kvSeqLen + contextParams.numTokens);
    auto maxKVseqlen = maxKVSeqlenIter != contextParams.kvSeqLen + contextParams.numTokens ? *maxKVSeqlenIter : 1;
    return maxKVseqlen;
}

ge::graphStatus GetNdMLATiling(ContextParamsForMLA& contextParams, MLATilingData& tilingData)
{
    ...
    for (int32_t seqIdx = 0; seqIdx < contextParams.batch; seqIdx++) {
        ...
        uint64_t addressQffset = static_cast<uint64_t>(contextParams.numHeads * qSeqLen);
        uint64_t addressOffset = static_cast<uint64_t>(contextParams.numHeads * contextParams.embeddingSize * qSeqLen);
    ...
    tilingData.ndMLATiling.set_curQNBlockTile(static_cast<uint32_t>(curQNBlockTile));
...
void GetTilingHead(ContextParamsForMLA& contextParams, uint32_t& blockDimToBeSet, MLATilingData& tilingData)
{
    ...
    tilingData.MLAHead.set_paraSize(contextParams.mtpTp1Flag ? static_cast<uint32_t>(7) : static_cast<uint32_t>(8));
    tilingData.MLAHead.set_numHeads(static_cast<uint32_t>(contextParams.numHeads));
    tilingData.MLAHead.set_headDim(static_cast<uint32_t>(contextParams.embeddingSize));
    tilingData.MLAHead.set_numBlocks(static_cast<uint32_t>(contextParams.numBlocks));
    tilingData.MLAHead.set_blockSize(static_cast<uint32_t>(contextParams.blockSize));
    tilingData.MLAHead.set_maxBlocks(static_cast<uint32_t>(contextParams.maxNumBlocksPerQuery));
    tilingData.MLAHead.set_tor(contextParams.tor);
    tilingData.MLAHead.set_kvHeads((contextParams.kvHeads == 0) ? contextParams.numHeads : contextParams.kvHeads);
    tilingData.MLAHead.set_maskTypeND(static_cast<uint32_t>(contextParams.maskType));

    if (contextParams.flashDecoding) {
        tilingData.MLAHead.set_taskNum(static_cast<uint32_t>(contextParams.normalTaskNum));
        tilingData.MLAHead.set_kvSplitNum(static_cast<uint32_t>(contextParams.splitKVNum));
        tilingData.MLAHead.set_splitTaskNum(static_cast<uint32_t>(contextParams.prevSplitNumSum));
    } else {
        tilingData.MLAHead.set_taskNum(static_cast<uint32_t>(contextParams.totalTaskNum));
    }

    tilingData.MLAHead.set_blockDim(static_cast<uint32_t>(blockDimToBeSet));
}

ge::graphStatus GetMLATilingParam(ContextParamsForMLA& contextParams, uint32_t& blockDimToBeSet, MLATilingData& tilingData)
{
...
size_t GetMLAWorkSpaceSize(ContextParamsForMLA& contextParams, uint32_t& blockDim)
{
    ...
    uint64_t oCoreWorkSpaceSize = contextParams.flashDecoding && contextParams.mtpTp1Flag ?
        WORKSPACE_BLOCK_SIZE_DB * contextParams.flashDecodingTaskNum * contextParams.splitKVNum * dataLenFloat * 2 : 0;
    uint64_t lWorkSpaceSize = contextParams.flashDecoding && contextParams.mtpTp1Flag ?
        contextParams.numHeads * contextParams.flashDecodingTaskNum * contextParams.splitKVNum * dataLenFloat * 2 * 8 : 0;
    if (contextParams.quantFlag) {
        uint64_t sWorkSpaceSize = contextParams.mtpTp1Flag ? basicWorkSpaceFloat * 2 : basicWorkSpaceFloat;
        ...
        uint64_t sWorkSpaceSize = contextParams.mtpTp1Flag ? basicWorkSpaceFloat * 4 : basicWorkSpaceFloat * 2;
        uint64_t pWorkSpaceSize = contextParams.mtpTp1Flag ? basicWorkSpaceHalf * 4 : basicWorkSpaceHalf * 2;
        uint64_t oTempWorkSpaceSize = contextParams.mtpTp1Flag ? basicWorkSpaceFloat * 4 : basicWorkSpaceFloat * 2;
        uint64_t goWorkSpaceSize = contextParams.mtpTp1Flag ? basicWorkSpaceFloat * 2 : basicWorkSpaceFloat;
        workspaceSize = sysWorkspaceSize + sWorkSpaceSize + 512 + pWorkSpaceSize +
                        oTempWorkSpaceSize + goWorkSpaceSize + oCoreWorkSpaceSize + lWorkSpaceSize;
    }
