...
int32_t GetKvFactor(ContextParamsForMLA& contextParams, int32_t kvSeqlen) {
    ...
    int32_t kvFactor = (static_cast<int64_t>(contextParams.batch) *
                            static_cast<int64_t>(contextParams.maxKvSeqLen) ==
                        kFirstDimVal)
                           ? contextParams.maxKvSeqLen
                           : kvSeqlen;
    return kvFactor;
}

void PrefillTilingHead(ContextParamsForMLA& contextParams, const float tor,
                       AddrOffsets& addrOffsets,
                       MLAPrefillTilingData& tilingData, int32_t kvRealHeads) {
    tilingData.MLAPrefillHead.set_batch(
        static_cast<uint32_t>(contextParams.batch));
    tilingData.MLAPrefillHead.set_maxSeqLen(
        static_cast<uint32_t>(contextParams.maxSeqLen));
    tilingData.MLAPrefillHead.set_numHeads(
        static_cast<uint32_t>(contextParams.numHeads));
    tilingData.MLAPrefillHead.set_embeddingSize(
        static_cast<uint32_t>(contextParams.embeddingSize));
    tilingData.MLAPrefillHead.set_kvRealHeads(
        static_cast<uint32_t>(kvRealHeads));
    tilingData.MLAPrefillHead.set_tor(tor);
    tilingData.MLAPrefillHead.set_headStride(
        static_cast<uint32_t>(contextParams.headStride));
    tilingData.MLAPrefillHead.set_maskStride(
        static_cast<uint32_t>(contextParams.maskStride));
    tilingData.MLAPrefillHead.set_totalQBlkNum(
        static_cast<uint32_t>(addrOffsets.totalQBlkNum));
    tilingData.MLAPrefillHead.set_headSize(
        static_cast<uint32_t>(TILING_HEAD_SIZE_PREFILL));
    tilingData.MLAPrefillHead.set_paraSize(
        static_cast<uint32_t>(TILING_PARA_SIZE_PREFILL));
    ...
    tilingData.MLAPrefillHead.set_embeddingSizeV(
        static_cast<uint32_t>(contextParams.embeddingSizeV));
    tilingData.MLAPrefillHead.set_maxKvSeqLenBNSD(
        contextParams.maxKvSeqLen);  // for bnsd, not used
    tilingData.MLAPrefillHead.set_windowSize(contextParams.windowSize);
}

ge::graphStatus PrefillTilingParam(ContextParamsForMLA& contextParams,
                                   float tor, AddrOffsets& addrOffsets,
                                   int32_t kvRealHeads,
    ...
    for (int32_t seqIdx = 0; seqIdx < contextParams.batch; seqIdx++) {
        ...
        int32_t embeddingSizeAligned =
            (contextParams.embeddingSize + 16 - 1) / 16 * 16;
        int32_t nIbd = 0;
        ...
        mUbd = PP_MM[mIbd];
        addrOffsets.totalQBlkNum +=
            (qSeqlen != 0 && kvSeqlen != 0) ? ((qSeqlen + mUbd - 1) / mUbd) : 0;
        ...
        totalQBlkNumList[seqIdx] =
            static_cast<uint32_t>(addrOffsets.totalQBlkNum);

        auto kvFactor = GetKvFactor(contextParams, kvSeqlen);
        addrOffsets.addrQSeqOffset += static_cast<uint64_t>(qSeqlen) *
                                      contextParams.numHeads *
                                      contextParams.embeddingSizeV;
        addrOffsets.addrKSeqOffset += static_cast<uint64_t>(kvFactor) *
                                      kvRealHeads *
                                      contextParams.embeddingSizeV;
        addrOffsets.addrVSeqOffset += static_cast<uint64_t>(kvFactor) *
                                      kvRealHeads *
                                      contextParams.embeddingSizeV;
        addrOffsets.addrOSeqOffset += static_cast<uint64_t>(qSeqlen) *
                                      contextParams.numHeads *
                                      contextParams.embeddingSizeV;
    }
    ...
    addrOffsets.block =
        static_cast<int64_t>(contextParams.numHeads) * addrOffsets.totalQBlkNum;
    return ge::GRAPH_SUCCESS;
}

ge::graphStatus GetMLAPrefillTilingParam(ContextParamsForMLA& contextParams,
                                         uint32_t& blockDimToBeSet,
                                         MLAPrefillTilingData& tilingData) {
    AddrOffsets addrOffsets;
    float tor = contextParams.tor;
    int32_t kvRealHeads = contextParams.kvHeads > 0 ? contextParams.kvHeads
                                                    : contextParams.numHeads;
    auto ret1 = PrefillTilingParam(contextParams, tor, addrOffsets, kvRealHeads,
                                   tilingData);
    if (ret1 == ge::GRAPH_FAILED) return ge::GRAPH_FAILED;

    blockDimToBeSet = contextParams.aicNum;
    if (addrOffsets.block < 0 ||
        static_cast<uint32_t>(addrOffsets.block) > UINT32_MAX)
    ...
    tilingData.MLAPrefillHead.set_blockDim(
        static_cast<uint32_t>(blockDimToBeSet));
    return ge::GRAPH_SUCCESS;
}
...
size_t GetMLAPrefillWorkSpaceSize(ContextParamsForMLA& contextParams,
    ...
    uint64_t sSize = static_cast<uint64_t>(blockDim) *
