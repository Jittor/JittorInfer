...
int32_t CalcSplitNum(ContextParamsForMLA& contextKeyParams, int32_t blockDim,
                     int32_t minKVSeqlen, int32_t blockSize) {
    if (blockDim - contextKeyParams.flashDecodingTaskNum <= 4 ||
        contextKeyParams.quantFlag) {
        return 1;
    }
    ...
    for (int32_t splitNum = 2; splitNum <= 6; splitNum++) {
        if ((contextKeyParams.flashDecodingTaskNum * splitNum) % blockDim !=
            0) {
            continue;
        }
        int32_t repeatTimesPerBlock =
            contextKeyParams.flashDecodingTaskNum * splitNum / blockDim;
        if (minKVSeqlen / splitNum >= blockSize &&
            repeatTimesPerBlock + 6 <=
                minKVBlocks - (minKVBlocks + splitNum - 1) / splitNum *
                                  repeatTimesPerBlock) {
            return splitNum;
        } else {
            return 1;
...
ge::graphStatus BatchSeqSort(ContextParamsForMLA& contextKeyParams,
                             uint32_t endIndex, uint32_t sortDim) {
    if (sortDim == 0 || endIndex <= 0) {
        return ge::GRAPH_SUCCESS;
    }
    std::sort(contextKeyParams.batchList.begin(),
              contextKeyParams.batchList.end());
    std::reverse(contextKeyParams.batchList.begin(),
                 contextKeyParams.batchList.begin() + endIndex);
    ...
    for (uint32_t sortIdx = 0; sortIdx < batchSortInfo; sortIdx++) {
        ...
        if (sortIdx % 2 == 0) {
            std::sort(contextKeyParams.batchList.begin() + startIndex,
                      contextKeyParams.batchList.begin() + sortEnd);
        } else {
            std::sort(contextKeyParams.batchList.begin() + startIndex,
                      contextKeyParams.batchList.begin() + sortEnd,
                      std::greater<BatchNode>());
        }
    }
    return ge::GRAPH_SUCCESS;
}

ge::graphStatus GetFlashDecodingInfo(ContextParamsForMLA& contextKeyParams,
                                     uint32_t blockDim) {
    if (blockDim == 0) {
    ...
    contextKeyParams.flashDecodingTaskNum = contextKeyParams.quantFlag
                                                ? contextKeyParams.tailTaskNum
                                                : contextKeyParams.tailBatch;
    auto minKVSeqlen = std::min_element(
        contextKeyParams.kvSeqLen,
        contextKeyParams.kvSeqLen + contextKeyParams.kvSeqLength);
    auto minQSeqlen = contextKeyParams.qSeqLen != nullptr
                          ? *std::min_element(contextKeyParams.qSeqLen,
                                              contextKeyParams.qSeqLen +
                                                  contextKeyParams.batch)
                          : 1;
    auto maxQSeqlen = contextKeyParams.qSeqLen != nullptr
                          ? *std::max_element(contextKeyParams.qSeqLen,
                                              contextKeyParams.qSeqLen +
                                                  contextKeyParams.batch)
                          : 1;
    contextKeyParams.flashDecoding =
        !contextKeyParams.quantFlag &&
        contextKeyParams.flashDecodingTaskNum != 0 && *minKVSeqlen >= 2048 &&
        ((minQSeqlen == 2 && maxQSeqlen == 2) ||
         (minQSeqlen == 1 && maxQSeqlen == 1));
    if (!contextKeyParams.flashDecoding) {
        if (BatchSeqSort(contextKeyParams, contextKeyParams.batch, blockDim) ==
            ge::GRAPH_FAILED) {
            return ge::GRAPH_FAILED;
        }
        return ge::GRAPH_SUCCESS;
    ...
    if (BatchSeqSort(
            contextKeyParams,
            contextKeyParams.batch - contextKeyParams.flashDecodingTaskNum,
            blockDim) == ge::GRAPH_FAILED) {
        return ge::GRAPH_FAILED;
    }
    contextKeyParams.splitKVNum =
        blockDim / contextKeyParams.flashDecodingTaskNum > 1
            ? blockDim / contextKeyParams.flashDecodingTaskNum
            : CalcSplitNum(contextKeyParams, blockDim, *minKVSeqlen,
                           contextKeyParams.blockSize);
    contextKeyParams.flashDecoding =
        contextKeyParams.splitKVNum == 1 ? false : true;
    int32_t taskNum = contextKeyParams.quantFlag ? contextKeyParams.totalTaskNum
                                                 : contextKeyParams.batch;
    contextKeyParams.normalTaskNum = taskNum / blockDim * blockDim;
    return ge::GRAPH_SUCCESS;
}

static ge::graphStatus ConvertContextToMLAParams(
    ...
    contextKeyParams.kNz = (kcacheShape.GetDim(KDims - 1) == 16 ||
                            kcacheShape.GetDim(KDims - 1) == 32)
                               ? 1
                               : 0;
    if (contextKeyParams.kNz) {
        contextKeyParams.embeddingSize =
            static_cast<int32_t>(kcacheShape.GetDim(3)) *
            static_cast<int32_t>(kcacheShape.GetDim(1));
        contextKeyParams.blockSize =
            static_cast<int32_t>(kcacheShape.GetDim(2));
    } else {
        contextKeyParams.embeddingSize =
            static_cast<int32_t>(kcacheShape.GetDim(3));
        contextKeyParams.blockSize =
    ...
    contextKeyParams.kvSeqLen =
        const_cast<int64_t*>(context->GetInputTensor(5)->GetData<int64_t>());
    contextKeyParams.kvSeqLength = context->GetInputTensor(5)->GetShapeSize();
    contextKeyParams.qSeqLen =
        context->GetInputTensor(7) != nullptr
            ? const_cast<int64_t*>(
                  context->GetInputTensor(7)->GetData<int64_t>())
            : nullptr;
    contextKeyParams.qSeqLength =
        context->GetInputTensor(7) != nullptr
            ? context->GetInputTensor(7)->GetShapeSize()
            : 0;

    contextKeyParams.numTokens = static_cast<int32_t>(
        context->GetInputShape(5)->GetOriginShape().GetDim(0));
    contextKeyParams.numBlocks = static_cast<int32_t>(kcacheShape.GetDim(0));
    contextKeyParams.maxNumBlocksPerQuery =
        static_cast<int32_t>(tableShape.GetDim(1));
    ...
    contextKeyParams.quantFlag =
        (static_cast<int32_t>(contextKeyParams.type) < 2) ? 0 : 1;
    auto maxQSeqlen = contextKeyParams.qSeqLen != nullptr
                          ? *std::max_element(contextKeyParams.qSeqLen,
                                              contextKeyParams.qSeqLen +
                                                  contextKeyParams.qSeqLength)
                          : 1;
    contextKeyParams.mtpTp1Flag =
        ((contextKeyParams.numHeads == 128) ||
         (context->GetInputTensor(0)->GetDataType() == ge::DT_INT8 &&
          maxQSeqlen > 1));
    if (contextKeyParams.mtpTp1Flag ||
        static_cast<int32_t>(contextKeyParams.type) >= 2) {
        contextKeyParams.maskType = 0;
    }
    int32_t beginQ = 0;
    for (int32_t batchIdx = 0; batchIdx < contextKeyParams.batch; batchIdx++) {
        contextKeyParams.batchList.push_back(BatchNode(
            batchIdx, *(contextKeyParams.kvSeqLen + batchIdx), beginQ));
        beginQ = contextKeyParams.qSeqLen == nullptr
                     ? beginQ + 1
                     : beginQ + *(contextKeyParams.qSeqLen + batchIdx);
    }
    contextKeyParams.totalTaskNum =
        contextKeyParams.qSeqLen != nullptr
            ? std::accumulate(contextKeyParams.qSeqLen,
                              contextKeyParams.qSeqLen + contextKeyParams.batch,
    ...
    if (contextKeyParams.mtpTp1Flag) {
        if (GetFlashDecodingInfo(contextKeyParams, blockDim) ==
            ge::GRAPH_FAILED) {
            return ge::GRAPH_FAILED;
        }
    }
...
ge::graphStatus GetMLATilingKeyTypeBase(
...
ge::graphStatus GenMLATilingKey(ContextParamsForMLA& contextParams,
                                gert::TilingContext* context) {
    uint32_t dataType = static_cast<int32_t>(contextParams.type);
    uint32_t tilingKey = dataType + (contextParams.kNz << 4) +
                         (contextParams.mtpTp1Flag << 2) +
                         (contextParams.flashDecoding << 6);
...
ge::graphStatus MLATilingFunc(gert::TilingContext* context) {
    ...
    if (GetMLATilingParam(contextParamsForMLA, blockDimToBeSet, tilingData) ==
        ge::GRAPH_FAILED)
        return ge::GRAPH_FAILED;
    size_t* workspaces = contextParamsForMLA.workspaceSize;
    workspaces[0] = GetMLAWorkSpaceSize(contextParamsForMLA, blockDimToBeSet);

    if (GenMLATilingKey(contextParamsForMLA, context) == ge::GRAPH_FAILED)
        return ge::GRAPH_FAILED;
...
class MLA : public OpDef {
   public:
    explicit MLA(const char* name) : OpDef(name) {
        ...
        this->SetInferShape(ge::MLAInferShape)
            .SetInferDataType(ge::MLAInferDataType);
