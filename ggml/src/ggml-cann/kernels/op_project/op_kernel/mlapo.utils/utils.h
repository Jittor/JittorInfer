...
__aicore__ inline void CreateCaMatrix(const AscendC::LocalTensor<IN_DTYPE> &dst,
    ...
    AscendC::InitConstValue<IN_DTYPE>(
        dst, AscendC::InitConstValueParams<IN_DTYPE>(repeats, blockNum, dstGap,
                                                     initValue));
}
...
__aicore__ inline void SetNdpara(uint16_t ndNum, uint16_t srcNdStride,
                                 uint16_t dstNdStride) {
    AscendC::SetFixpipeNz2ndFlag(ndNum, srcNdStride, dstNdStride);
}
template <typename IN_DTYPE>
__aicore__ inline void SetVectorMask(const uint64_t maskHigh,
                                     const uint64_t maskLow) {
...
__aicore__ inline void SetFpc(const AscendC::LocalTensor<IN_DTYPE> &preTensor,
