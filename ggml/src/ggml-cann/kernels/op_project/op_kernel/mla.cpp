...
#ifdef __DAV_C220_CUBE__
template <TilingKeyType tilingKeyType = TilingKeyType::TILING_HALF_DATA, typename IN_DTYPE = half, typename OUT_DTYPE = half, typename IN_KVDTYPE = half,
...
public:
    ...
    __aicore__ __attribute__((always_inline)) inline void Init(
        __gm__ uint8_t *__restrict__ qNope,
        __gm__ uint8_t *__restrict__ qRope,
        __gm__ uint8_t *__restrict__ ctKV,
        __gm__ uint8_t *__restrict__ kRope,
        ...
        __gm__ const MLATilingData* __restrict__ tiling)
    {
        ...
        k_gm_tensor.SetGlobalBuffer(reinterpret_cast<__gm__ IN_KVDTYPE *>(ctKV));
        k_rope_gm_tensor.SetGlobalBuffer(reinterpret_cast<__gm__ IN_ROPE_DTYPE *>(k_rope_gm));

        __gm__ const MLAHeadParams* __restrict headparams = &(tiling_gm->MLAHead);
        __gm__ const NdMLATiling* __restrict ndtiling = &(tiling_gm->ndMLATiling);
        ...
        const uint64_t sWorkSpaceSize = mtpTp1Flag ? basicWorkSpaceFloat * 4 : basicWorkSpaceFloat * 2;
        const uint64_t pWorkSpaceSize = mtpTp1Flag ? basicWorkSpaceHalf * 4 : basicWorkSpaceHalf * 2;
        const uint64_t oTempWorkSpaceSize = mtpTp1Flag ? basicWorkSpaceFloat * 4 : basicWorkSpaceFloat * 2;
        const uint64_t defaultSysWorkspaceSize910B = 16U * 1024U * 1024U;
  
        __gm__ mm1CopyType* s_out_gm = reinterpret_cast<__gm__ mm1CopyType*>(workspace);
        __gm__ IN_DTYPE* p_out_gm = reinterpret_cast<__gm__ IN_DTYPE*>(workspace + sWorkSpaceSize + 512);
        __gm__ mm2CopyType* o_tmp_gm = reinterpret_cast<__gm__ mm2CopyType *>(workspace + sWorkSpaceSize + 512 + pWorkSpaceSize);
    ...
    __aicore__ __attribute__((always_inline)) inline void Run()
    {
        ...
        for (uint32_t process = block_idx; process < process_num; process += (uint32_t)block_num) {  // for task
            ...
            InnerRunCubeMLA(cur_batch, start_head, cur_head_num, start_kv, cur_q_seq_len, cur_kv_seqlen, ndtiling);
        }
    ...
    __aicore__ __attribute__((always_inline)) inline void RunTP1()
    {
        ...
        __gm__ const NdMLADecodingMtpTilingTP1* __restrict ndMtpTiling = &(tiling_gm->ndMLADecodingMtpTilingTP1);
        ...
        if constexpr (flashDecoding) {
            for (uint32_t process = block_idx; process < flashDecodingTaskNum; process += (uint32_t)block_num) {  // for task
                ...
                uint32_t cur_batch = (uint32_t)(ndMtpTiling->batchIdx[totalTaskNum + process]);
                uint32_t q_seqlen = (uint32_t)(ndMtpTiling->qSeqLen[totalTaskNum + process]);
                uint32_t kv_seqlen = (uint32_t)(ndMtpTiling->kvBlockIdx[totalTaskNum + process]);
                if (kv_seqlen == 0) {
                    continue;
                }
                uint32_t start_kv = (uint32_t)(ndMtpTiling->prevKVSeqlen[totalTaskNum + process]);
                uint32_t prev_task = (uint32_t)(ndMtpTiling->batchSize[totalTaskNum + process]);
                uint32_t split_num = (uint32_t)(ndMtpTiling->splitNum[totalTaskNum + process]);
                uint32_t prev_split_num = (uint32_t)(ndMtpTiling->prevSplitNumSum[totalTaskNum + process]);
...
private:
    __aicore__ __attribute__((always_inline)) inline void InnerRunCubeMLA(uint32_t cur_batch, uint32_t start_head, uint32_t cur_head_num,
        uint32_t start_kv, uint32_t cur_q_seqlen, uint32_t cur_kv_seqlen, const __gm__ NdMLATiling* ndtiling)
    {
...
public:
        ...
        __gm__ uint8_t* __restrict__ s_rope_out_gm = nullptr)
    {
        ...
        const uint64_t pWorkSpaceSize = mtpTp1Flag ? basicWorkSpaceHalf * 4 : basicWorkSpaceHalf * 2;
        const uint64_t oTempWorkSpaceSize = mtpTp1Flag ? basicWorkSpaceFloat * 4 : basicWorkSpaceFloat * 2;
        const uint64_t goWorkSpaceSize = mtpTp1Flag ? basicWorkSpaceFloat * 2 : basicWorkSpaceFloat;
        const uint64_t defaultSysWorkspaceSize910B = 16U * 1024U * 1024U;

        __gm__ mm1CopyType* s_gm = reinterpret_cast<__gm__ mm1CopyType*>(workspace);
        __gm__ IN_DTYPE* p_gm = reinterpret_cast<__gm__ IN_DTYPE*>(workspace + sWorkSpaceSize + 512);
        __gm__ mm2CopyType* o_tmp_gm = reinterpret_cast<__gm__ mm2CopyType *>(workspace + sWorkSpaceSize + 512 + pWorkSpaceSize);
        __gm__ float* go_gm = reinterpret_cast<__gm__ float *>(workspace + sWorkSpaceSize + 512 + pWorkSpaceSize + oTempWorkSpaceSize);

        o_gm = reinterpret_cast<__gm__ OUT_DTYPE *>(attenOut);
        o_gm_tensor.SetGlobalBuffer(reinterpret_cast<__gm__ OUT_DTYPE *>(o_gm));
        mask_gm_tensor.SetGlobalBuffer(reinterpret_cast<__gm__ OUT_DTYPE *>(mask));
        ...
        if constexpr (tilingKeyType == TilingKeyType::TILING_INT8_DATA) {
            deq_scale_gm_tensor_q1.SetGlobalBuffer(reinterpret_cast<__gm__ float*>(qkDescale));
            deq_scale_gm_tensor_k1.SetGlobalBuffer(reinterpret_cast<__gm__ float*>(pvDescale));
   ...
   __aicore__ __attribute__((always_inline)) inline void ReduceMaxRepeatM(
    ...
    __aicore__ __attribute__((always_inline)) inline void CombineScaleBlock(uint32_t num_tokens, uint32_t q_heads, uint32_t kv_split_core_num, uint32_t embedding_size)
    {
        ...
        if(curDecodingBatch == 0) {
            batchTilingOffset = (uint32_t)(ndMtpTiling->batchIdx[totalTaskNum + flashDecodingTaskNum]);
            qOffset = (uint32_t)(ndMtpTiling->qSeqLen[totalTaskNum + flashDecodingTaskNum]);
        } else if(curDecodingBatch == 1) {
            batchTilingOffset = (uint32_t)(ndMtpTiling->kvBlockIdx[totalTaskNum + flashDecodingTaskNum]);
            qOffset = (uint32_t)(ndMtpTiling->prevKVSeqlen[totalTaskNum + flashDecodingTaskNum]);
        } else if(curDecodingBatch == 2) {
            batchTilingOffset = (uint32_t)(ndMtpTiling->batchSize[totalTaskNum + flashDecodingTaskNum]);
            qOffset = (uint32_t)(ndMtpTiling->splitNum[totalTaskNum + flashDecodingTaskNum]);
        }
    ...
    {
        __gm__ const NdMLADecodingMtpTilingTP1* __restrict ndMtpTiling = &(tiling_gm->ndMLADecodingMtpTilingTP1);
        uint32_t prev_task = (uint32_t)(ndMtpTiling->batchSize[batchTilingOffset]);
        uint32_t cur_kv_split_core_num = (uint32_t)(ndMtpTiling->splitNum[batchTilingOffset]);
        uint32_t prev_split_core_num = (uint32_t)(ndMtpTiling->prevSplitNumSum[batchTilingOffset]);
                    ...
                    for (uint32_t m_ind = 0; m_ind < m_end; m_ind++) {
                        ...
                        if constexpr (std::is_same_v<IN_DTYPE,int8_t>) {
                            ...
                            OnlineSoftmaxStage1Quant(
                                ...
                                pm32_ubuf_tensor[((n_idx / s_block_stack) % 2) * UB_FLOAT_LINE_SIZE * 2 + row_offset],
...
extern "C" __global__ __aicore__ void mla(
    ...
    __gm__ uint8_t *__restrict__ kRope,
    __gm__ uint8_t *__restrict__ blockTables,
    __gm__ uint8_t *__restrict__ contextLens,
    __gm__ uint8_t *__restrict__ mask,
    ...
    __gm__ uint8_t *__restrict__ tiling)
{
    ...
    __gm__ const MLATilingData* __restrict tiling_data = reinterpret_cast<__gm__ const MLATilingData*>(tiling);
    ...
    if (TILING_KEY_IS(0)) { // fp16
        #ifdef __DAV_C220_CUBE__
        MLAttentionOp_C<TilingKeyType::TILING_HALF_DATA, half, half, half, InputFormat::ND_FORMAT> pa_aic_fp16 {};
        pa_aic_fp16.Init(qNope, qRope, ctKV, kRope, blockTables, user, tiling_data);
        pa_aic_fp16.Run();
        #elif __DAV_C220_VEC__
        ...
        #ifdef __DAV_C220_CUBE__
        MLAttentionOp_C<TilingKeyType::TILING_HALF_DATA, half, half, half, InputFormat::NZ_FORMAT> pa_aic_fp16 {};
        pa_aic_fp16.Init(qNope, qRope, ctKV, kRope, blockTables, user, tiling_data);
        pa_aic_fp16.Run();
        #elif __DAV_C220_VEC__
        MLAttentionOp_V<TilingKeyType::TILING_HALF_DATA, half, half> pa_aiv {};
        ...
        pa_aic_fp16.RunTP1();
        #elif __DAV_C220_VEC__
        MLAttentionOp_V<TilingKeyType::TILING_HALF_DATA, half, half, false, BlockStack::FOUR_FLOW> pa_aiv {};
        pa_aiv.Init(qkDescale, pvDescale, mask, attenOut, user, tiling_data);
        pa_aiv.RunTP1();
        #endif
        ...
        #ifdef __DAV_C220_CUBE__
        MLAttentionOp_C<TilingKeyType::TILING_HALF_DATA, half, half, half, InputFormat::ND_FORMAT, true> pa_aic_fp16 {};
        pa_aic_fp16.Init(qNope, qRope, ctKV, kRope, blockTables, user, tiling_data);
        pa_aic_fp16.RunTP1();
        #elif __DAV_C220_VEC__
        MLAttentionOp_V<TilingKeyType::TILING_HALF_DATA, half, half, false, BlockStack::FOUR_FLOW, true> pa_aiv {};
        pa_aiv.Init(qkDescale, pvDescale, mask, attenOut, user, tiling_data);
        pa_aiv.RunTP1();
        #endif
    } else if (TILING_KEY_IS(84)) { // fp16 + nz + FOUR_FLOW + flashdecoding 0b1010100
        #ifdef __DAV_C220_CUBE__
        MLAttentionOp_C<TilingKeyType::TILING_HALF_DATA, half, half, half, InputFormat::NZ_FORMAT, true> pa_aic_fp16 {};
        pa_aic_fp16.Init(qNope, qRope, ctKV, kRope, blockTables, user, tiling_data);
        pa_aic_fp16.RunTP1();
        #elif __DAV_C220_VEC__
        MLAttentionOp_V<TilingKeyType::TILING_HALF_DATA, half, half, false, BlockStack::FOUR_FLOW, true> pa_aiv {};
