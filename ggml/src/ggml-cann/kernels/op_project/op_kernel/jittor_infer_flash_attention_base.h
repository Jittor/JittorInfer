...
/*!
 * \file jittor_infer_flash_attention_base.h
 * \brief
 */
#ifndef JITTOR_INFER_FLASH_ATTENTION_BASE_H
#define JITTOR_INFER_FLASH_ATTENTION_BASE_H
...
enum Mode { HighPrecision, HighPerformance };

template <typename T, Mode M = Mode::HighPerformance>
struct JittorInferFlashAttentionTypeTraits {
    using mmInputType = T;
    using mmBiasType = T;
...
template <>
struct JittorInferFlashAttentionTypeTraits<half, Mode::HighPerformance> {
...
template <>
struct JittorInferFlashAttentionTypeTraits<half, Mode::HighPrecision> {
    using mmInputType = half;
    using mmBiasType = float;
    using mmOutputType = float;
    using softmaxType = float;
    using pseShiftType = half;
    using pseShiftCastType = float;  // pseShiftCastType is only fp32 in the
                                     // case of high precision and bf16
};

template <>
struct JittorInferFlashAttentionTypeTraits<bfloat16_t> {
...
template <>
struct JittorInferFlashAttentionTypeTraits<int8_t> {
...
template <typename T, typename U, CubeFormat FORMAT, typename O,
          Mode M = Mode::HighPerformance>
class JittorInferFlashAttentionBase {
   public:
    __aicore__ inline JittorInferFlashAttentionBase(){};
    ...
    // define datatype
    using mmInputType =
        typename JittorInferFlashAttentionTypeTraits<T, M>::mmInputType;
    using mmBiasType =
        typename JittorInferFlashAttentionTypeTraits<T, M>::mmBiasType;
    using mmOutputType =
        typename JittorInferFlashAttentionTypeTraits<T, M>::mmOutputType;
    using softmaxType =
        typename JittorInferFlashAttentionTypeTraits<T, M>::softmaxType;
    using pseShiftType =
        typename JittorInferFlashAttentionTypeTraits<T, M>::pseShiftType;
    using pseShiftCastType =
        typename JittorInferFlashAttentionTypeTraits<T, M>::pseShiftCastType;
    // define matmul
    using a1Type =
        MatmulType<TPosition::GM, CubeFormat::ND, mmInputType, false>;
    ...
    using c2Type = MatmulType<TPosition::VECCALC, CubeFormat::ND_ALIGN,
                              mmOutputType>;  // cpu
#else
    using c2Type =
        MatmulType<TPosition::VECCALC, CubeFormat::ND, mmOutputType>;  // npu
#endif
   ...
   protected:
    ...
    __aicore__ inline void ElewiseCompute(LocalTensor<mmOutputType>& mmResUb,
                                          uint32_t computeSize, uint32_t type);

    __aicore__ inline void SoftmaxBasicComputeFirst(
        LocalTensor<mmOutputType>& mmResUb, LocalTensor<float>& softmaxMaxUb,
        LocalTensor<float>& softmaxSumUb, SoftMaxShapeInfo& shapeInfo);

    __aicore__ inline void SoftmaxComputeFirst(
        LocalTensor<mmOutputType>& mmResUb, LocalTensor<float>& softmaxMaxUb,
        LocalTensor<float>& softmaxSumUb, SoftMaxShapeInfo& shapeInfo);

    __aicore__ inline bool IsSoftmaxBasic();

    __aicore__ inline void SoftmaxBasicCompute(
        LocalTensor<mmOutputType>& mmResUb, LocalTensor<float>& softmaxMaxUb,
    ...
    __aicore__ inline void Bmm2UpdateDivNoTail(
        LocalTensor<mmOutputType>& bmm2ResPreUb,
        LocalTensor<float>& softmaxSumUb,
        LocalTensor<softmaxType>& softmaxExpUb);

    __aicore__ inline void Bmm2Compute(uint32_t offset,
    ...
    __aicore__ inline void DataCopyTransposeOut(
        LocalTensor<mmOutputType>& bmm2ResUb);

    __aicore__ inline void DataCopyOutWithBNSD(
        LocalTensor<mmOutputType>& bmm2ResUb);
    ...
    __aicore__ inline void LoopSOuterOffsetInit(uint32_t seqListOffsetSize,
                                                int sIdx);

    __aicore__ inline void LoopSOuterOffsetInitWithBNSD(
    ...
    __aicore__ inline void GetSparseParam(int32_t* preTokens,
                                          int32_t* nextTokens);
...
__aicore__ inline void JittorInferFlashAttentionBase<T, U, FORMAT, O, M>::Init(
    __gm__ uint8_t* query, __gm__ uint8_t* key, __gm__ uint8_t* value,
    __gm__ uint8_t* pseShift, __gm__ uint8_t* attenMask,
    __gm__ uint8_t* actualSeqLengths, __gm__ uint8_t* actualSeqLengthsKV,
    __gm__ uint8_t* blocktable, __gm__ uint8_t* attentionOut,
    ...
    for (int i = 0; i < tilingData->promptAttentionBaseParams.batchSize; i++) {
        actualSeqLengthsIdx =
            isActualLenDimsNull ? tilingData->promptAttentionBaseParams.seqSize
                                : actualSeqLengthsGm.GetValue(i);
        if (tilingData->promptAttentionBaseParams.isActualSeqLengthsNull) {
    ...
    uint32_t pseMaskMaxSize =
        tilingData->promptAttentionBaseParams.pseMaskMaxSize;
    pipe->InitBuffer(
        attenMaskQueue, 1,
        (tilingData->promptAttentionTensorSizeRect.attenMaskUbSize) *
            pseMaskMaxSize);
    pipe->InitBuffer(
        softmaxOutQueue, 1,
    ...
    if (tilingData->promptAttentionBaseParams.splitS2 == 1) {
        pipe->InitBuffer(
            ...
            tilingData->promptAttentionTensorSizeRect.softmaxExpSize *
                sizeof(softmaxType));
        softmaxExpUb = softmaxExpUb_.Get<softmaxType>(
            tilingData->promptAttentionTensorSizeRect.softmaxExpSize);
    ...
    pipe->InitBuffer(Bmm1Queue, 1,
                     tilingData->promptAttentionTensorSizeRect.mmResUbSize *
                         sizeof(mmOutputType));
    if ((tilingData->promptAttentionTensorSizeRect.tmpSoftMaxV2Size) != 0 &&
        (tilingData->promptAttentionTensorSizeRect.mm2TmpUbSize != 0)) {
        pipe->InitBuffer(
            tmpSoftmaxFlashV2Ub_,
            tilingData->promptAttentionTensorSizeRect.tmpSoftMaxV2Size /
                UB_ALIGN * UB_ALIGN * sizeof(uint8_t));
        pipe->InitBuffer(
            tmpmm2Ub_, tilingData->promptAttentionTensorSizeRect.mm2TmpUbSize *
                           sizeof(uint8_t));
    }
...
JittorInferFlashAttentionBase<T, U, FORMAT, O, M>::InitOutputSingleCore() {
    auto& initParams = tilingData->promptAttentionInitOutputParams;
    uint32_t tailSize =
        initParams.totalOutputSize - tmp_block_idx * initParams.singleCoreSize;
    uint32_t singleInitOutputSize = tailSize < initParams.singleCoreSize
                                        ? tailSize
                                        : initParams.singleCoreSize;
    InitOutput<O>(attentionOutGm[tmp_block_idx * initParams.singleCoreSize],
                  singleInitOutputSize, 0);
    SyncAll();
}
#ifndef PFA_UT
template <>
__aicore__ inline void JittorInferFlashAttentionBase<
    int8_t, bool, CubeFormat::ND, int8_t>::InitOutputSingleCore() {}

template <>
__aicore__ inline void JittorInferFlashAttentionBase<
    int8_t, half, CubeFormat::ND, int8_t>::InitOutputSingleCore() {}

template <>
__aicore__ inline void JittorInferFlashAttentionBase<
    int8_t, float, CubeFormat::ND, int8_t>::InitOutputSingleCore() {}
#endif
template <typename T, typename U, CubeFormat FORMAT, typename O, Mode M>
__aicore__ inline void
JittorInferFlashAttentionBase<T, U, FORMAT, O, M>::initOffset() {
...
template <typename T, typename U, CubeFormat FORMAT, typename O, Mode M>
__aicore__ inline void
JittorInferFlashAttentionBase<T, U, FORMAT, O, M>::InitTensorSize(
    const PromptAttentionSingleCoreTensorSize* tensorSizeTiling) {
    mmResUbSize = tensorSizeTiling->mmResUbSize;  // Matrix mu result UB size
    attenMaskUbSize =
        tensorSizeTiling->attenMaskUbSize;  // Attention mask UB size
    ...
    bmm2ResUbSize =
        tensorSizeTiling->bmm2ResUbSize;  // Second matric mul result UB size
...
template <typename T, typename U, CubeFormat FORMAT, typename O, Mode M>
__aicore__ inline void
JittorInferFlashAttentionBase<T, U, FORMAT, O, M>::ElewiseCompute(
    ...
    if (useMask) {
        if (maskTypeByteNum == BOOLBYTENUM) {
            ...
            attenMaskUb.SetSize(this->singleProcessSOuterSize *
                                this->maskCopyInCol);
            LocalTensor<uint8_t> selectSpace =
                selectSpaceUb.Get<uint8_t>(selectSpaceUbSize);
            mmOutputType scalar;
            ...
            selectWithBytesMaskShapeInfo.firstAxis =
                this->singleProcessSOuterSize;
            selectWithBytesMaskShapeInfo.srcLastAxis =
                this->singleProcessSInnerSizeNow;
            selectWithBytesMaskShapeInfo.maskLastAxis = this->maskCopyInCol;
            if (type == 0) {
                SelectWithBytesMask(
                    mmResUb, mmResUb, static_cast<mmOutputType>(scalar),
                    attenMaskUb, selectSpace, selectWithBytesMaskShapeInfo);
            } else if (type == 1) {
                SelectWithBytesMask(
                    mmResUb, static_cast<mmOutputType>(scalar), mmResUb,
                    attenMaskUb, selectSpace,
            ...
            LocalTensor<mmOutputType> attenMaskUb =
                attenMaskQueue.DeQue<mmOutputType>();
            Muls(attenMaskUb, attenMaskUb, static_cast<mmOutputType>(-10000.0),
                 computeSize);
            pipe_barrier(PIPE_V);
...
template <typename T, typename U, CubeFormat FORMAT, typename O, Mode M>
__aicore__ inline void
JittorInferFlashAttentionBase<T, U, FORMAT, O, M>::SoftmaxBasicComputeFirst(
    LocalTensor<mmOutputType>& mmResUb, LocalTensor<float>& softmaxMaxUb,
    LocalTensor<float>& softmaxSumUb, SoftMaxShapeInfo& shapeInfo) {
    SoftMax<softmaxType, true, true>(mmResUb, softmaxSumUb, softmaxMaxUb,
                                     mmResUb, softmaxTilingData, shapeInfo);
}

template <typename T, typename U, CubeFormat FORMAT, typename O, Mode M>
__aicore__ inline void
JittorInferFlashAttentionBase<T, U, FORMAT, O, M>::SoftmaxComputeFirst(
    LocalTensor<mmOutputType>& mmResUb, LocalTensor<float>& softmaxMaxUb,
    LocalTensor<float>& softmaxSumUb, SoftMaxShapeInfo& shapeInfo) {
    SoftMax<softmaxType, true>(mmResUb, softmaxSumUb, softmaxMaxUb, mmResUb,
                               softmaxTilingData, shapeInfo);
}

template <typename T, typename U, CubeFormat FORMAT, typename O, Mode M>
__aicore__ inline bool
JittorInferFlashAttentionBase<T, U, FORMAT, O, M>::IsSoftmaxBasic() {
    return ((this->softmaxTilingData.splitM % 8 == 0) &&
            (this->softmaxTilingData.splitK % 64 == 0));
}

template <typename T, typename U, CubeFormat FORMAT, typename O, Mode M>
__aicore__ inline void
JittorInferFlashAttentionBase<T, U, FORMAT, O, M>::SoftmaxBasicCompute(
    LocalTensor<mmOutputType>& mmResUb, LocalTensor<float>& softmaxMaxUb,
    LocalTensor<float>& softmaxSumUb, LocalTensor<softmaxType>& softmaxExpUb,
    SoftMaxShapeInfo& shapeInfo) {
    SoftmaxFlash<softmaxType, true, true>(
        mmResUb, softmaxSumUb, softmaxMaxUb, mmResUb, softmaxExpUb,
        softmaxSumUb, softmaxMaxUb, softmaxFlashTilingData, true, shapeInfo);
}

template <typename T, typename U, CubeFormat FORMAT, typename O, Mode M>
__aicore__ inline void
JittorInferFlashAttentionBase<T, U, FORMAT, O, M>::SoftmaxCompute(
    LocalTensor<mmOutputType>& mmResUb, LocalTensor<float>& softmaxMaxUb,
    LocalTensor<float>& softmaxSumUb, LocalTensor<softmaxType>& softmaxExpUb,
    SoftMaxShapeInfo& shapeInfo) {
    SoftmaxFlash<softmaxType, true>(
        mmResUb, softmaxSumUb, softmaxMaxUb, mmResUb, softmaxExpUb,
        softmaxSumUb, softmaxMaxUb, softmaxFlashTilingData, true, shapeInfo);
}

template <typename T, typename U, CubeFormat FORMAT, typename O, Mode M>
__aicore__ inline void JittorInferFlashAttentionBase<T, U, FORMAT, O, M>::
    SoftmaxBasicComputeFirstNoTail(LocalTensor<mmOutputType>& mmResUb,
                                   LocalTensor<float>& softmaxMaxUb,
                                   LocalTensor<float>& softmaxSumUb,
                                   LocalTensor<softmaxType>& softmaxExpUb,
                                   LocalTensor<uint8_t>& sharedTmpUb,
                                   SoftMaxShapeInfo& shapeInfo) {
    SoftmaxFlashV2<softmaxType, false, true, true, true>(
        mmResUb, softmaxSumUb, softmaxMaxUb, mmResUb, softmaxExpUb,
...
template <typename T, typename U, CubeFormat FORMAT, typename O, Mode M>
__aicore__ inline void
JittorInferFlashAttentionBase<T, U, FORMAT, O, M>::SoftmaxBasicComputeNoTail(
    LocalTensor<mmOutputType>& mmResUb, LocalTensor<float>& softmaxMaxUb,
    LocalTensor<float>& softmaxSumUb, LocalTensor<softmaxType>& softmaxExpUb,
    LocalTensor<uint8_t>& sharedTmpUb, SoftMaxShapeInfo& shapeInfo) {
    SoftmaxFlashV2<softmaxType, true, true, true, true>(
        mmResUb, softmaxSumUb, softmaxMaxUb, mmResUb, softmaxExpUb,
        softmaxSumUb, softmaxMaxUb, sharedTmpUb, softmaxFlashTilingData,
        shapeInfo);
}
...
__aicore__ inline void JittorInferFlashAttentionBase<T, U, FORMAT, O, M>::
    SoftmaxBasicComputeFirstNoTail(LocalTensor<mmOutputType>& mmResUb,
                                   ...
                                   LocalTensor<softmaxType>& softmaxExpUb,
                                   SoftMaxShapeInfo& shapeInfo) {
    SoftmaxFlashV2<softmaxType, false, true, true>(
        mmResUb, softmaxSumUb, softmaxMaxUb, mmResUb, softmaxExpUb,
...
JittorInferFlashAttentionBase<T, U, FORMAT, O, M>::SoftmaxBasicComputeNoTail(
    ...
    LocalTensor<float>& softmaxSumUb, LocalTensor<softmaxType>& softmaxExpUb,
    SoftMaxShapeInfo& shapeInfo) {
    SoftmaxFlashV2<softmaxType, true, true, true>(
        mmResUb, softmaxSumUb, softmaxMaxUb, mmResUb, softmaxExpUb,
        softmaxSumUb, softmaxMaxUb, softmaxFlashTilingData, shapeInfo);
}
...
template <typename T, typename U, CubeFormat FORMAT, typename O, Mode M>
__aicore__ inline void
JittorInferFlashAttentionBase<T, U, FORMAT, O, M>::SoftmaxComputeFirstTail(
    LocalTensor<mmOutputType>& mmResUb, LocalTensor<float>& softmaxMaxUb,
    LocalTensor<float>& softmaxSumUb, LocalTensor<softmaxType>& softmaxExpUb,
    SoftMaxShapeInfo& shapeInfo) {
    SoftmaxFlashV2<softmaxType, false, true, false>(
        mmResUb, softmaxSumUb, softmaxMaxUb, mmResUb, softmaxExpUb,
        softmaxSumUb, softmaxMaxUb, softmaxFlashTilingData, shapeInfo);
}
...
template <typename T, typename U, CubeFormat FORMAT, typename O, Mode M>
__aicore__ inline void
JittorInferFlashAttentionBase<T, U, FORMAT, O, M>::SoftmaxComputeTail(
    LocalTensor<mmOutputType>& mmResUb, LocalTensor<float>& softmaxMaxUb,
    LocalTensor<float>& softmaxSumUb, LocalTensor<softmaxType>& softmaxExpUb,
    SoftMaxShapeInfo& shapeInfo) {
    SoftmaxFlashV2<softmaxType, true, true, false>(
        mmResUb, softmaxSumUb, softmaxMaxUb, mmResUb, softmaxExpUb,
        softmaxSumUb, softmaxMaxUb, softmaxFlashTilingData, shapeInfo);
}
...
template <typename T, typename U, CubeFormat FORMAT, typename O, Mode M>
__aicore__ inline bool
JittorInferFlashAttentionBase<T, U, FORMAT, O, M>::IsSoftmaxFlashBasic() {
    return ((this->softmaxFlashTilingData.splitM % 8 == 0) &&
            (this->softmaxFlashTilingData.splitK % 64 == 0));
}

// quant: add quant functions
template <typename T, typename U, CubeFormat FORMAT, typename O, Mode M>
__aicore__ inline void
JittorInferFlashAttentionBase<T, U, FORMAT, O, M>::QuantCompute(
...
template <typename T, typename U, CubeFormat FORMAT, typename O, Mode M>
__aicore__ inline void
JittorInferFlashAttentionBase<T, U, FORMAT, O, M>::Bmm2UpdateDivNoTail(
    LocalTensor<mmOutputType>& bmm2ResPreUb, LocalTensor<float>& softmaxSumUb,
    LocalTensor<softmaxType>& softmaxExpUb) {
    ...
    if constexpr (IsSameType<softmaxType, half>::value) {
        ...
        LocalTensor<float> tmpBuffer =
            attenMaskQueue.template AllocTensor<float>();
        ...
        for (int i = 0; i < loop; i++) {
            Div(bmm2ResPreUb[i * REPEAT_DATA_NUM],
                bmm2ResPreUb[i * REPEAT_DATA_NUM], softmaxExpUb,
                REPEAT_DATA_NUM, singleProcessSOuterSize, repeatParams);
        }
        if (remain) {
            Div(bmm2ResPreUb[loop * REPEAT_DATA_NUM],
                bmm2ResPreUb[loop * REPEAT_DATA_NUM], softmaxExpUb, remain,
...
template <typename T, typename U, CubeFormat FORMAT, typename O, Mode M>
__aicore__ inline void
JittorInferFlashAttentionBase<T, U, FORMAT, O, M>::Bmm2Compute(
    ...
    if constexpr (IsSameType<mmInputType, bfloat16_t>::value ||
        ...
        pipe_barrier(PIPE_V);
        LocalTensor<mmInputType>
            tmpBmm2ResCastTensor;  // The same ub buffer is used before and
                                   // after the cast.
        tmpBmm2ResCastTensor =
            bmm2ResL1.template ReinterpretCast<mmInputType>();
        tmpBmm2ResCastTensor.SetSize(bmm2ResL1.GetSize());
        Cast(tmpBmm2ResCastTensor, bmm2ResL1, RoundMode::CAST_ROUND,
             bmm2ResL1.GetSize());
...
template <typename T, typename U, CubeFormat FORMAT, typename O, Mode M>
__aicore__ inline void
JittorInferFlashAttentionBase<T, U, FORMAT, O, M>::UpdateVmul(
    LocalTensor<softmaxType>& softmaxExpUb) {
    LocalTensor<mmOutputType> bmm2ResPreUb =
        tempBmm2Ub.Get<mmOutputType>(bmm2ResUbSize);
    ...
    for (int i = 0; i < loop; i++) {
        Mul(bmm2ResPreUb[i * numOneRep], softmaxExpUb,
            bmm2ResPreUb[i * numOneRep], numOneRep, singleProcessSOuterSize,
            repeatParams);
    }
    if (remain) {
        Mul(bmm2ResPreUb[loop * numOneRep], softmaxExpUb,
            bmm2ResPreUb[loop * numOneRep], remain, singleProcessSOuterSize,
            repeatParams);
    }
}
...
template <typename T, typename U, CubeFormat FORMAT, typename O, Mode M>
__aicore__ inline void
JittorInferFlashAttentionBase<T, U, FORMAT, O, M>::ComputePseShiftOffset(
...
template <typename T, typename U, CubeFormat FORMAT, typename O, Mode M>
__aicore__ inline void
JittorInferFlashAttentionBase<T, U, FORMAT, O, M>::ComputeAttenMaskOffset(
    int sInnerOffsetDataSize) {
    int32_t delta;
    if (attentionMaskType == 2 || attentionMaskType == 3 ||
        ...
        if (attentionMaskType == 2) {
            delta = sOuterOffset - sInnerOffsetDataSize +
        ...
        if (delta < 0) {
            ...
            attenMaskOffset = (((int32_t)singleProcessSInnerSize - delta) > 0
                                   ? delta
...
template <typename T, typename U, CubeFormat FORMAT, typename O, Mode M>
__aicore__ inline void
JittorInferFlashAttentionBase<T, U, FORMAT, O, M>::ComputeAttenMaskOffsetPre(
    int sInnerOffsetDataSize) {
    ...
    if (delta < 0) {
        attenMaskOffsetPre = ((int32_t)singleProcessSOuterSizeWhole + delta) > 0
                                 ? (-delta)
                                 : singleProcessSOuterSizeWhole;
    } else {
        attenMaskOffsetPre = (((int32_t)singleProcessSInnerSize - delta) > 0
                                  ? delta
                                  : singleProcessSInnerSize) *
                             attentionMaskStride;
    }
...
template <typename T, typename U, CubeFormat FORMAT, typename O, Mode M>
__aicore__ inline void
JittorInferFlashAttentionBase<T, U, FORMAT, O, M>::ComputeOffset(
...
template <typename T, typename U, CubeFormat FORMAT, typename O, Mode M>
__aicore__ inline void
JittorInferFlashAttentionBase<T, U, FORMAT, O, M>::ComputeOffsetWithBNSD(
    uint32_t sInnerLoopIdx) {
    int sInnerOffsetDataSize = sInnerLoopIdx * singleProcessSInnerSize;
    ComputePseShiftOffset(sInnerOffsetDataSize);
    ComputeAttenMaskOffset(sInnerOffsetDataSize);
...
template <typename T, typename U, CubeFormat FORMAT, typename O, Mode M>
__aicore__ inline void
JittorInferFlashAttentionBase<T, U, FORMAT, O, M>::DataCopyTransposeOut(
    LocalTensor<mmOutputType>& bmm2ResUb) {
    ...
    if (preTokensPerBatch < 0) {
        ...
        if (sOuterOffset < preTokenLength &&
            (sOuterOffset + this->singleProcessSOuterSize) > preTokenLength) {
            preTokensOffset =
                sOuterOffset + this->singleProcessSOuterSize - preTokenLength;
        } else {
            preTokensOffset = 0;
    ...
    transposeTilingData22.srcShapeS =
        singleProcessSOuterSize - preTokensOffset - nextTokensOffset;
    transposeTilingData22.invalidParamCopyTransposeTiling = 0;
    transposeParams.sIndex = transposeParams.sIndex + nextTokensOffset;

    if constexpr (IsSameType<O, half>::value &&
                  IsSameType<mmOutputType, half>::value) {
        ...
        if (tilingData->promptAttentionBaseParams.headSize ==
            tilingData->promptAttentionBaseParams.alignedHeadSize) {
            ...
            intriParams.blockLen =
                tilingData->promptAttentionBaseParams.headSize *
                sizeof(O);  // This should be unaligned
            intriParams.srcStride = 0;
            intriParams.dstStride = 0;
            int startAddr =
                multiSeqOffset +
                transposeParams.nIndex * transposeTilingData22.dstShapeHN +
                (transposeParams.sIndex + nextTokensOffset) *
                    transposeTilingData22.dstShapeH +
                transposeParams.hNIndex;
            for (int i = 0; i < transposeTilingData22.srcShapeB; i++) {
                for (int j = 0; j < transposeTilingData22.srcShapeN; j++) {
                    for (int k = nextTokensOffset;
                         k < transposeTilingData22.srcShapeS; k++) {
                        DataCopyPad(
                            attentionOutGm
                                [startAddr +
                                 i * (transposeTilingData22.shapeSHValue) +
                                 j * transposeTilingData22.dstShapeHN +
                                 k * transposeTilingData22.dstShapeH],
                            bmm2ResUb[k * tilingData->promptAttentionBaseParams
                                              .alignedHeadSize +
    ...
    if constexpr (IsSameType<O, bfloat16_t>::value ||
        ...
        tmpBmm2ResCastTensor.SetSize(
            bmm2ResUb.GetSize());  // Set the size of the converted
        pipe_barrier(PIPE_V);
        if (tilingData->promptAttentionBaseParams.headSize ==
            tilingData->promptAttentionBaseParams.alignedHeadSize) {
            Cast(tmpBmm2ResCastTensor, bmm2ResUb, RoundMode::CAST_ROUND,
                 bmm2ResUb.GetSize());
            event_t enQueEvtID = static_cast<event_t>(
                GetTPipePtr()->FetchEventID(HardEvent::V_MTE3));
            SetFlag<HardEvent::V_MTE3>(enQueEvtID);
            ...
            DataCopyTranspose2<O>(
                attentionOutGm,
                tmpBmm2ResCastTensor[nextTokensOffset *
            ...
            int32_t loop = tilingData->promptAttentionBaseParams.headSize /
                           REPEAT_DATA_NUM;
            int32_t remain = tilingData->promptAttentionBaseParams.headSize %
                             REPEAT_DATA_NUM;
            UnaryRepeatParams repeatParams;
            repeatParams.srcBlkStride = 1;
            repeatParams.srcRepStride =
                (tilingData->promptAttentionBaseParams.headSize * 4 + 32 - 1) /
                32;
            repeatParams.dstBlkStride = 1;
            repeatParams.dstRepStride =
                (tilingData->promptAttentionBaseParams.headSize * 2 + 32 - 1) /
                32;
            for (int i = 0; i < loop; i++) {
                Cast(tmpBmm2ResCastTensor[i * REPEAT_DATA_NUM],
                     bmm2ResUb[i * REPEAT_DATA_NUM], RoundMode::CAST_ROUND,
                     REPEAT_DATA_NUM, this->singleProcessSOuterSize,
                     repeatParams);
            }
            if (remain) {
                Cast(tmpBmm2ResCastTensor[loop * REPEAT_DATA_NUM],
                     bmm2ResUb[loop * REPEAT_DATA_NUM], RoundMode::CAST_ROUND,
                     remain, this->singleProcessSOuterSize, repeatParams);
            }
            event_t enQueEvtID = static_cast<event_t>(
                GetTPipePtr()->FetchEventID(HardEvent::V_MTE3));
            SetFlag<HardEvent::V_MTE3>(enQueEvtID);
                        ...
                        DataCopyPad(
                            attentionOutGm
                                [startAddr +
                                 ...
                                 k * transposeTilingData22.dstShapeH],
                            tmpBmm2ResCastTensor
                                [k * tilingData->promptAttentionBaseParams
                                         .alignedHeadSize +
                                 j * transposeTilingData22.shapeNsValue +
                                 i * transposeTilingData22.shapeNsnValue],
                            intriParams);
    ...
    if constexpr (IsSameType<O, int8_t>::value) {
        ...
        QuantCompute(outputQuantRes, bmm2ResUb, quantScale2, quantOffset2,
                     bmm2ResUbSize);
        // Quantifying vector computation and interpolating synchronization
        // between datacopy to ensure timing
        event_t enQueEvtID = static_cast<event_t>(
            GetTPipePtr()->FetchEventID(HardEvent::V_MTE3));
        SetFlag<HardEvent::V_MTE3>(enQueEvtID);
        WaitFlag<HardEvent::V_MTE3>(enQueEvtID);
...
template <typename T, typename U, CubeFormat FORMAT, typename O, Mode M>
__aicore__ inline void
JittorInferFlashAttentionBase<T, U, FORMAT, O, M>::DataCopyOutWithBNSD(
    LocalTensor<mmOutputType>& bmm2ResUb) {
    ...
    if constexpr (IsSameType<O, half>::value &&
        ...
        if (tilingData->promptAttentionBaseParams.headSize ==
            ...
            dataCopyParams.blockCount =
                this->singleProcessSOuterSize - preTokensOffset;
            ...
            dataCopyParams.dstStride = 0;
            DataCopyPad(
                attentionOutGm[attentionOutOffset],
                bmm2ResUb[nextTokensOffset *
                          tilingData->promptAttentionBaseParams.headSize],
                dataCopyParams);
        }
    ...
    if constexpr (IsSameType<O, bfloat16_t>::value ||
                  (IsSameType<O, half>::value &&
                   IsSameType<mmOutputType, float>::value)) {
        LocalTensor<O> tmpBmm2ResCastTensor;  // The same ub buffer is used
    ...
    if constexpr (IsSameType<O, int8_t>::value) {
        ...
        QuantCompute(outputQuantRes, bmm2ResUb, quantScale2, quantOffset2,
                     bmm2ResUbSize);
        // Insert synchronization between quantization calculation and datacopy
...
template <typename T, typename U, CubeFormat FORMAT, typename O, Mode M>
__aicore__ inline void
JittorInferFlashAttentionBase<T, U, FORMAT, O, M>::CalPseShiftOffset(int sIdx) {
    if (!usePseShift) {
        return;
    }
    ...
    pseShiftCoreOffset = pseShiftBatchOffset +
                         (uint64_t)batchNOffset * pseShiftS1 * pseShiftS2 +
                         (uint64_t)sOuterOffset * pseShiftS2;
}

template <typename T, typename U, CubeFormat FORMAT, typename O, Mode M>
__aicore__ inline void
JittorInferFlashAttentionBase<T, U, FORMAT, O, M>::LoopSOuterOffsetInit(
    uint32_t seqListOffsetSize, int sIdx) {
    ...
    tensorACoreOffset =
        seqListOffsetSize + sOuterOffset * MultiHeadQ +
    ...
    uint32_t seqInnerOffsetSize =
        tilingData->promptAttentionBaseParams.seqSize ==
            ...
            : sIdx * tilingData->promptAttentionBaseParams.seqInnerSize *
                  MultiHeadKV;
    tensorBCoreOffset =
        seqInnerOffsetSize + batchNOffset / headNumRatio *
                                 tilingData->promptAttentionBaseParams.headSize;
...
template <typename T, typename U, CubeFormat FORMAT, typename O, Mode M>
__aicore__ inline void
JittorInferFlashAttentionBase<T, U, FORMAT, O, M>::LoopSOuterOffsetInitWithBNSD(
    uint32_t seqListOffsetSize, int sIdx) {
    uint64_t attenMaskBatchOffset = 0;
    if (attenMaskBatch != 1) {
        attenMaskBatchOffset =
    ...
    tensorACoreOffset = seqListOffsetSize + batchNOffset * head_stride_q +
                        sOuterOffset * seq_stride;
    ...
    tensorBCoreOffset =
        seqInnerOffsetSize + batchNOffset / headNumRatio * head_stride_kv;

    valueCoreOffset = tensorBCoreOffset;

    attentionOutOffset = seqListOffsetSize + batchNOffset * head_stride_q +
                         (sOuterOffset + nextTokensOffset) * seq_stride;
}

template <typename T, typename U, CubeFormat FORMAT, typename O, Mode M>
__aicore__ inline void
JittorInferFlashAttentionBase<T, U, FORMAT, O, M>::Bmm2UpdateAdd(
    LocalTensor<mmOutputType>& bmm2ResUb) {
    LocalTensor<mmOutputType> bmm2ResPreUb =
        tempBmm2Ub.Get<mmOutputType>(bmm2ResUbSize);
...
template <typename T, typename U, CubeFormat FORMAT, typename O, Mode M>
__aicore__ inline void
JittorInferFlashAttentionBase<T, U, FORMAT, O, M>::GetSingleCoreParam(
    int sIdx) {
    actualSeqLengthPerBatch =
        isActualLenDimsNull ? tilingData->promptAttentionBaseParams.seqSize
                            : actualSeqLengthsGm.GetValue(sIdx);
    actualSeqLengthKVPerBatch =
        isActualLenDimsKVNull
            ? tilingData->promptAttentionBaseParams.seqInnerSize
    ...
    unalignSInner = (actualSeqLengthKVPerBatch % singleProcessSInnerSize != 0)
                        ...
                        : singleProcessSInnerSize;
    maxInnerLoopTimes =
        (actualSeqLengthKVPerBatch + singleProcessSInnerSize - 1) /
        singleProcessSInnerSize;
    singleProcessSInnerSizeTail =
        (unalignSInner + typeByteNum - 1) / typeByteNum * typeByteNum;
    maskInnerTailAlign = (unalignSInner + maskTypeByteNum - 1) /
                         maskTypeByteNum * maskTypeByteNum;
    padSize = maskInnerTailAlign - unalignSInner;

    pseShiftInnerTailAlign = (unalignSInner + pseShiftTypeByteNum - 1) /
...
template <typename T, typename U, CubeFormat FORMAT, typename O, Mode M>
__aicore__ inline void
JittorInferFlashAttentionBase<T, U, FORMAT, O, M>::GetSparseParam(
    ...
    if (attentionMaskType == 4) {
        *preTokens = (int32_t)tilingData->promptAttentionBaseParams.preTokens -
                     actualSeqLengthKVPerBatch + actualSeqLengthPerBatch;
        *nextTokens =
            (int32_t)tilingData->promptAttentionBaseParams.nextTokens +
            actualSeqLengthKVPerBatch - actualSeqLengthPerBatch;
    }
...
template <typename T, typename U, CubeFormat FORMAT, typename O, Mode M>
__aicore__ inline void
JittorInferFlashAttentionBase<T, U, FORMAT, O, M>::ComputeTokenOffset() {
    if (sOuterOffset < nextTokensPerBatch * (-1) &&
        (sOuterOffset + this->singleProcessSOuterSize) >
            nextTokensPerBatch * (-1)) {
...
#endif  // JITTOR_INFER_FLASH_ATTENTION_BASE_H