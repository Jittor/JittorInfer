...
constexpr static int32_t repeatMaxSize =
    repeatMaxBytes / 4;  // 4 means sizeof(T)
...
__aicore__ inline void BoolCopyIn(LocalTensor<uint8_t> &dstTensor,
                                  GlobalTensor<uint8_t> &srcTensor,
                                  int64_t srcOffset, uint32_t s1Size,
                                  uint32_t s2Size, int64_t totalS2Size) {
    ...
    if (totalS2Size % blockBytes == 0) {
        ...
        DataCopyPad(dstTensor, srcTensor[srcOffset], dataCopyParams,
                    dataCopyPadParams);
    }
}
...
__aicore__ inline void Bit2Int8CopyIn(LocalTensor<uint8_t> &dstTensor,
                                      GlobalTensor<uint8_t> &srcTensor,
                                      int64_t srcOffset, uint32_t batchSize,
                                      uint32_t s1BaseSize, uint32_t s2BaseSize,
                                      int64_t s2TotalSize) {
    uint32_t alignedS2Size =
        CeilDiv(s2BaseSize / byteBitRatio, blockBytes) * blockBytes;
    ...
    if (s2TotalSize / byteBitRatio % blockBytes == 0 &&
        s2BaseSize / byteBitRatio % blockBytes == 0) {
        dataCopyParams.srcStride = (s2TotalSize / byteBitRatio -
                                    dataCopyParams.blockLen * blockBytes) /
                                   blockBytes;
        DataCopy(dstTensor, srcTensor[srcOffset / byteBitRatio],
                 dataCopyParams);
    } else {
        ...
        dataCopyPadParams.paddingValue = 0;
        DataCopyPad(dstTensor, srcTensor[srcOffset / byteBitRatio],
