...
struct PseInfo {
    ...
    uint32_t pseType;        // 0: outer, mul-add   1:outer, add-mul   2:inner,
                             // mul-add   3:inner, mul-add-sqrt
...
__aicore__ inline void DataCopyInCommon(LocalTensor<INPUT_T> &dstTensor,
                                        ...
                                        int64_t offset, int64_t s1Size,
                                        int64_t s2Size, int64_t actualS2Len,
                                        int32_t dtypeSize,
    ...
    if constexpr (hasPse == true) {
        ...
        dataCopyParams.blockLen =
            CeilDiv(s2Size * dtypeSize, blockBytes);  // 单位32B
        dataCopyParams.dstStride = 0;                 // gap
        if (actualS2Len * 2 % blockBytes == 0) {
            dataCopyParams.srcStride = (actualS2Len * dtypeSize -
            ...
            dataCopyParams.srcStride =
                (actualS2Len * dtypeSize - dataCopyParams.blockLen);
            ...
            DataCopyPad(dstTensor, srcTensor[offset], dataCopyParams,
                        dataCopyPadParams);
        }
    }
}
...
template <typename INPUT_T, bool hasPse>
__aicore__ inline void DataCopyIn(LocalTensor<INPUT_T> &dstTensor,
                                  GlobalTensor<INPUT_T> &srcTensor,
                                  int64_t offset, int64_t s1Size,
                                  int64_t s2Size, int64_t actualS2Len) {
    if constexpr (hasPse == true) {
        ...
        DataCopyInCommon<INPUT_T, hasPse>(dstTensor, srcTensor, offset, s1Size,
                                          s2Size, actualS2Len, dtypeSize,
                                          alignedS2Size);
    }
}
...
__aicore__ inline void DataCopyInAlign8(LocalTensor<INPUT_T> &dstTensor,
    ...
    if constexpr (hasPse == true) {
        ...
        int32_t alignedS2Size =
            CeilDiv(s2Size, 32 / dtypeSize) * (32 / dtypeSize);
        DataCopyInCommon<INPUT_T, hasPse>(dstTensor, srcTensor, offset, s1Size,
                                          s2Size, actualS2Len, dtypeSize,
                                          alignedS2Size);
    }
...
__aicore__ inline void BroadcastAdd(const LocalTensor<T> &src0Tensor,
                                    ...
                                    int64_t src0Offset, int32_t src1Size,
                                    int32_t repeatTimes) {
    if constexpr (hasPse == true) {
        ...
        for (int32_t j = 0; j < innerLoop; j++) {
            ...
            Add(src0Tensor[ubOffset], src0Tensor[ubOffset],
                src1Tensor[innerOffset], repeatMaxSize, repeatTimes,
                binaryRepeatParams);
        }
        if (innerRemain > 0) {
            ...
            auto ubOffset = src0Offset + innerOffset;
            Add(src0Tensor[ubOffset], src0Tensor[ubOffset],
                src1Tensor[innerOffset], innerRemain, repeatTimes,
                binaryRepeatParams);
        }
    }
}
...
template <typename T, bool hasPse>
__aicore__ inline void PseBroadcastAdd(int32_t s1Size, int32_t s2Size,
                                       int32_t computeSize,
                                       const LocalTensor<T> &pseUb,
                                       const LocalTensor<T> &dstTensor,
                                       uint32_t pseShapeType) {
    if constexpr (hasPse == true) {
        if (pseShapeType == pseS1S2 || pseShapeType == pseSlopeBn ||
            pseShapeType == pseSlopeN) {
            ...
            for (int32_t s1OuterIdx = 0; s1OuterIdx < s1OuterLoop;
                 s1OuterIdx++) {
                int32_t s1OuterOffset = s1OuterIdx * repeatMaxTimes * s2Size;
                BroadcastAdd<T, hasPse>(dstTensor, pseUb, s1OuterOffset, s2Size,
                                        repeatMaxTimes);
            }
            if (s1OuterRemain > 0) {
...
__aicore__ inline int64_t PseComputeOffset(PseInfo &pseInfo) {
    if constexpr (hasPse == true) {
        ...
        if (pseInfo.pseShapeType == pseS1S2) {
            ...
            n2Offset = pseInfo.n2oIdx * pseInfo.gSize * pseInfo.s1Size *
                       pseInfo.s2Size;
            gOffset = pseInfo.goIdx * pseInfo.s1Size * pseInfo.s2Size;
            s1Offset =
                pseInfo.s1oIdx * pseInfo.s1BaseSize * pseInfo.s2Size +
...
__aicore__ inline int64_t PseAlibiComputeOffset(PseInfo &pseInfo) {
    if constexpr (hasPse == true) {
        int64_t bOffset = (pseInfo.boIdx % pseInfo.pseBSize) * pseInfo.n2G *
                          pseInfo.pseS2Size * pseInfo.pseS1Size;
        int64_t n2Offset = pseInfo.n2oIdx * pseInfo.gSize * pseInfo.pseS2Size *
                           pseInfo.pseS1Size;
        int64_t gOffset = pseInfo.goIdx * pseInfo.pseS2Size * pseInfo.pseS1Size;
        int64_t row = pseInfo.s1oIdx * pseInfo.s1BaseSize +
        ...
        if constexpr (layOutType != LayOutTypeEnum::LAYOUT_TND) {
            ...
            if (row >= threshold) {
                ...
                k = pseInfo.pseS2Size - (row - column) -
                    (pseInfo.pseS1Size - m);
            }
        } else {
...
__aicore__ inline bool NeedPseAlibiCompute(PseInfo &pseInfo) {
    if constexpr (hasPse == true) {
        // Alibi编码只计算下三角
        if (pseInfo.s1oIdx * pseInfo.s1BaseSize +
                (pseInfo.loopIdx + 1) * pseInfo.vec1S1BaseSize <=
            pseInfo.s2StartIdx +
                pseInfo.s2LoopCount * pseInfo.s2BaseNratioSize) {
            return false;
        }
        return true;
    } else {
...
__aicore__ inline void PseAlibiCopyIn(LocalTensor<T> &dstTensor,
                                      LocalTensor<INPUT_T> &tmpTensor,
                                      GlobalTensor<INPUT_T> &srcTensor,
                                      PseInfo &pseInfo) {
    if constexpr (hasPse == true) {
        ...
        if constexpr (IsSameType<INPUT_T, T>::value) {
            if (!pseInfo.align8) {
                DataCopyIn<INPUT_T, hasPse>(
                    dstTensor, srcTensor, offset, pseInfo.vec1S1RealSize,
        ...
        DataCopyIn<INPUT_T, hasPse>(tmpTensor, srcTensor, offset,
...
template <typename T, bool hasPse>
__aicore__ inline void PseSlopeCopyIn(LocalTensor<T> &dstTensor,
                                      LocalTensor<half> &helpTensor,
                                      __gm__ uint8_t *pseSlope,
                                      GlobalTensor<half> &alibiGm,
                                      PseInfo &pseInfo) {
    if constexpr (hasPse == true) {
        int64_t bOffset = 0;
        ...
        if (pseInfo.needCast) {
            ...
            int64_t s1Offset = pseInfo.s1oIdx * pseInfo.s1BaseSize +
                               pseInfo.loopIdx * pseInfo.vec1S1BaseSize;
            int64_t s2Offset = pseInfo.s2StartIdx +
                               pseInfo.s2LoopCount * pseInfo.s2BaseNratioSize;

            float posShift = float(s2Offset + pseInfo.kvStartIdx - s1Offset -
                                   pseInfo.qStartIdx);
            ...
            if (pseInfo.pseType ==
                (uint32_t)PseTypeEnum::PSE_INNER_MUL_ADD_SQRT_TYPE) {
...
template <typename T, bool hasPse>
__aicore__ inline void PseSlopeCast(LocalTensor<T> &dstTensor,
                                    LocalTensor<half> &helpTensor,
                                    __gm__ uint8_t *pseSlope,
                                    PseInfo &pseInfo) {
    if constexpr (hasPse == true) {
        ...
        float posShift =
            float(s2Offset + pseInfo.kvStartIdx - s1Offset - pseInfo.qStartIdx);
...
template <typename INPUT_T, typename T, LayOutTypeEnum layOutType, bool hasPse>
__aicore__ inline void PseCopyIn(LocalTensor<T> &dstTensor,
                                 LocalTensor<INPUT_T> &tmpTensor,
                                 GlobalTensor<INPUT_T> &srcTensor,
                                 PseInfo &pseInfo) {
    if constexpr (hasPse == true) {
        if (pseInfo.pseEncodeType == pseEncodeALibiS2Full) {
            return PseAlibiCopyIn<INPUT_T, T, layOutType, hasPse>(
                dstTensor, tmpTensor, srcTensor, pseInfo);
        }
        int64_t offset = PseComputeOffset<hasPse>(pseInfo);
        int64_t s1Size =
            pseInfo.pseShapeType == pse1S2
                ? (pseInfo.blockCount == 0 ? 1 : pseInfo.blockCount)
                : pseInfo.vec1S1RealSize;

        if constexpr (IsSameType<INPUT_T, T>::value) {
            if (!pseInfo.align8) {
                DataCopyIn<INPUT_T, hasPse>(dstTensor, srcTensor, offset,
                                            s1Size, pseInfo.s2RealSize,
                                            pseInfo.s2Size);
            } else {
        ...
        DataCopyIn<INPUT_T, hasPse>(tmpTensor, srcTensor, offset, s1Size,
...
template <typename T, bool hasPse>
__aicore__ inline void PseAlibiCompute(LocalTensor<T> &dstTensor,
                                       LocalTensor<T> &pseTensor,
                                       PseInfo &pseInfo) {
    if constexpr (hasPse == true) {
        if (!NeedPseAlibiCompute<hasPse>(pseInfo)) {
            return;
        }
        Add(dstTensor, dstTensor, pseTensor,
            pseInfo.vec1S1RealSize * pseInfo.pseS2ComputeSize);
        return;
    }
}
...
template <typename T, bool hasPse>
__aicore__ inline void PseCompute(LocalTensor<T> &dstTensor,
                                  LocalTensor<T> &pseTensor, PseInfo &pseInfo) {
    if constexpr (hasPse == true) {
        if (pseInfo.pseEncodeType == pseEncodeALibiS2Full) {
            return PseAlibiCompute<T, hasPse>(dstTensor, pseTensor, pseInfo);
        }
        int64_t computeSize =
            (pseInfo.pseShapeType == pseS1S2 ||
             pseInfo.pseShapeType == pseSlopeBn ||
             pseInfo.pseShapeType == pseSlopeN)
                ? pseInfo.vec1S1RealSize * pseInfo.s2AlignedSize
                : pseInfo.s2AlignedSize;
        PseBroadcastAdd<T, hasPse>(pseInfo.vec1S1RealSize,
                                   pseInfo.s2AlignedSize, computeSize,
...
__aicore__ inline void PseInnerAlibiCreate(GlobalTensor<half> &dstTensor,
    ...
    if constexpr (hasPse == true) {
        ...
        event_t eventIdMte3ToV = static_cast<event_t>(
        ...
        for (int64_t i = 0; i < pseInfo.pseAlibiBaseS1; i++) {
            CreateVecIndex(helpTensor, (half)(i * tmpValue),
                           pseInfo.pseAlibiBaseS2);
            SetFlag<HardEvent::V_MTE3>(eventIdVToMte3);
            WaitFlag<HardEvent::V_MTE3>(eventIdVToMte3);
            DataCopy(dstTensor[i * pseInfo.pseAlibiBaseS2], helpTensor,
                     pseInfo.pseAlibiBaseS2);
