...
struct DropMaskInfo {
    ...
    // 参数按B N G S1
    // S2全部切分设置进行偏移计算，没有切分的轴对应的参数设置为合适的0或者原始值
    int64_t n2G;              // n2 * g
    int64_t gSize;            // g
    int64_t s1Size;           // s1
    int64_t s2Size;           // s2
    ...
    int64_t
        s2BaseNratioSize;  // s2的配比长度: s2BaseSize(S2基本块大小) * nRatio
...
__aicore__ inline int64_t ComputeDropOffset(DropMaskInfo& dropMaskInfo) {
    if constexpr (hasDrop == true) {
        ...
        // n2oIdx * g * s1 *s2
        int64_t n2Offset = dropMaskInfo.n2OutIdx * dropMaskInfo.gSize *
                           dropMaskInfo.s1Size * dropMaskInfo.s2Size;
        // goIdx * s1 * s2
        int64_t gOffset =
            dropMaskInfo.gOutIdx * dropMaskInfo.s1Size * dropMaskInfo.s2Size;
        // s1oIdx * s1BaseSize * s2Size + s1innerindex * vec1S1BaseSize * s2Size
        int64_t s1Offset = dropMaskInfo.s1OutIdx * dropMaskInfo.s1BaseSize *
                               dropMaskInfo.s2Size +
                           dropMaskInfo.s1InnerIdx *
                               dropMaskInfo.splitS1BaseSize *
                               dropMaskInfo.s2Size;
        // s2StartIdx + s2index * s2BaseNratioSize
        int64_t s2Offset = dropMaskInfo.s2StartIdx +
                           dropMaskInfo.s2Idx * dropMaskInfo.s2BaseNratioSize;
    ...
    if constexpr (hasDrop == true) {
        ...
        if (unlikely(dropMaskInfo.boolMode)) {
            BoolCopyIn(dstTensor, srcBoolTensor, dropMaskOffset,
                       dropMaskInfo.s1CopySize, dropMaskInfo.s2CopySize,
...
__aicore__ inline void ComputeDropMask(LocalTensor<T>& dstTensor,
    ...
    if constexpr (hasDrop == true) {
        ...
        if (unlikely(dropMaskInfo.boolMode)) {
            ...
            DropOut(dstTensor, srcTensor, dropoutBuffer, tmpDropBuffer,
                    dropMaskInfo.keepProb, dropOutShapeInfo);
        } else {
            dropOutShapeInfo.maskLastAxis =
                CeilDiv(dropMaskInfo.maskLstAxis / byteBitRatio, blockBytes) *
                blockBytes;
            if (likely(dropMaskInfo.lstAxis / byteBitRatio % blockBytes == 0)) {
                ...
                DropOut<T, false, DROPOUT_MODE_BIT_MISALIGN>(
                    dstTensor, srcTensor, dropoutBuffer, tmpDropBuffer,
                    dropMaskInfo.keepProb, dropOutShapeInfo);
            }
        }
        return;
