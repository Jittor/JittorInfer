...
using _aclCreateTensor =
    aclTensor* (*)(const int64_t* view_dims, uint64_t view_dims_num,
...
using _aclCreateIntArray = aclIntArray* (*)(const int64_t* value,
                                            uint64_t size);
using _aclCreateFloatArray = aclFloatArray* (*)(const float* value,
                                                uint64_t size);
using _aclCreateBoolArray = aclBoolArray* (*)(const bool* value, uint64_t size);
using _aclCreateTensorList = aclTensorList* (*)(const aclTensor* const* value,
                                                uint64_t size);
...
inline void* GetOpApiFuncAddrInLib(void* handler, const char* libName,
    ...
    if (funcAddr == nullptr) {
        OPS_LOG_W("aclnnfallback", "dlsym %s from %s failed, error:%s.",
                  apiName, libName, dlerror());
    }
    return funcAddr;
}
...
inline void* GetOpApiLibHandler(const char* libName) {
    ...
    if (handler == nullptr) {
        OPS_LOG_W("aclnnfallback", "dlopen %s failed, error:%s.", libName,
                  dlerror());
    }
    return handler;
}
...
inline void* GetAclnnArrdByApiName(const char* apiName) {
    ...
    for (const auto& libName : libs) {
        ...
        if (libHandler != nullptr) {
            auto funcAddr =
                GetOpApiFuncAddrInLib(libHandler, libName.c_str(), apiName);
            if (funcAddr != nullptr) {
                return funcAddr;
            }
        }
...
inline aclTensor* ConvertType(const gert::Tensor* ge_tensor) {
    ...
    OPS_ERR_IF(aclCreateTensor == nullptr,
               OPS_LOG_E("aclnnfallback", "aclCreateTensor nullptr"),
               return nullptr);
    ...
    OPS_ERR_IF(out == nullptr, OPS_LOG_E("aclnnfallback", "out nullptr"),
               return nullptr);
...
inline aclTensorList* ConvertType(
    std::vector<const gert::Tensor*>& ge_tenserList) {
    OPS_ERR_IF(ge_tenserList.size() == 0,
               OPS_LOG_E("aclnnfallback", "ge_tenserList size 0"),
               return nullptr);
...
inline aclScalar* ConvertScalarType(T value) {
    ...
    OPS_ERR_IF(aclCreateScalar == nullptr,
               OPS_LOG_E("aclnnfallback", "aclCreateScalar nullptr"),
               return nullptr);
...
inline aclTensor* ConvertMmType(const gert::Tensor* ge_tensor, bool transpose,
    ...
    if (enable_NZ && GetPrimaryFormat(ge_tensor->GetStorageFormat()) ==
                         ge::Format::FORMAT_FRACTAL_NZ) {
        acl_format = aclFormat::ACL_FORMAT_FRACTAL_NZ;
    }
    aclTensor* out = aclCreateTensor(viewShape.data(), shape.size(), dataType,
                                     strides.data(), 0, acl_format,
...
inline void Release(aclIntArray* p) {
    ...
    OPS_ERR_IF(aclDestroyIntArray == nullptr,
               OPS_LOG_E("aclnnfallback", "aclDestroyIntArray is null"),
               return);
    aclDestroyIntArray(p);
}

inline void Release(aclBoolArray* p) {
    static const auto aclDestroyBoolArray =
        GET_OP_API_FUNC(aclDestroyBoolArray);
    OPS_ERR_IF(aclDestroyBoolArray == nullptr,
               OPS_LOG_E("aclnnfallback", "aclDestroyBoolArray is null"),
               return);
...
inline void Release(aclTensorList* p) {
    ...
    OPS_ERR_IF(aclDestroyTensorList == nullptr,
               OPS_LOG_E("aclnnfallback", "aclDestroyTensorList is null"),
               return);
...
auto ConvertTypes(Ts&... args)
    -> decltype(std::make_tuple(ConvertType(args)...)) {
...
#define EXEC_OPAPI_CMD(aclnn_api, ...)                                         \
    ({                                                                         \
        ...
        do {                                                                   \
            ...
            if (getWorkspaceSizeFuncAddr == nullptr ||                         \
                opApiFuncAddr == nullptr ||                                    \
                ResetCacheThreadLocalAddr == nullptr) {                        \
                OPS_LOG_E("aclnnfallback",                                     \
                          "%s or %s not in  %s or %s  or "                     \
                          "ResetCacheThreadLocal not found.",                  \
                          #aclnn_api "GetWorkspaceSize", #aclnn_api,           \
                          GetOpApiLibName(), GetOpApiLibName());               \
                ret = GRAPH_FAILED;                                            \
                break;                                                         \
            }                                                                  \
            auto ResetCacheThreadLocalFunc =                                   \
                reinterpret_cast<ResetCacheThreadLocal>(                       \
                    ResetCacheThreadLocalAddr);                                \
            ...
            static auto getWorkspaceSizeFunc = ConvertToOpApiFunc(             \
                converted_params, getWorkspaceSizeFuncAddr);                   \
            ...
            if (workspace_size > 0) {                                          \
                workspace_addr =                                               \
                    host_api_ctx->MallocWorkspace(workspace_size);             \
                if (workspace_addr == nullptr) {                               \
                    OPS_LOG_E("aclnnfallback",                                 \
                              "call %s allocate workspace failed",             \
                              #aclnn_api);                                     \
                    ret = GRAPH_FAILED;                                        \
                    break;                                                     \
                }                                                              \
            ...
            auto acl_call = [converted_params, workspace_addr, workspace_size, \
                ...
                auto api_ret = opApiFunc(workspace_addr, workspace_size,       \
                                         executor, acl_stream);                \
                ReleaseConvertTypes(converted_params);                         \
                host_api_ctx->FreeWorkspace();                                 \
                if (api_ret != 0) {                                            \
                    OPS_LOG_E("aclnnfallback",                                 \
                              "call %s allocate workspace failed api_ret: %d", \
                              #aclnn_api, api_ret);                            \
                    return GRAPH_FAILED;                                       \
                }                                                              \
                return api_ret;                                                \
