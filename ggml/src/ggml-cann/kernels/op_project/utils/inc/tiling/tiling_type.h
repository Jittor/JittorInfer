...
constexpr uint64_t RecursiveSum(T templateId, Args... templateIds) {
    return static_cast<uint64_t>(templateId) +
           10 * RecursiveSum(templateIds...);
}

// TilingKey 的生成规则：
// FlashAttentionScore/FlashAttentionScoreGrad 十进制位组装tiling
// key，包含以下关键参数，从低位到高位依次是：Ub0, Ub1, Block, DataType, Format,
// Sparse, 特化模板 Ub0、Ub1:
...
//     DataType: 表示当前tiling
//     key支持的输入输出的数据类型，使用枚举SupportedDtype来表示，占一个十进制位
//     Format: 表示当前tiling key支持的Format,
//     使用枚举InputLayout表示，占一个十进制位 Sparse: 表示当前tiling
//     key是否支持Sparse，使用枚举SparseCapability表示，占一个十进制位
//     其余特化场景，定义自己的位域和值
// usage: get tilingKey from inputed types
//     uint64_t tilingKey = GET_FLASHATTENTION_TILINGKEY(AxisEnum::AXIS_S1,
//     AxisEnum::AXIS_S2, AxisEnum::AXIS_N2,
//                                     SupportedDtype::FLOAT32,
//                                     InputLayout::BSH,
...
#define TILINGKEY(ub2, ub1, block, dtype, layout, sparse)         \
    (GET_TILINGKEY(AxisEnum::ub2, AxisEnum::ub1, AxisEnum::block, \
