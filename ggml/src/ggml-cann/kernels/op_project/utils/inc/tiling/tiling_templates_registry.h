...
using TilingClassCase =
    std::unique_ptr<TilingBaseClass> (*)(gert::TilingContext *);
    ...
    void AddTiling(int32_t priority) {
        OPS_ERR_IF(cases_.find(priority) != cases_.end(),
                   OPS_REPORT_VECTOR_INNER_ERR(
                       op_type_, "There are duplicate registrations."),
                   return);
        ...
        OPS_ERR_IF(
            ...
            OPS_REPORT_VECTOR_INNER_ERR(
                op_type_,
                "Register op tiling func failed, please check the class name."),
            return);
    }
   ...
   public:
    ...
    std::shared_ptr<TilingCases> RegisterOp(const std::string &op_type) {
        if (registry_map_.find(op_type) == registry_map_.end()) {
            registry_map_[op_type] = std::shared_ptr<TilingCases>(
                new (std::nothrow) TilingCases(op_type));
        }
        OPS_ERR_IF(
            registry_map_[op_type] == nullptr,
            OPS_REPORT_VECTOR_INNER_ERR(
                op_type,
                "Register tiling func failed, please check the class name."),
            return nullptr);
    ...
    ge::graphStatus DoTilingImpl(gert::TilingContext *context) {
        ...
        for (auto it = tilingTemplateRegistryMap.begin();
             it != tilingTemplateRegistryMap.end(); ++it) {
            ...
            if (tilingTemplate != nullptr) {
                ...
                if (status != ge::GRAPH_PARAM_INVALID) {
                    OPS_LOG_D(context,
                              "Do general op tiling success priority=%d",
                              it->first);
                    return status;
                }
                OPS_LOG_D(context, "Ignore general op tiling priority=%d",
                          it->first);
            }
        }
        OPS_REPORT_VECTOR_INNER_ERR(
            op_type, "Do op tiling failed, no valid template is found.");
    ...
    const std::map<int32_t, TilingClassCase> &GetTilingTemplates(
        const std::string &op_type) {
        OPS_ERR_IF(registry_map_.find(op_type) == registry_map_.end(),
                   OPS_REPORT_VECTOR_INNER_ERR(
                       op_type,
                       "Get op tiling func failed, please check the op name."),
   ...
   public:
    ...
    Register &tiling(int32_t priority) {
        ...
        OPS_ERR_IF(
            tilingCases == nullptr,
            OPS_REPORT_VECTOR_INNER_ERR(
                op_type_, "Register op tiling failed, please the op name."),
            return *this);
