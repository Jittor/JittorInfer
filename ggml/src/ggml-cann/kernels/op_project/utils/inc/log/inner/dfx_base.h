...
class LogBase {
   public:
    ...
    static inline std::string GetOpInfo(
        const gert::TilingParseContext *context) {
        return GetOpInfoFromContext(context);
    }
    ...
    static inline std::string GetOpInfo(
        const gert::InferDataTypeContext *context) {
        return GetOpInfoFromContext(context);
    }
    ...
    do {                                                                       \
        if (AlogCheckDebugLevel(static_cast<int>(MOD_ID), (LOG_LEVEL)) == 1) { \
            AlogRecord(static_cast<int>(MOD_ID), DLOG_TYPE_DEBUG, (LOG_LEVEL), \
                       "[%s:%d][%s]%s[%s][%lu] OpName:[%s] " #FMT, __FILE__,   \
                       __LINE__, (OPS_UTILS_LOG_SUB_MOD_NAME),                 \
