...
public:
    PpMatmulTilingApi(uint32_t numBatch, uint32_t m, uint32_t k, uint32_t n, bool transA, bool transB, bool enDequant,
        ...
        : context_(context), numBatch_(numBatch), m_(m), k_(k), n_(n), transA_(transA), transB_(transB), enDequant_(enDequant),
...
float PpMatmulTilingApi::GetCost(const uint32_t m0, const uint32_t n0)
{
    ...
    uint32_t nOnce = blockDim < nLoop ? ascendcPlatform.GetCoreNumAic() * n0 : n_;
...
public:
    ...
    void RmsNormQuantTiling(const uint32_t numTokens, const uint32_t numVectorCore);
    void RopeConcatTiling(const int64_t ntokens, const int64_t headNum, const uint32_t &aicNum);
    void EinSumQuantTiling(const int64_t ntokens, const int64_t headNum, const int64_t quantMode, const uint32_t &aicNum, const ge::DataType inDtype);
    void SetTilingKey(gert::TilingContext* context, int64_t cacheMode, uint64_t quantMode);
    void SetMlapoWorkSpace(const ge::DataType inDtype, int64_t cacheMode, uint64_t headNum, uint64_t quantMode, gert::TilingContext* context);
};

void MlaPreprocessTiling::RmsNormQuantTiling(const uint32_t numTokens, const uint32_t numVectorCore)
{
...
void MlaPreprocessTiling::RopeConcatTiling(const int64_t ntokens, const int64_t headNum, const uint32_t &aicNum)
{
...
void MlaPreprocessTiling::EinSumQuantTiling(const int64_t ntokens, const int64_t headNum, const int64_t quantMode, const uint32_t &aicNum,
                                            const ge::DataType inDtype)
{
    ...
    if (inDtype == ge::DT_BF16 || static_cast<QuantMode>(quantMode) == QuantMode::PER_TOKEN_SYMM_QUANT) {
...
void MlaPreprocessTiling::SetTilingKey(gert::TilingContext* context, int64_t cacheMode, uint64_t quantMode)
{
    // TODO: 确认一下是不是StorageFormat
    ge::Format formatWeight1 = context->GetInputTensor(INDEX_WDQKV)->GetStorageFormat();
    ...
    uint64_t tilingKey = static_cast<uint64_t>(context->GetInputTensor(0)->GetDataType() == ge::DT_BF16);
    tilingKey = (tilingKey << 2) + static_cast<uint64_t>(cacheMode); // 2bit for cacheMode.
    ...
    tilingKey = (tilingKey << 2) + static_cast<uint64_t>(quantMode); // 2bit for quantMode.
    context->SetTilingKey(tilingKey);
    std::cout << "tilingKey is: " << tilingKey << std::endl;
}

std::ostream &operator<<(std::ostream &os, optiling::PpMatmulTilingData &tilingData) {
  os << "(bSize, mSize, kSize, nSize) = ("
     << tilingData.get_numBatch() << ", "
     << tilingData.get_m() << ", "
     << tilingData.get_k() << ", "
     << tilingData.get_n() << ")"
     << std::endl;
  ...
  os << "(mLoop, kLoop, nLoop) = ("
     << tilingData.get_mLoop() << ", "
     << tilingData.get_kLoop() << ", "
     << tilingData.get_nLoop() << ")"
     << std::endl;

  os << "coreLoop = " << tilingData.get_coreLoop() << std::endl;

  os << "(SwizzleDirect, SwizzleCount) = ("
     << tilingData.get_swizzleDirect() << ", "
     << tilingData.get_swizzleCount() << ")"
     << std::endl;
...
std::ostream &operator<<(std::ostream &os, optiling::MLAPreprocessTilingData &tilingData)
{
    ...
    os << "maxNPerLoopForUb = " << tilingData.get_maxNPerLoopForUb() << std::endl;
    os << "preCoreLoopTime = " << tilingData.get_preCoreLoopTime() << std::endl;
    os << "preCoreLoopNLast = " << tilingData.get_preCoreLoopNLast() << std::endl;
    os << "lastCoreLoopTime = " << tilingData.get_lastCoreLoopTime() << std::endl;
    os << "lastCoreLoopNLast = " << tilingData.get_lastCoreLoopNLast() << std::endl;
    os << "esqFrontCore      = " << tilingData.get_esqFrontCore() << std::endl;
    os << "esqTailCore       = " << tilingData.get_esqTailCore() << std::endl;
    os << "esqFrontCoreBatch = " << tilingData.get_esqFrontCoreBatch() << std::endl;
    os << "esqTailCoreBatch  = " << tilingData.get_esqTailCoreBatch() << std::endl;
    ...
    os << "esqHeadPerLoop    = " << tilingData.get_esqHeadPerLoop() << std::endl;
...
void MlaPreprocessTiling::SetMlapoWorkSpace(const ge::DataType inDtype, int64_t cacheMode, uint64_t headNum, uint64_t quantMode, gert::TilingContext* context)
{
    uint64_t s1wsFactor = static_cast<uint64_t>(
        cacheMode == 2 ?
        std::max(HIDDEN_STRATE * sizeof(int8_t), headNum * AXES_ALIGN_SIZE * sizeof(uint16_t)) :
        HIDDEN_STRATE * sizeof(int8_t)
    );
    uint64_t workSizeS1 = static_cast<uint64_t>(tilingData.get_n()) * s1wsFactor;
    uint64_t workSizeS2 = static_cast<uint64_t>(tilingData.get_n()) * headNum * HIDDEN_STRATE_ROPE * sizeof(uint16_t);
    uint64_t workSizeS3 = static_cast<uint64_t>(tilingData.get_n()) * HIDDEN_STRATE_MM * sizeof(uint16_t);
    uint64_t workSizeS4 = static_cast<uint64_t>(tilingData.get_n()) *
                          std::max((uint32_t)headNum * HIDDEN_STRATE_ROPE, HIDDEN_STRATE_MM) * sizeof(uint32_t);
    uint64_t pertokenWorkspace = static_cast<uint64_t>(tilingData.get_n()) * sizeof(float) * 2;
    ...
    size_t usrSize = 0;
    if (inDtype == ge::DT_BF16 || static_cast<QuantMode>(quantMode) == QuantMode::PER_TOKEN_SYMM_QUANT) {
        usrSize = maxWorkspaceSize + maxWorkspaceSize + maxWorkspaceSize + maxWorkspaceSize + pertokenWorkspace;
    ...
    // printf("usrSize: %lu, maxWorkspaceSize: %lu, pertokenWorkspace: %lu, workSizeS1: %lu, workSizeS2: %lu, workSizeS3: %lu, workSizeS4: %lu\n", usrSize, maxWorkspaceSize, pertokenWorkspace, workSizeS1, workSizeS2, workSizeS3, workSizeS4);
