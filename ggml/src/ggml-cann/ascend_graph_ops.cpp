 ...
 * 输出: output (expanded x), row_idx (expanded row indices), token_count
 * (tokens per expert)
 *
 * @param graph 计算图引用
 * @param node 当前节点
...
void handle_moe_init_routing_op(
    ...
    if (ggml_tensor_to_ge_op_map.find(expert_idx) !=
        ggml_tensor_to_ge_op_map.end()) {
    ...
    // 确保维度合法
    x_op =
        create_reshape_op(graph, x_op, {x->ne[1], x->ne[0]},
                          "moe_init_routing_reshape_x" + std::to_string(op_idx),
                          get_data_type(x->type));
    expert_idx_op = create_reshape_op(
        graph, expert_idx_op, {expert_idx->ne[1], expert_idx->ne[0]},
        "moe_init_routing_reshape_expert_idx" + std::to_string(op_idx),
        get_data_type(expert_idx->type));
    ...
    moe_init_op.set_attr_expert_tokens_count_or_cumsum_flag(
        1);  // 1表示输出前缀和
    ...
    token_count_identity.set_input_x(moe_init_op,
 ...
 * 输入: x (input tensor), weight (expert weights), token_count (tokens per
 * expert) 输出: result (grouped matmul result)
 *
 * @param graph 计算图引用
 * @param node 当前节点
 * @param ggml_tensor_to_ge_op_map 张量到算子的映射
...
ge::Operator handle_moe_grouped_matmul_op(
    ...
    if (ggml_tensor_to_ge_op_map.find(weight) !=
        ggml_tensor_to_ge_op_map.end()) {
    ...
    if (ggml_tensor_to_ge_op_map.find(token_count) !=
        ggml_tensor_to_ge_op_map.end()) {
        token_count_op = ggml_tensor_to_ge_op_map[token_count];
    } else {
        assert(false &&
               "MOE_GROUPED_MATMUL: token_count tensor not found in map");
    }
    ...
    // 确保维度合法
    x_op = create_reshape_op(
        graph, x_op, {x->ne[1], x->ne[0]},
        "moe_grouped_matmul_reshape_x" + std::to_string(op_index),
        get_data_type(x->type));
    weight_op = create_reshape_op(
        graph, weight_op, {weight->ne[2], weight->ne[1], weight->ne[0]},
        "moe_grouped_matmul_reshape_weight" + std::to_string(op_index),
        get_data_type(weight->type));
    token_count_op = create_reshape_op(
        graph, token_count_op, {token_count->ne[0]},
        "moe_grouped_matmul_reshape_token_count" + std::to_string(op_index),
        get_data_type(token_count->type));
...
ge::Operator handle_moe_finalize_routing_op(
    ...
    if (ggml_tensor_to_ge_op_map.find(row_idx) !=
        ggml_tensor_to_ge_op_map.end()) {
        row_idx_op = ggml_tensor_to_ge_op_map[row_idx];
    } else {
        assert(false &&
               "MOE_FINALIZE_ROUTING: row_idx tensor not found in map");
    }

    if (ggml_tensor_to_ge_op_map.find(scales) !=
        ggml_tensor_to_ge_op_map.end()) {
    ...
    x_op = create_reshape_op(
        graph, x_op, {x->ne[1], x->ne[0]},
        "moe_finalize_routing_reshape_x" + std::to_string(op_index),
        get_data_type(x->type));
    row_idx_op = create_reshape_op(
        graph, row_idx_op, {row_idx->ne[0]},
        "moe_finalize_routing_reshape_row_idx" + std::to_string(op_index),
        get_data_type(row_idx->type));
    scales_op = create_reshape_op(
        graph, scales_op, {scales->ne[1], scales->ne[0]},
        "moe_finalize_routing_reshape_scales" + std::to_string(op_index),
        get_data_type(scales->type));
...
ge::Operator handle_moe_swiglu_op(
    ...
    x_op = create_reshape_op(graph, x_op, target_shape,
                             "moe_swiglu_reshape_x" + std::to_string(op_index),
                             get_data_type(x->type));
...
ge::Operator handle_flash_attn_jittor_v1_op(
    ...
    std::string flash_attn_name =
        "flash_attn_jittor_" + std::to_string(op_index);
    ...
    flash_attn_jittor_op.set_attr_numKeyValueHeads(
        num_key_value_heads);                                   // 默认 0
...
#define GET_TENSOR_AND_OP(name, i, op_name)                              \
    ...
    do {                                                                 \
        if (gmml_tensor_to_ge_op_map.find(name) !=                       \
            gmml_tensor_to_ge_op_map.end()) {                            \
...
#define SQUEEZE_OP(op, prefix, ...)                 \
    ge::Operator squeezed_##op = create_squeeze_op( \
        graph, #prefix #op "_", op_suffix, op, {__VA_ARGS__})
...
ge::Operator handle_mla_preprocess_op(
    ...
    ge::op::MLAPreprocess mla_preprocess_op(
        ("mla_preprocess" + op_suffix).c_str());
    assert(N == 32);
    assert(headNum == 128);
    assert(cacheMode == 1);
    assert(quantMode == 0);
    ...
    mla_preprocess_op.set_input_hiddenState(squeezed_op_hiddenState)
    ...
    ge::Operator q1_identity_op = create_identity_op_by_name(
        graph, "mla_preprocess_q1_", op_suffix, mla_preprocess_op, "q1");
    ge::Operator q2_identity_op = create_identity_op_by_name(
        graph, "mla_preprocess_q2_", op_suffix, mla_preprocess_op, "q2");

    ge::Operator concat_op =
        create_concat_op(graph, "mla_preprocess_", op_suffix,
                         {q1_identity_op, q2_identity_op}, 2);

    ge::DataType data_type = get_data_type(node->type);
    ge::TensorDesc desc_out(ge::Shape({N, headNum, 512 + 64}), ge::FORMAT_ND,
                            data_type);
...
ge::Operator handle_mla_op(
    ...
    if (gmml_tensor_to_ge_op_map.find(query_rope) !=
        gmml_tensor_to_ge_op_map.end()) {
    ...
    mla_params *params = reinterpret_cast<mla_params *>(node->op_params);
    ...
    ge::op::FusedInferAttentionScore mla_op(mla_name);

    // q, k, v
    mla_op.set_input_query(op_query);
    mla_op.create_dynamic_input_byindex_key(1, 1);
    mla_op.set_dynamic_input_key(0, op_context_kv);
    mla_op.create_dynamic_input_byindex_value(1, 2);
    mla_op.set_dynamic_input_value(0, op_context_kv);

    // rope
    mla_op.set_input_query_rope(op_query_rope);
    mla_op.set_input_key_rope(op_key_rope);

    // actual seq length
    mla_op.set_input_actual_seq_lengths_kv(op_context_length);
    mla_op.set_input_block_table(op_block_tables);

    // attrs
    mla_op.set_attr_num_heads(headNum);
    mla_op.set_attr_num_key_value_heads(kvHeadNum);
    mla_op.set_attr_scale(qkScale);
    mla_op.set_attr_input_layout(input_layout);
    mla_op.set_attr_block_size(blockSize);
    ...
    // 设置输出描述
    ge::TensorDesc desc_out(ge::Shape(output_shape), ge::FORMAT_ND, dataType);
    mla_op.update_output_desc_attention_out(desc_out);

    // 添加算子到图中
    graph.AddOp(mla_op);

    std::string identity_name = "mla_identity_" + std::to_string(op_index);
    ge::op::Identity identity_op(identity_name);
    identity_op.set_input_x_by_name(mla_op, "attention_out");
    identity_op.update_output_desc_y(desc_out);
    graph.AddOp(identity_op);
