...
static common_init_result common_init_from_params_local() {
    ...
    if (llama_model_has_decoder(model)) {
        llama_decode(lctx, llama_batch_get_one(tmp.data(), std::min(tmp.size(), (size_t) default_mini_params.n_batch)), true);
    }
...
static void recv_batch_list(int * all_server_tokens, int * self_server_tokens) {
    MPI_Allgather(self_server_tokens, 1, MPI_INT, all_server_tokens, 1, MPI_INT, MPI_COMM_WORLD);
}
...
int main(int argc, char** argv) {
    ...
    while (true) {
        ...
        // 这已经是decode阶段了，n_batch数量等于待处理序列数量.prefill阶段以及全部处理结束seq_id = -1
        ...
        // 这个阶段是prefill，结束后将所有client的tokens连成一个batch，唯一一个n_tokens很长的阶段，decode阶段g_seq_id == n_seq
            ...
            for (auto & client : clients) {
                ...
                if (client.n_decoded > 2 &&
                        (llama_vocab_is_eog(vocab, id) ||
                         (default_mini_params.n_predict > 0 && client.n_decoded + client.n_prompt >= default_mini_params.n_predict) ||
        ...
        // printf("input %s; output %s\n\n", ::trim(clients[0].input).c_str(), ::trim(clients[0].response).c_str());
    ...
    printf("Total speed (AVG):   %6s  speed: %5.2f t/s\n", "", (double) (llama_all_processed_tokens(ctx)) / (t_main_end - t_main_start) * 1e6);
